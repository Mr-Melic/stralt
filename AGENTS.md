...
- Migrations: `mops.toml` chain `src/backend/migrations`, `check-limit = 4`, empty-canister baseline `.old/src/backend/dist/backend.most`. Lex order: `20260826_000000.mo` (empty-canister genesis for Caffeine M0263), `20260827_000000.mo` (drop transients), `20260831_000000.mo` (**frozen** after Caffeine applied it: SpellConfig summon fields + rollback stables + `adminAuditLog` — do not add fields here), `20260901_000000.mo` (GameKey maps: `gameKeyRequests` / `gameKeyLedger` / `gameKeyReveals` / `lastGameKeyRequestAt` / `nextGameKeyRequestId`). Live `main.mo` is a plain persistent `actor {` (no `(with migration = Migration.run)`). **mops injects the chain** via `--enhanced-migration`. That annotation exists only on `backend_extended`. `BaseToCore.mo` is the completed mo:base→mo:core marker. Adding stables by stuffing them into an already-shipped `NewActor` (the #258 GameKey maps on `20260831`) causes Caffeine `install_code` to trap: `RTS error: Memory-incompatible program upgrade`. Always add a **new later** file; `OldActor` = the deployed tail (previous `NewActor` without the new fields).
