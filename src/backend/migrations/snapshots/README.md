...
**Stralt_V2** actually installed v356 (PR #181 merge `f8aa05e`): chain
`20260826/20260827/20260831`, 42 stables, no GameKey. `.old` in the repo is a
byte-identical copy of that build's `.most` (sha256 `3cf210e1…`) so `mops check`
here behaves exactly like Caffeine's. After the next successful Caffeine deploy
of this fork, replace `.old` with that build's `src/backend/dist/backend.most`.

The original stralt project's canister `zh6cg-aaaaa-aaaad-aar2q-cai` never ran
any imported GitHub build; Caffeine still recorded v356 when the frontend
deploy succeeded. That is why every later import trapped IC0503 there. This
fork does not have that problem.
...
a chain file, the cumulative chain type at that name must equal what that
canister holds, and every stable the deployed tail lacks must be produced by a
**later** chain file. Stralt_V2 genesis stays `20260826_000000` (v356 recorded
it). Do not rename to `20260801` or add `20260803_185500` (that name sorts
before genesis).
...
| :--- | :--- | :--- | :--- |
| `../../../../.old/src/backend/dist/backend.most` (tracked) | `20260831_000000` (no GameKey) | v356 / Stralt_V2 (PR #181 merge `f8aa05e`, sha256 `3cf210e1…`) | yes (`mops check`) |
| `deployed/caffeine-aug31-import-tail-20260831-no-gamekey.most` | same | same content with a provenance header | yes |
| `deployed/caffeine-354-tail-20260827.most` | `20260827_000000` | Caffeine build #354 `10d44dd` — still a valid prefix of this chain | yes |
| `deployed/pr259-tail-20260901.most` | `20260901_000000` | stralt main between PR #259 and #311, fresh install | yes (head of the chain, nothing runs) |
| `empty-canister.most` | none | brand-new Caffeine project / first install | yes (whole chain runs) |
| `unsupported/pr258-tail-20260831-gamekey.most` | `20260831_000000` (with GameKey) | PR #258 `58302bc` | **no** — same name as the deployed tail but five extra fields (M0169 / Memory-incompatible) |
| `unsupported/caffeine-340-legacy-no-chain.most` | none, 37 legacy stables | Caffeine build #340 `cfe614b` | **no** |
| `unsupported/caffeine-348-tail-20260803.most` | `20260803_185500` | Caffeine build #348 `214fc41` | **no** on Stralt_V2 — that name is not a chain file (adding it would sort before `20260826`) |
...
PocketIC (`scripts/eop-upgrade-matrix.mjs` / `upgrade-paths.json`): empty → HEAD,
v356-fresh (`9bf8368`) → HEAD, HEAD → HEAD.
