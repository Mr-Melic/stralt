/**
 * Battle-start destack can sit hostiles on every generate-time dump alcove.
 * Unique player→exit bridges then have dump ≥ 2 (#494 / #600) but free dump
 * = 0, so a corpse or summon still seals the unlocked portal.
 *
 * Generate-time `ensureFreeProgressionDump` (#538) runs from the pre-fight
 * spawn. Destack dump=0 (#589) and destack dump-floor-2 (#600) count cells
 * without occupancy. This is the destack-time free-dump restore: punch until
 * the fight graph has one unoccupied dump cell. Punches that would join a
 * leftover island or a corridor around a portal choke are skipped.
 *
 * WorldExploration destack should call this after `ensureDumpFloorAfterBattleStart`
 * (#600). This PR does not import it from WX — that adjacent destack hunk
...
/** Same floor as generate-time `PROGRESSION_FREE_DUMP_FLOOR` (#538). */
export const BATTLE_START_FREE_DUMP_FLOOR = 1;
 ...
 * tile, expand to the largest adjacent floor component so free-dump
 * counting does not see an empty graph.
 */
export function floodFreeDumpBattleFrom(
  tiles: string[][],
...
/**
 * Fight-graph dump cells that destacked hostiles do not occupy. Far-island
 * floors past a portal choke do not count — corpses relocated there cannot
 * unseal the unlocked gate. Occupied alcoves still count as `dump` so
 * destack dump-floor-2 can no-op while free dump stays 0.
 */
export function countFreeBattleDumpCells(
  tiles: string[][],
  voidTiles: Set<string> | Map<string, unknown> | undefined,
  ...
  const battle = floodFreeDumpBattleFrom(tiles, vt, playerSpawn, portals, w, h);
 ...
 * {@link BATTLE_START_FREE_DUMP_FLOOR} unoccupied dump cells. Returns how
 * many walls were opened (0 when free dump already meets the floor / no
 * unique bridge / no safe wall neighbor).
 */
export function ensureFreeDumpAfterBattleStart(
  tiles: string[][],
  voidTiles: Set<string> | Map<string, unknown> | undefined,
  playerSpawn: { x: number; y: number },
  portals: { x: number; y: number }[],
  ...
  let punched = 0;
  for (let i = 0; i < BATTLE_START_FREE_DUMP_FLOOR * 8; i++) {
    const counts = countFreeBattleDumpCells(
      tiles,
      vt,
      playerSpawn,
      portals,
    ...
    if (counts.mandatory === 0 || counts.free >= BATTLE_START_FREE_DUMP_FLOOR) {
      return punched;
    }
    const reachable = floodFreeDumpBattleFrom(
      tiles,
      vt,
      playerSpawn,
      portals,
    ...
    const cell = punchSafeFreeAlcove(tiles, vt, reachable, exclude, w, h);
    if (!cell) return punched;
    punched += 1;
    const next = countFreeBattleDumpCells(
      tiles,
      vt,
      playerSpawn,
      portals,
