/**
 * Overworld wander can sit hostiles on every generate-time dump alcove
 * (`isEnemyWanderFloor` only keeps them on the fight graph — it does not
 * reserve dump cells). Unique player→exit bridges then still report dump ≥ 1
 * so generate-time alcove / dump-floor helpers no-op, but free dump is 0 and
 * a corpse or summon seals the unlocked progression portal.
 *
 * Unique extra vs generate-time free-dump (#538: occupancy at spawn, before
 * wander) and destack free-dump (#589 / #600 / #603: occupancy after battle
 * start). This pass restores a free fight-graph dump after wander.
 *
 ...
 * `simulateEnemyWanderOnWorld` / live wander ticks. This module is not
 * imported from WX or `mapGen.simulate.ts` — those files 3-way on the
...
export const WANDER_FREE_DUMP_FLOOR = 1;
 ...
 * portal tile, expand to the largest adjacent floor component so free-dump
 * counting does not see an empty graph.
 */
export function floodWanderDumpBattleFrom(
  tiles: string[][],
...
/**
 * Fight-graph dump cells that wandered hostiles do not occupy. Far-island
 ...
 * generate-time dump-floor helpers can no-op while free dump stays 0.
 */
export function countFreeWanderDumpCells(
  tiles: string[][],
  voidTiles: Set<string> | Map<string, unknown> | undefined,
 ...
 * {@link WANDER_FREE_DUMP_FLOOR} unoccupied dump cells. Returns how
 * many walls were opened (0 when free dump already meets the floor / no
 * unique bridge / no safe wall neighbor).
 */
export function ensureFreeDumpAfterWander(
  tiles: string[][],
  voidTiles: Set<string> | Map<string, unknown> | undefined,
  playerSpawn: { x: number; y: number },
  portals: { x: number; y: number }[],
  ...
  let punched = 0;
  for (let i = 0; i < WANDER_FREE_DUMP_FLOOR * 8; i++) {
    const counts = countFreeWanderDumpCells(
      tiles,
      vt,
      playerSpawn,
      portals,
    ...
    if (counts.mandatory === 0 || counts.free >= WANDER_FREE_DUMP_FLOOR) {
      return punched;
    }
    const reachable = floodWanderDumpBattleFrom(
      tiles,
      vt,
      playerSpawn,
      portals,
    ...
    punched += 1;
    const next = countFreeWanderDumpCells(
      tiles,
      vt,
      playerSpawn,
      portals,
