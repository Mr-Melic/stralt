    ...
    const timeout = setTimeout(() => {
      ...
      if (summonEnemy?.isSummon && summonEnemy.side === "player") {
        ...
        const executorHelpers: SummonExecutorHelpers = {
          ...
          getTargetPos: (id: string) => {
            if (id === "player" || id === "__player__") {
              return playerPositionRef.current;
            }
            const found = enemiesRef.current.find((e: any) => e.id === id);
            return found
              ? { x: Number(found.x), y: Number(found.y) }
              : undefined;
          },
          onSpentCast: ({ caster, tiles }) => {
            const nextDirect = applyChallengeDirectHitOnCast(
              {
                stillDirect: challengeDirectHitRef.current,
                attempts: challengeDirectHitAttemptsRef.current,
