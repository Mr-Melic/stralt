...
describe("clampSaveBattleStatsWrite", () => {
  it("lets death and heals cut Doka/XP but refuses a mint or level rewrite", () => {
    ...
    // Failure: saveBattleStats used min(stored, client) for level, so a
    // stale/custom _level=1 demoted a post-applyRewards character.
    // Canister saveBattleStats still writes min(stored, client) for level
    // (SDEG-2026-09-01-001; vehicle #209 / #215). This helper must keep
    // stored level so a stale/custom _level=1 cannot demote after applyRewards.
    assert.deepEqual(
