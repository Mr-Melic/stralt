# ACTION_IDs — Content Diversity & Repetition Auditor 2026-09-26

Durable ledger for implementers and the Report Action Orchestrator.  
Source of every record: Content Diversity & Repetition Auditor.  
Audit: [`CONTENT_DIVERSITY_AUDIT_2026-09-26.md`](./CONTENT_DIVERSITY_AUDIT_2026-09-26.md).  
HEAD: `0f5363f` (unchanged since 2026-09-21). Gameplay / production code was **not** modified this run.

Prior ledger on `main`: [`ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md`](./ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md) (`CDA-2026-09-02-001` … `015`). Line numbers below match live HEAD.

Unpublished ledgers: GitHub **PR #359** (09-21, `016`–`018`), **PR #403** (09-22, copies `016`–`018` + `019`), **PR #465** (09-23, copies `016`–`019` + `020`), **PR #521** (09-24, copies `016`–`020` + `021`), **PR #557** (09-25, copies `016`–`021` + `022`). All still **OPEN**. Do **not** mint twins. Those ids are copied here so they exist on `main` if those PRs never merge. Status stays **NEW** until an implementer picks them.

This run mints **`CDA-2026-09-26-023`** only.
...
## Status of CDA-2026-09-02-001 … 015, unpublished 016–022, and 023
...
| CDA-2026-09-02-013 | PARTIAL | Chrome = FLAVOR LORE. Per-row copy still lies — CDA-016 |
| CDA-2026-09-02-014 | OPEN | Dungeon extras still `[0,2,3,4,4,5]`; 52 `WF-*` unused on `main` |
...
| CDA-2026-09-22-019 | NEW | Unpublished in #403. Freeze ENC / Rush *code* until 014/008 |
| CDA-2026-09-23-020 | NEW | Unpublished in #465. Freeze Wave-6 TypeScript catalogs |
| CDA-2026-09-24-021 | NEW | Unpublished in #521. Freeze Wave-7 WF TypeScript dumps (`#503` 52→116) until 001/014/008/018 |
| CDA-2026-09-25-022 | NEW | Unpublished in #557. Freeze Wave-7 elite / SDE / formation TypeScript (`#535` / `#533` / `#537`) until 001 / 002+006 / 004 |
| CDA-2026-09-26-023 | NEW | **This run.** Freeze Wave-8/9 TypeScript + next-wave docs (`#578` 52→132 `WF-*`; `#558`/`#590`/`#575`/`#572`/`#574`/`#563`) until 001 / 002+006 / 004 / 008 / 014 |
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: Seven `EnemyFamily` ids (`gameTypes.ts` 12–20). 30% overlay (`spawnPolicy.ts` 35, 279–286; WX 5862–5866) writes HP/RES/SP then battle start overwrites HP via `calcEnemyMaxHp` (3607–3612, 11970–11997) and RES/SP via `computeEnemyStats` (11873, 11898–11902). Overlay `mp`/`ap` never assigned (`spawnPolicy.ts` 14–15, 64–66). Family RES `0.05–0.75` is the wrong unit vs integer `getEnemyBaseStats`. Only ember melee burn, tide melee MP−1 (WX 16789–16821), void 25% reflect (`castHelpers.ts` 335–345) exist. Chassis stays a random chess piece. Re-verified 2026-09-26 on HEAD `0f5363f`.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: No `isElite` / variant type (repo-wide grep empty). Leader = highest level (WX 12010–12015). `computeAITier` (`combatMath.ts` 36–51) 30% uniform 1–10. `WF-ELT-BANNER_PATROL` / `WF-ELT-TOLL_KEEPER` / `WF-ELT-CART_GUARD` (`worldFeatures.ts` 452, 1011, 1489) are unwired. Re-verified 2026-09-26.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Live placement is 4-quadrant + Chebyshev ≥ 4 (`SPAWN_MIN_CHEBYSHEV`) then battle-start re-scatter ≥ 2 (WX 11869–11891). `docs/design/ENEMY_FORMATIONS_*` (including drop 7 **#537**) and `worldFeatures.ts` encounter slots are unwired. Re-verified 2026-09-26.  
...
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: Formation drop docs; CDA-2026-09-02-001 (roles need families/kits); CDA-2026-09-25-022 freeze on extra formation TypeScript  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Shield ≡ Iron Skin (`spellData.ts` 30–47 vs 293–312: +30% RES / 3, range 3; AP 2 vs 3). Poison ≡ Venom (49–67 vs 394–415: 4×3 DoT). Expose ≈ Shadow Veil (`spellData.ts` 373+ vs 480+): damage + `debuffStat: "res_sp"`. Blood Mend ≈ Rallying Cry: self heal + +15% CHC / 2. Boss kits reuse Veil (7), Cursed Wound (6), Frost Nova (6), Swap (5). Re-verified 2026-09-26.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: `inferArchetype` (`enemyAI.ts` 447–452) returns healer if `spellType === "heal"` **or** `healAmount > 0` (Life Drain, Drain Courage, Lifesteal Nova). Never returns `summoner` (grep: no `return "summoner"`). WX uses `isSummoner` instead (16375). `aiStrategy` is written `""` at WX 16160. No family sets `"berserk"`. Knight → flanker by piece name even when kit is Strike-only. Re-verified 2026-09-26.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `ENEMY_SUMMONER_CHANCE_BASE + characterStats.level * ENEMY_SUMMONER_CHANCE_PER_LEVEL_ZONE` (`gameConstants.ts` 298–299; WX 11932–11939). Hits 1.0 at player level 44. Pets 50/50 wolf or archer only (11935–11941). Sentinel / bomber / wisp stay `usableByEnemy: false`. Comment says “scales with levelZone”; code uses raw level. Re-verified 2026-09-26.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: `combinedMechanic` exists only as strings on `BOSS_RUSH_ROOMS` (`useBossRush.ts` 19–131). Grep finds 11 hits, all in that file. Room 9 `boss2Id: "weeping_pawn_2"` is not in `BOSS_IDS` (`bossTypes.ts` 390–410). Admin already documents the lie (`AdminDashboard.tsx` 7144). Kits remix Veil (7), Cursed Wound (6), Frost Nova (6), Swap (5). Phase 2 adds a catalog spell (`bossKits.ts` 90–92). Re-verified 2026-09-26. `BOSS_RUSH_ROOMS.length` still 10.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Nine `DEFAULT_CHALLENGES` (`challengeCompletion.ts` 44–109). Three turn-count clones (15/10/5), three damage clones (50 / 30+no-heal / 0). Unique: `no_healing`, `under_8_ap_per_turn`, `direct_hit`. Honesty patches landed; types did not. Re-verified 2026-09-26.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `defaultAchievements()` (`admin.mo` 309–326): `doka_1000` + `doka_10000`; `betrayal_witness` + `double_betrayal`; `level_10` (`unstoppable`) on a game with no level cap; all rewards Doka. WX already fires `level_10` at 10 (3632). Re-verified 2026-09-26.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `slime_flood` and `frozen_terrain` are identical `onMpCost: ×2` (`mapModifiers.ts` 154–172). Registry hooks for `blood_moon` / `mirror_field` are empty but `spellEngine.ts` 895 applies ×1.25 and 901–907 applies 20% ST reflect. `gravity_well` / `fog_of_war` are empty hooks with announce text. Numeric reskins: `titans_vigor`, `glass_realm`, `doka_fever`. Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `mapModifiers.ts`; WX active-id branches (`isBloodMoon` 2325, `isMirrorField` 2329); `spellEngine.ts`  
RECOMMENDED_ACTION: Frozen must be ice tiles / slip, not a second slime. Document Blood Moon / Mirror Field as live DAMAGE/REACTION (do not add a fifth reflect). Placeholders: implement one distinct verb each (vision cap, attract-on-move) **or** remove from the two-roll. Do not add another ×2 damage modifier. NEW PLAYER DECISION: “Fog: I cannot snipe, I must walk in; Frozen: standing still is safer than sliding into lava.”  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: Occupancy `applyAttract` for gravity_well; do not invent damage formulas; CDA-017 if Mirror Field is collapsed  
REGRESSION_RISK: MEDIUM if fog hides portals (solvability) or gravity pulls onto void  
VALIDATION_REQUIRED: `slime_flood` and `frozen_terrain` produce different battle-log verbs; a listed modifier with empty hooks cannot roll; Blood Moon ×1.25 still skipped for heals  
STATUS: OPEN  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Frontend `SPELL_ID_CATALOG` (`bossKits.ts` 29–62) = 32 ids, all treated as base (WX 2395–2400). Backend `defaultSpells()` (`admin.mo` 168–191) = six different ids with `linear`/`diagonal`/`hitTiles`. `defaultBossConfigs()` still lists `fireball`, `cursed_gust`, `entangle`, `mist_form`. Live kits cannot assign those ids. Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `spellData.ts`; `admin.mo` defaultSpells / defaultBossConfigs; hydrate `ownedSpells`  
RECOMMENDED_ACTION: One catalog. Backend seeds must be ids that exist in frontend metadata. Do not append Wave-2–7 SPELL_PROPOSALS into `starterSpells` (that makes discovery worse). NEW PLAYER DECISION: once innate vs acquired splits, “I do not own Void Collapse yet — this bishop’s frost is information,” which is impossible while 32 ids are pre-owned and six others are a parallel universe.  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: SDE-2026-08-31-001 (innate four); SDA catalog admin; CDA-2026-09-02-005 clone pass; CDA-2026-09-25-022 freeze on Wave-7 SDE TypeScript  
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: Enemy Register lists Crimson Spawn / Shadow Lurker / Storm Caller (`EnemyRegister.tsx` 71–88) that are not `EnemyFamily`. Chrome is FLAVOR LORE (`enemyRegisterCopy.ts` 8–15; banner 356–370). Wave 1–7 sheets already propose dozens of ids (Wave 7 = **#535**). Shipping another sticker repeats CDA-001’s failure at larger scale. `EnemyFamily` union still the original seven + `default` (`gameTypes.ts` 12–20). Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `EnemyRegister.tsx`; `gameTypes.ts` EnemyFamily; elite evolution docs  
RECOMMENDED_ACTION: Implementers must not add `EnemyFamily` members or Register monsters until CDA-001 is live. Register extras should be labeled proposed/unwired **in the row** (CDA-016). Wave-2–7 families stay docs. NEW PLAYER DECISION: none until 001 exists — this id prevents a fake decision (“new name, same Strike pawn”).  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-001 is the gate; CDA-2026-09-25-022 names the Wave-7 elite hour  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Dungeon only adds `[0,2,3,4,4,5]` enemies and `[0,1,2,2,3,3]` tier steps (`spawnPolicy.ts` 27–28; WX 5720–5722). ENC-* catalogs and `worldFeatures.ts` (52 `WF-*` on `main`; shrine, teleport, elite patrol, spell-bearing enemy) are unwired. Live fight structure is always kill-all. Re-verified 2026-09-26.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `applyPushback` / `applyAttract` (`occupancy.ts` 482, 537) have tests and no spell caller. Live POSITIONING is Swap only. REACTION is the reflect cluster (CDA-017). SPELL_PROPOSALS Wave 1 already reserved push, pull, delayed execute, root, real trap, absorb, redirect. Backend `void_collapse` advertises `attract_multi` (`admin.mo` 190) but is not in the frontend catalog. Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `spellData.ts` (one id at a time); occupancy callers; enemy kits only after CDA-002  
RECOMMENDED_ACTION: Consume **one** already-proposed id (push **or** attract, not both in the same drop) as metadata + occupancy call. Do not author a new shred. Do not put it on `starterSpells` (CDA-012 / SDE). Do not ship a fifth reflect. NEW PLAYER DECISION: “if I stand on this file they slam me into lava / pull me off the shrine — my tile choice is the counter,” which no current enemy kit asks.  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: SPELL_PROPOSALS-2026-08-31 push/attract ids; CDA-2026-09-02-012 (do not dual-catalog it); CDA-2026-09-02-002 before enemies get it; CDA-017 owns non-reflect REACTION  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Chrome is honest (`enemyRegisterCopy.ts` 11–15: “Flavor only — not the live spawn roster”; banner `EnemyRegister.tsx` 356–370). The seven live-family rows (`EnemyRegister.tsx` 28–70) still teach wall-phase, poison-on-hit, burning tiles, adjacent slow + regen, Weaken-from-range, magic immunity until physical. Live verbs are only ember melee burn, tide melee MP−1 (WX 16789–16821), void 25% reflect (`castHelpers.ts` 335–345). Crimson / Lurker / Storm (71–88) are not `EnemyFamily`. Re-verified 2026-09-26 on HEAD `0f5363f`.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Four live reflects: `spell-mirror` (next ST, `spellEngine.ts` 917–926); `void_mirror` 25% of pre-crit (`castHelpers.ts` 335–345); `mirror_field` 20% ST (`spellEngine.ts` 901–907); boss `reflectShieldActive` 30% (`castHelpers.ts` 363–375). Backend `reflect_barrier` is a fifth ghost (`admin.mo` 182). 09-02 census listed REACTION as Under; it is a numeric clone cluster. SPELL_PROPOSALS already reserved delayed execute / redirect / absorb — unused. Re-verified 2026-09-26.  
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: `worldFeatures.ts` holds **52** `WF-*` ids (Wave 1–3) on `main`. Grep: imported only by `worldFeatures.test.ts`. WX does not import the module. Open TypeScript dumps: #344 (Wave-4), #399 (Wave-5), #454 (Wave-6, +1436 lines), **#503 (Wave-7, 52→116 ids, +1914 lines)** — CDA-021 — and **#578 (Wave-8, 52→132 ids, +2390 lines)** — CDA-023. CDA-014 already asks to wire **one** object. Re-verified 2026-09-26: still 52 on `main` `0f5363f`.  
SYSTEMS_AFFECTED: `engine/worldFeatures.ts`; WDD / ENC design docs; dungeon `generateEnemies`  
RECOMMENDED_ACTION: Implementers must not append `WF-*` rows until CDA-014 (or equivalent) imports one existing id (`WF-ELT-TOLL_KEEPER` or `WF-ZON-SHRINE_POOL` preferred). Wave-4/5/6/7/8 world-dynamics catalogs stay docs without new TypeScript rows (#344, #399, #454, #503, #578). NEW PLAYER DECISION: none until one object is live — this id prevents a fake decision (“new shrine name, same kill-all pack”).  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-014 is the gate; do not invent a 53rd id to “teach” hold/escort; CDA-2026-09-22-019 for ENC/Rush code freeze; CDA-2026-09-23-020 / CDA-2026-09-24-021 for later-wave TypeScript hours  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Live spawn is still extra-bodies (`spawnPolicy.ts` 27–28) and unread `combinedMechanic` (`useBossRush.ts` 19–131; room 9 `weeping_pawn_2`). 09-21 CDA PR #359 is still open. `BOSS_RUSH_ROOMS.length` still 10. Same-hour siblings include Rush Table E docs (#474) and Ley/Fan/Pit/Font ENC (#479). CDA-013/018 freeze families and `WF-*` rows; this id covers **BOSS_RUSH_ROOMS rows, generateEnemies encounter branches, and ENC TypeScript**. Wave-6 TypeScript is CDA-020; Wave-7 TypeScript is CDA-021. Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `useBossRush.ts` `BOSS_RUSH_ROOMS`; dungeon `generateEnemies`; ENC-* / WDEAD TypeScript if any; Wave-5–8 boss sheets  
...
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-008; CDA-2026-09-02-014; CDA-2026-09-21-018 (WF rows); CDA-2026-09-02-013 (families); CDA-2026-09-23-020; CDA-2026-09-24-021  
...
## Unpublished 09-23 id (copied so it exists if PR #465 never merges)
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: Live spawn on HEAD `0f5363f` is unchanged vs 09-21/09-22: 30% family sticker (`spawnPolicy.ts` 279–286) wiped at battle start; 52 unused `WF-*` rows (WX does not import `worldFeatures.ts`); `BOSS_RUSH_ROOMS.length === 10` with unread `combinedMechanic` and room-9 `weeping_pawn_2` (`useBossRush.ts` 24–135). Wave-6 flock: elite sheets (#452, docs), Expansion Director (#453, docs), **Wave-6 world dynamics (#454) mutates `worldFeatures.ts` (+1436 / −3)** — TypeScript catalog growth, not docs. CDA-018 already forbids a 53rd `WF-*`; CDA-019 already forbids ENC/Rush *code*. Re-verified 2026-09-26: #454 still open; Wave-7 dump is CDA-021.  
...
## Unpublished 09-24 id (copied so it exists if PR #521 never merges)

---

ACTION_ID: CDA-2026-09-24-021  
SOURCE_AUTOMATION: Content Diversity & Repetition Auditor  
TITLE: Freeze Wave-7 TypeScript catalog dumps until one family binds, one WF-* is imported, and room-0 resurge exists  
CATEGORY: OBJECTIVE_PLAY  
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: HEAD `0f5363f` unchanged vs 09-23. CDA-018 / CDA-020 already freeze `WF-*` TypeScript growth. Same-hour 2026-09-24 **#503** is titled “docs: add wave-7 world dynamics catalog” but mutates `src/frontend/src/engine/worldFeatures.ts` (+1914 / −3) and its test file (+187). Live file has **52** `WF-*` ids; #503’s file has **116** (+64 unread rows). Stack of unread TypeScript already open: #344 Wave-4, #399 Wave-5, #454 Wave-6 (+1436). Same-hour docs siblings: #474 Rush Table E, #506 AI evolution, #479 Ley/Fan/Pit/Font ENC, **#518 Rush Table F**, **#519 Gale/Twin/Pincer ENC**. None of CDA-001 / 008 / 014 shipped. `EnemyFamily` still seven + `default`. `BOSS_RUSH_ROOMS.length` still 10. WX still does not import `worldFeatures.ts`. Re-verified 2026-09-26: #503 still open; #503 files still +1914 on `worldFeatures.ts`.  
SYSTEMS_AFFECTED: `engine/worldFeatures.ts`; `gameTypes.ts` `EnemyFamily`; `useBossRush.ts` `BOSS_RUSH_ROOMS`; spawn / dungeon `generateEnemies`; Wave-7 elite / AI / ENC docs  
RECOMMENDED_ACTION: #474 / #506 / #479 / #452 / #518 / #519 may land as **docs**. #503 / #454 / #399 / #344 must not merge additional `WF-*` ids until CDA-014 imports one existing id (prefer `WF-ELT-TOLL_KEEPER` or `WF-ZON-SHRINE_POOL`). Do not add Wave-7 names to `EnemyFamily` until CDA-001. Do not append Rush Table E/F as live rooms until CDA-008. Do not write ENC-TEACH-06. NEW PLAYER DECISION: none until 001/008/014 — this prevents a 116-row unread shrine catalog, an eighth family sticker, and an 11th dual-boss flavor lie.  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-001; CDA-2026-09-02-008; CDA-2026-09-02-013; CDA-2026-09-02-014; CDA-2026-09-21-018; CDA-2026-09-22-019; CDA-2026-09-23-020  
REGRESSION_RISK: LOW (process). HIGH if ignored (`worldFeatures.ts` more than doubles while spawn stays extra pawns; Wave-7 families become Register-class stickers).  
VALIDATION_REQUIRED: `worldFeatures.ts` stays 52 `WF-*` ids and `EnemyFamily` stays the live seven + `default` until 001/014 ship; `BOSS_RUSH_ROOMS.length` stays 10 until 008 maps `weeping_pawn_2`; #503 does not land TypeScript rows first  
STATUS: NEW  

---

## Unpublished 09-25 id (copied so it exists if PR #557 never merges)

---

ACTION_ID: CDA-2026-09-25-022  
SOURCE_AUTOMATION: Content Diversity & Repetition Auditor  
TITLE: Freeze Wave-7 elite / SDE / formation TypeScript until families bind, kits+AI bind, and one named formation is live  
CATEGORY: TEAM_SYNERGY  
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: HEAD `0f5363f` unchanged vs 09-24. CDA-021 (PR #521, 00:21Z) froze Wave-7 **WF-\*** TypeScript (#503). After that freeze, the same day’s remaining flock authored Wave-7 elite sheets (**#535**, 00:28Z, docs +732 `ENEMY_ELITE_EVOLUTION_2026-09-24.md`), Wave-7 SDE (**#533**, 00:26Z, docs), and formations drop 7 (**#537**, 00:35Z, docs +1320 `ENEMY_FORMATIONS_2026-09-24.md`). All three are docs today. Live spawn still has no `isElite`, kit band NaN (WX 11920), and quadrant scatter (WX 11869–11891). CDA-013 already freezes `EnemyFamily` members; CDA-012 already forbids appending `starterSpells`; CDA-004 is the formation bind. This id names the Wave-7 elite/SDE/formation hour so implementers do not treat #535/#533/#537 as a green light for TypeScript, the way 021 named #503 for `WF-*`. Wave-8 Rush Table F already exists as **#518** (docs, CDA-019).  
SYSTEMS_AFFECTED: `gameTypes.ts` `EnemyFamily`; `spellData.ts` `starterSpells`; spawn / `generateEnemies` formation ids; Wave-7 elite / SDE / formation docs  
RECOMMENDED_ACTION: #535 / #533 / #537 / #525 may land as **docs**. They must not add `EnemyFamily` members, `starterSpells` rows, spawn `formationId`, or `ENEMY_KITS` band-0 DAMAGE ids until CDA-001 (family persist + chassis + verb), CDA-002 **with** 006 (numeric kit band without turning queens into healers), and CDA-004 (one named formation that battle start does not dissolve). Do not author Wave-8 elite / SDE / formation catalogs. Do not write ENC-TEACH-07. NEW PLAYER DECISION: none until 001 / 002+006 / 004 — this prevents an eighth family sticker, a 33rd innate spell, and a named formation that battle-start still scatters into “same Strike pawn, different tint.”  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-001; CDA-2026-09-02-002; CDA-2026-09-02-004; CDA-2026-09-02-006; CDA-2026-09-02-012; CDA-2026-09-02-013; CDA-2026-09-24-021 (WF TypeScript remains 021)  
REGRESSION_RISK: LOW (process). HIGH if ignored (Wave-7 names enter `EnemyFamily` / `starterSpells` while chassis stays random, kits stay band 0, and battle start still re-scatters).  
VALIDATION_REQUIRED: `EnemyFamily` stays the live seven + `default`; `starterSpells` does not grow; spawn still has no `formationId` until 004 ships; #535/#533/#537 do not land TypeScript rows first  
STATUS: NEW

---

## New this run (do not twin 001–022)

---

ACTION_ID: CDA-2026-09-26-023  
SOURCE_AUTOMATION: Content Diversity & Repetition Auditor  
TITLE: Freeze Wave-8/9 TypeScript catalogs until families bind, one WF-* is imported, kits+AI bind, one formation is live, and room-0 resurge exists  
CATEGORY: OBJECTIVE_PLAY  
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: HEAD `0f5363f` unchanged vs 09-21–09-25. CDA-022 (PR #557, 00:09Z) froze Wave-7 elite / SDE / formation TypeScript and said “Do not author Wave-8 elite / SDE / formation catalogs.” After that freeze, the same day’s remaining flock authored Wave-8 elite sheets (**#558**, 00:10Z, docs +743), Wave-8 SDE (**#590**, 00:28Z, docs), formations drop 8 (**#575**, 00:22Z, docs +1247), Wave-8 spell proposals (**#563**, docs +911), Wave-9 boss / Rush **Table G** (**#572**, docs +1159), Face/Mute/Span/Brand ENC (**#574**, docs +1080), and Wave-8 world dynamics (**#578**, 00:24Z) which mutates `worldFeatures.ts` (**+2390 / −3**; **52 → 132** `WF-*` ids) plus its test file (+230). Live spawn still has no `isElite`, kit band NaN (WX 11920), quadrant scatter (WX 11869–11891), unread `combinedMechanic`, room-9 `weeping_pawn_2`, and 52 unused `WF-*` on `main`. CDA-018/020/021 already freeze `WF-*` growth; CDA-019 already freezes ENC/Rush *code*; CDA-013/022 already freeze `EnemyFamily` / `starterSpells` / `formationId`. This id names the Wave-8/9 hour so implementers do not treat #578 as “just docs” (it is TypeScript) or #558/#590/#575/#572/#574/#563 as a green light for live catalogs. #503 (Wave-7, 52→116) is still open. Re-verified 2026-09-26.  
SYSTEMS_AFFECTED: `engine/worldFeatures.ts`; `gameTypes.ts` `EnemyFamily`; `spellData.ts` `starterSpells`; `useBossRush.ts` `BOSS_RUSH_ROOMS`; spawn / `generateEnemies` formation ids; Wave-8/9 elite / SDE / formation / ENC / Rush docs  
RECOMMENDED_ACTION: #558 / #590 / #575 / #563 / #572 / #574 may land as **docs**. #578 / #503 / #454 / #399 / #344 must not merge additional `WF-*` ids until CDA-014 imports one existing id (prefer `WF-ELT-TOLL_KEEPER` or `WF-ZON-SHRINE_POOL`). Do not add Wave-8 names to `EnemyFamily` until CDA-001. Do not append `starterSpells` or `ENEMY_KITS` band-0 DAMAGE ids until CDA-002 **with** 006. Do not add spawn `formationId` until CDA-004. Do not append Rush Table G (or E/F) as live rooms until CDA-008. Do not write ENC-TEACH-08. Do not author Wave-9 elite / SDE / formation catalogs. NEW PLAYER DECISION: none until 001 / 002+006 / 004 / 008 / 014 — this prevents a 132-row unread shrine catalog, a ninth family sticker, a 33rd innate spell, a named formation battle-start still scatters, and an 11th dual-boss flavor lie.  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-001; CDA-2026-09-02-002; CDA-2026-09-02-004; CDA-2026-09-02-006; CDA-2026-09-02-008; CDA-2026-09-02-012; CDA-2026-09-02-013; CDA-2026-09-02-014; CDA-2026-09-21-018; CDA-2026-09-22-019; CDA-2026-09-23-020; CDA-2026-09-24-021; CDA-2026-09-25-022  
REGRESSION_RISK: LOW (process). HIGH if ignored (`worldFeatures.ts` jumps to 132 unread rows while spawn stays extra pawns; Wave-8 names enter `EnemyFamily` / `starterSpells` / `BOSS_RUSH_ROOMS` while chassis stays random, kits stay band 0, and room 9 still ghosts).  
VALIDATION_REQUIRED: `worldFeatures.ts` stays 52 `WF-*` ids and `EnemyFamily` stays the live seven + `default` until 001/014 ship; `starterSpells` does not grow; spawn still has no `formationId` until 004; `BOSS_RUSH_ROOMS.length` stays 10 until 008 maps `weeping_pawn_2`; #578 does not land TypeScript rows first  
STATUS: NEW
