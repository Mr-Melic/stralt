# Stralt expansion catalog — 2026-09-25

**Author:** Expansion Director (cron `0 */24 * * *`, automation `3f31b18f-a492-11f1-a7d1-d6b4613131ce`)  
**This run:** eighth catalog. Re-rank against `origin/main` `0f5363f` (`Merge pull request #332` — accepted-challenge HUD). Author-date on that tip is **2026-09-03**. Calendar: 2026-09-25.  
**Prior living catalog on `main`:** [`EXPANSION_PROPOSALS_2026-09-02.md`](./EXPANSION_PROPOSALS_2026-09-02.md) (`HEAD` then `58302bc`).  
**Unmerged sibling catalogs:** PR **#366** (09-21; 019/020), **#397** (09-22; 021/022), **#453** (09-23; 023/024), **#510** (09-24; 025/026) — same `HEAD`, still drafts. This file **adopts** 019–026 rather than reminting them.  
...
All seventeen 08-31 / 09-01 / 09-02 cards remain **PROPOSED**. 019–026 remain **PROPOSED**. None shipped. None superseded. New IDs in this file are additive first-ship slices of already-ranked cards.

`README.md` still indexes the 09-01 catalog. Older open PRs own that file (**#369**, **#402**, **#420**, **#422**, **#427**, **#448**, **#468**, **#477**, **#509**, **#513**, **#527**). This run does not touch `README.md` / `AGENTS.md` / `ARCHITECTURE.md` / `WorldExploration.tsx` / `mapGen.ts` / `challengeCompletion.ts` / the 09-02 catalog (#366) / the 09-22 catalog (#397) / the 09-23 catalog (#453) / the 09-24 catalog (#510).
...
`main` did **not** move. #366 / #397 / #453 / #510 ranked the same `0f5363f`. This run is a re-rank against a still-growing open-PR queue (gh lists a 200-PR window; same-hour flock already through **#563**), plus two first-ship slices of 016 and 005 that do not need a new family / spell / Wave-N sheet.

| Fact | Evidence |
| :--- | :--- |
| No expansion implementation | Still 158 commits / 70 merges `58302bc` → `0f5363f`, all 2026-09-02…03. Zero commits 2026-09-03 00:28 UTC → this cron. Cards did not ship off-branch. Re-verified live hooks on this `HEAD`. |
| Open queue grew again | Oldest still-open blockers: **#327** Striker AoE (`WX` / targeting / `challengeCompletion`), **#331** portal destack (`mapGen`). Twin **#370** restacks the same Striker gate. Then 09-21…09-25 flocks (200+ drafts; gh list cap 200; same-hour Wave 8 sheets **#556–#563**). Do not restack those files. |
| #366 / #397 / #453 / #510 still drafts | 09-21…09-24 catalogs are not on `main`. Adopt 019–026. Do not fight #366 on `EXPANSION_PROPOSALS_2026-09-02.md`, #397 on the 09-22 file, #453 on the 09-23 file, or #510 on the 09-24 file. |
...
| `inferArchetype` never returns `summoner` | Inference (`enemyAI.ts` 447–477) never emits it. Live summoner path is the `isSummoner` flag (`WX` 16375–16376 `decideSummonerAction`). 023 must keep that flag. |
...
| Telemetry still none | TBC on `main` is 09-02 `WAITING_FOR_TELEMETRY`. Same-hour **#556** still waiting. 0 collectors / 0 rows. Do not claim live demand. Do not retune XP, Doka, or spawn weights. |
| Engineering-health on `main` | Last landed reports: `MASTER_ROADMAP.md` / LONG_HORIZON / CDA / PXA dated **2026-09-02**. 09-21…09-25 MTD / CDA / PX / LHIPS / Wave-N sheets are still drafts. Code claims that match this `HEAD` are accepted. |
...
| Family still a sticker | `maybeApplyEnemyFamilyVariant` (`spawnPolicy.ts` 279–286) at `WX` 5864. Chassis stays a random chess piece (`WX` 5764–5765). Four mute families. Family HP dies at `calcEnemyMaxHp`. No elite token layer. |
...
| `worldFeatures` still tests-only | 1913 lines, Wave-1 Ember Vein already authored (`WF-HAZ-EMBER_VEIN`). No WX import. Freeze new rows until **one** live tile exists (027). |
| `combinedMechanic` still copy | `useBossRush.ts` 19–135. Zero engine readers. |
| Nine challenges still numeric | `challengeCompletion.ts` 11–20. No `kill_leader_last`. Open **#327** owns the file. |
| Unused AI gates still unused | `ENEMY_AI_TIER_GATES` (`gameConstants.ts` 200–208) has **zero** readers in `enemyAI.ts` except a comment at 1422. Master AI toggles stay globally on. 028 is the first gate. |
...
| Wave 2–8 elite / family sheets (09-01 / 09-02 + open **#349** / **#405** / **#452** / **#558**) | More family ids | **Hold** until 001/014/018/019/021 exist |
| [`SPELL_DISCOVERY_ECOSYSTEM_2026-08-31.md`](./SPELL_DISCOVERY_ECOSYSTEM_2026-08-31.md) | Observe → win → unlock; Wave 1 pool | 003 |
| SDE 09-01 / 09-02 + open **#371** / **#480** | Later discovery ids | **Hold** until 003 is live |
...
| SPELL_PROPOSALS 08-31…09-02 + open **#342** / **#411** / **#463** | Tactical gap-fill ids | **Hold** until 003 ships |
| [`ENEMY_AI_EVOLUTION.md`](../ENEMY_AI_EVOLUTION.md) + 09-01 / 09-02 + open **#351** / **#416** / **#457** | Unbounded AI modules | 005, **028** |
| [`BOSS_AND_SPELL_DISCOVERY.md`](../design/BOSS_AND_SPELL_DISCOVERY.md) + open **#367** / **#406** / **#474** | Boss sheets / Rush Tables C–E | 011, 017 |
| ENCOUNTER_EVOLUTION 08-31…09-02 + open **#347** / **#396** / **#479** | Teach → pressure dungeon beats | 007, 010, 026 |
| [`WORLD_DYNAMICS.md`](../WORLD_DYNAMICS.md) + `engine/worldFeatures.ts` + open **#344** / **#399** / **#454** / later WDD drafts | Rarity / relative-difficulty overlay | 009, 016, 022, 024, **027** |
| ENEMY_FORMATIONS 08-31…09-02 + open **#348** / **#401** / **#459** | Synergy packs | 004, 023 |
| [`MECHANIC_INTERACTION_MATRIX_2026-09-02.md`](./MECHANIC_INTERACTION_MATRIX_2026-09-02.md) + open **#336** / **#410** / **#476** | Live join gaps | 013, 018, 024 |
...
| [`LONG_HORIZON_2026-09-02.md`](./LONG_HORIZON_2026-09-02.md) + open **#357** / **#407** / **#475** / **#530** / **#560** | XP wall; kit NaN; summoner saturate | PREREQ-A/B/C; do not retune curve |
| [`CONTENT_DIVERSITY_AUDIT_2026-09-02.md`](./CONTENT_DIVERSITY_AUDIT_2026-09-02.md) + open **#359** / **#403** / **#465** / **#521** / **#557** | Live spawn is sticker + NaN kits; Blood Moon live; no elites | Confirms 001/014/019/021/022/023/025 |
| [`TELEMETRY_BALANCE_2026-09-02.md`](./TELEMETRY_BALANCE_2026-09-02.md) + open **#333** / **#395** / **#462** / **#556** | STATUS WAITING_FOR_TELEMETRY | No demand claims |
| Wave 2–8 elite / spell / encounter / boss sheets (incl. same-hour **#518** / **#525** / **#558** / **#563**) | More ids | **Hold** until 021/014/019+F/018/003 exist |
...
- Spawn writes a random chess piece (`WX` 5764–5765), then `applyFamilyVariantsToRoster` (`spawnPolicy.ts` 289+, `WX` 5864) rolls 30% and multiplies paper HP / damage / RES / SP. Catalog `ap` / `mp` still unused. **Family does not force `pieceType`.** A “Wraith Bishop” may be a pawn with Strike.
- Battle start then **re-rolls** `sp` / `sr` / `init` / `res` / `chc` from `computeEnemyStats(e.level, e.pieceType, e.id)` (`WX` 11872, 11898–11902) and sets combat HP from `calcEnemyMaxHp(level)` (`WX` 11970–11974). **Family HP, RES, and SP never reach the fight.** Family overlay RES 0.05–0.75 is also the wrong unit vs `getEnemyBaseStats` integers. No `isElite` / champion token.
...
- `inferArchetype` (`enemyAI.ts` 447–477) returns `"healer"` if any assigned spell has `healAmount > 0`. Drain kits become healers. Band-1 queens would become Blood-Mend healers if 019 ships alone. The union includes `summoner`; inference **never returns it**. Live summoner path is `enemy.isSummoner` → `decideSummonerAction` (`WX` 16375–16376).
- Leader death → erratic and 5% betrayal still exist. Crown chrome is already live (`WX` 8107–8131, `InitiativeStrip.tsx`). `ENEMY_AI_TIER_GATES` (`gameConstants.ts` 200–208) names `groupTactics`, `instantKill`, `chokepointCamp`, `escapeRoute`, `bottleneckControl`, `defensiveRetreat`. **Zero references in `enemyAI.ts`** except a comment at 1422. Master AI toggles are **global on**, not `aiTier`-gated.
- Summoner chance is `0.12 + playerLevel * 0.02` (`WX` 11932–11942; constants `gameConstants.ts` 298–299). Saturates by mid-40s **and** can fire per body. `ENEMY_SUMMON_CAP = 2` contains the board, not the pack. Pets are 50/50 wolf or archer — not family-bound. 023 is the first composition slice of 004.
...
- `shouldIncludeBackendSpellInLibrary` (`adminSafety.ts`) returns true whenever `usableByPlayer !== false`. The `ownedSpellIds` argument is a **retirement** gate. It is **not** discovery.
...
- MTD 09-21/09-22: confirm Caffeine deploy of the `20260901` tail and refresh `.old` before new Motoko stables. Source chain is correct; live deploy is unconfirmed.
...
| PREREQ-C | Summoner chance uses raw `characterStats.level` (`WX` 11932–11942) | Saturates by the mid 40s **and** can fire on every body. 023 replaces the coin flip with a pack slot; C is then “use player **tier** for the slot chance,” not a second linear formula. Keep `ENEMY_SUMMON_CAP`. | OPEN — **couple with 023** |
...
| PREREQ-H | Battle start overwrites family HP via `calcEnemyMaxHp` (`WX` 11970–11974) **and** overwrites RES/SP/CHC via `computeEnemyStats` (`WX` 11872, 11898–11902). `applyEnemyFamilyStats` (`spawnPolicy.ts` 261–272) still multiplies paper HP first. | The 30% family roll is a lie even as stat flavor. Stamp **identity** in `spawnPolicy`; do not multiply paper HP/damage and do not try to keep overlay RES (wrong unit). Natural union with 021 (that PR already touches `applyEnemyFamilyStats`). | OPEN — **couple with 021** |
...
PREREQ-G (dual frontend / Motoko spell catalogs) stays a hygiene note, not a ship-blocker for 001/014/006/018/019/021/023/025. Align ids when a spell PR actually touches `defaultSpells()`.

---

## Ranked opportunities

| Rank | EXPANSION_ID | Title | Category | SCORE | Priority | vs 09-24 |
| ---: | :--- | :--- | :--- | ---: | :--- | :--- |
...
| 3 | EXP-2026-09-21-020 | First mechanic challenge: `kill_leader_last` | PROGRESSION | 29 | P0 | same |
...
| 9 | EXP-2026-09-21-019 | Relative kit-band adapter (unlock authored piece kits) | ENEMIES | 25 | P0 | same; **must ship with F** |
| 10 | EXP-2026-09-22-021 | Family-preferred chassis overlay | ENEMIES | 25 | P0 | same; **ship with PREREQ-H** |
| 11 | EXP-2026-09-23-023 | One summoner slot per pack | ENEMIES / AI | 25 | P1 | same |
| 12 | EXP-2026-09-02-018 | Wire push / pull on existing ids | COMBAT | 24 | P1 | same |
| 13 | EXP-2026-08-31-010 | Optional tactical objectives | COMBAT | 24 | P1 | same |
| 14 | EXP-2026-09-25-027 | First world feature: Ember Vein tile | WORLD | 24 | P2 | **new** — first ship slice of 016 |
| 15 | EXP-2026-09-24-026 | First side objective: `contest_shrine` | COMBAT | 23 | P1 | adopted |
| 16 | EXP-2026-08-31-008 | Uncapped achievement / mastery ladder | PROGRESSION | 23 | P2 | same |
| 17 | EXP-2026-08-31-013 | Explicit spell-interaction layer | COMBAT | 23 | P2 | 018 first |
| 18 | EXP-2026-09-01-016 | Wire `worldFeatures` overlay (no mapGen rewrite) | WORLD | 23 | P2 | same; 027 first; freeze new `WF-*` |
| 19 | EXP-2026-09-23-024 | Wire Gravity Well via existing attract | WORLD | 22 | P2 | same |
| 20 | EXP-2026-09-25-028 | First AI gate: `chokepointCamp` | AI | 22 | P2 | **new** — first ship slice of 005 |
| 21 | EXP-2026-08-31-009 | Finish unused modifiers + combo-only world rules | WORLD | 21 | P2 | same; 024 can land first |
| 22 | EXP-2026-08-31-005 | AI sophistication ladder (unused gates) | AI | 21 | P2 | same; 028 first |
| 23 | EXP-2026-08-31-007 | Dungeon room types without rewriting mapGen | DUNGEONS | 21 | P2 | wait for #331 |
| 24 | EXP-2026-09-02-017 | Honor Boss Rush `combinedMechanic` | BOSSES | 20 | P2 | same |
| 25 | EXP-2026-08-31-011 | Boss mechanic-pool scaling (not more HP) | BOSSES | 17 | P3 | same |
| 26 | EXP-2026-09-22-022 | Convert Blood Moon off ×1.25 damage | WORLD | 16 | P3 | same |
| 27 | EXP-2026-09-01-015 | Convert Titan's Vigor off +1000 HP | WORLD | 15 | P3 | same |

**Ship-next (not the same as SCORE):** **021 + PREREQ-H** → **014** → **PREREQ-A + 019 + PREREQ-F (one PR)** → **001** → **025** (after 021, `spawnPolicy` + new `eliteModifiers.ts`) → **023** (after #327) → **020** (after #327) → **018** → **024** (after walk freeze / 018) → **027** (Ember Vein on existing floor; no `mapGen` punch) → **026** (after #327; no `mapGen` punch) → **028** (tiny `aiTierGates` helper; do not grow `enemyAI.ts` with Wave-N tables) → **003** (PREREQ-J) → 002/004 remainder.

SCORE ranks opportunity size. Ship-next ranks what creates a player decision **this week** without Motoko schema, without growing `enemyAI.ts` for content tables, and without racing open WX / mapGen / challenge / walk-occupancy PRs.

021 before 019 because band-0 kits already differ by piece type: coupling family → preferred chassis works **today**, lives in `spawnPolicy.ts`, and does not wait on #327. Bundle PREREQ-H in that same PR — `applyEnemyFamilyStats` is the paper-HP liar **and** battle start already discards overlay RES/SP. 019 before full 001 because the piece-kit table already exists and currently cannot fire — but 019 **without F** turns late queens into healers, so A+F+019 is one change. 014 still creates family identity independent of kits. 025 is the cheapest elite decision (one shielded body, not +HP) and can stamp in `spawnPolicy` after 021 makes chassis readable. 023 is the cheapest composition decision but its WX call site waits on #327. 027 is the cheapest world-path decision (one authored Ember Vein, catalog already tested) and must not punch corridors or append a 53rd `WF-*`. 028 is the cheapest AI-sophistication decision (camp a choke at `aiTier >= 3`) without rewriting `decideEnemyAction`. 026 is the cheapest fight-side-goal and must not punch corridors.
...
**MECHANIC:** Add `kill_leader_last` to `ChallengeCondition`. Fail if the annotated leader (`isLeader` / `leaderEnemyIdRef`, `WX` 12010–12029) dies while any other battle-start hostile still has store HP `> 0`. Win if that leader is the last hostile into `deathPipeline`. Rewards stay on the existing recap → `applyRewards` path.  
...
**IMPLEMENTATION_APPROACH:** Copy the hook-union pattern from `engine/mapModifiers.ts` (32–40). Do not apply tokens by multiplying `level * 8 + 20` again. Leader crown stays a separate identity; do not conflate `isLeader` with the first elite token until 004 recipes exist. First shippable slice is 025 (`shielded` only).  
...
**TITLE:** First elite token: `shielded`  
**CATEGORY:** ENEMIES  
**PLAYER_OPPORTUNITY:** Spend a burst (or Mark → Inferno) to break one protected body before its kit matters — instead of treating the pack as equal HP bars.  
**MECHANIC:** Stamp at most **one** non-boss combatant per pack with `eliteToken: "shielded"`. Chance scales with player **tier**, not raw HP. The token grants one existing identity hook: the first lethal hit (store HP would go to 0) leaves the combatant at 1 HP **once**, then the shield is spent. Reuse the same “survive at 1” idea 015 proposes for Titan's Vigor, but on **one** body, not the whole map. Nameplate badge (stone + crimson, `DESIGN.md`). Do **not** add `hexer` / `coordinator` / `echo_cast` / `phase_step` in this PR. Do not multiply `level * 8 + 20`.  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable slice of 002. CDA reconfirmed there is no elite layer — leader boost + 5% betrayal are the only specials. One cheat-death on a readable chassis (after 021) is a kill-order question at any player level, without new art, Motoko, `enemyAI.ts` growth, or a Wave-N family.  
**SYSTEMS_AFFECTED:** New `engine/eliteModifiers.ts` + tests; `spawnPolicy.ts` `assignPackElite(roster, playerTier, rng)`; combat inspect / initiative chip. One death-pipeline call (`wouldDie && !shieldSpent → hp = 1`). **Do not grow WX while #327 is open** — helper tests can land first; WX one-line after. Leader crown stays separate (`isLeader` ≠ shielded).  
**INFINITE_PROGRESSION_BEHAVIOUR:** Higher tiers later add a second token id (002 remainder), not a thicker shield. `shielded` stays valid at band 0 and band 20 because it is identity, not +N% HP.  
**IMPLEMENTATION_APPROACH:** After 021 so the shielded body is a preferred chassis, not a random pawn sticker. Prefer a non-leader filler so 020 still has a crown that is not automatically the cheat-death. Lava / spikes / reflect still record challenge damage; the second lethal must kill. Do not compose with Titan's Vigor +1000 in this PR.  
**BALANCE_CONSIDERATIONS:** Band-0 packs should often have **zero** elites. Do not put the token on a boss or on every body. Do not also grant iron-skin / poison immunity (014 owns family hooks). Shield does not write Doka.  
**QA_REQUIREMENTS:** Second lethal still enters `deathPipeline.ts`; leftover-roster XP unchanged; enemy-summon deaths do not inherit the token; `countsTowardKillRewards` unchanged; no Doka write; mixed-family packs legal; Register stays lore (optional combat inspect chip is the live tell).  
**PRIORITY:** P1  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “The shielded rook shrugs the first Inferno — do I chip it with Strike, or ignore it and race the scribe?”
...
**WHY_IT_IMPROVES_STRALT:** Quadrant spawn still places bodies, not roles. Summoner chance already exists and is over-firing (PREREQ-C). Formations sheet: [`ENEMY_FORMATIONS_2026-09-01.md`](../design/ENEMY_FORMATIONS_2026-09-01.md). First shippable slice is 023.  
...
**IMPLEMENTATION_APPROACH:** Pure `buildPack(playerLevel, tierCfg, rng)` returning `{ pieceType, family, role, aiTier }[]`. WX places the returned roster on legal tiles. Land 023 first so the summoner slot exists before full recipes.  
**BALANCE_CONSIDERATIONS:** After PREREQ-C / 023, summoner is a role slot, not a per-enemy coin flip. Do not spawn two healers + two summoners in band 0.  
...
**WHY_IT_IMPROVES_STRALT:** This is the cheapest spell-pool-depth signal in the repo and the brief’s preferred alternative to HP. The table has survived five director cycles while the call site passes an object. Sibling LONG_HORIZON / CDA / PXA all name this NaN. 019 is the **policy** (relative R, unbounded). PREREQ-A is the **type bug** (stop passing an object). PREREQ-F is the **healer trap**. Ship A+F+019 together. Adopted from #366.  
...
**IMPLEMENTATION_APPROACH:** `FAMILY_PREFERRED_PIECE: Record<Family, ChessPieceType[]>` plus `rng` pick. Call from `applyEnemyFamilyStats` or immediately after. Unit-test: 1000 rolls of `wraith_bishop` majority bishop; default family never forced; Chebyshev spacing unchanged; family HP after the helper equals pre-helper HP.  
**BALANCE_CONSIDERATIONS:** More frost bishops at band 0 is intended. Do not re-introduce family HP mults. Do not force 100% chassis lock — that deletes the piece axis.  
...
**TITLE:** One summoner slot per pack  
**CATEGORY:** ENEMIES / AI  
**PLAYER_OPPORTUNITY:** Find the one body that will fill the board, instead of treating every mid-level pack as a summoner lottery.  
**MECHANIC:** Replace the per-enemy coin flip (`WX` 11932–11942: `0.12 + playerLevel * 0.02`, 50/50 wolf/archer) with a pack-level helper: at most **one** non-summon, non-boss combatant may gain `isSummoner`. Chance scales with player **tier**, not raw level (PREREQ-C). Prefer the highest-level filler that is not already the leader, so 020 still has a crown that is not automatically the pet battery. Keep `ENEMY_SUMMON_CAP = 2`. Do not family-bind pets here (that is 001).  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable slice of 004. Independent rolls currently collapse composition: by the mid-40s almost every body can summon, and even at low level a lucky map can field several. The player decision “break the battery” does not exist until summoner is a **slot**. No new creature art. No Motoko. No `enemyAI.ts` growth.  
**SYSTEMS_AFFECTED:** New `engine/packRoles.ts` (`assignPackSummoner(roster, playerTier, rng)`) + tests; one call site replacing the `for` loop at `WX` 11937 after #327. Constants `ENEMY_SUMMONER_CHANCE_*` become unused — delete or keep as the **slot** probability.  
**INFINITE_PROGRESSION_BEHAVIOUR:** Higher tiers raise the chance that the slot is filled, then (later 004) add a synergy tag. Never “every body summons.” After tier saturation, add 002 coordinator / 001 family pets — not a second coin flip.  
**IMPLEMENTATION_APPROACH:** Pure helper first; tests can land without WX. Do not grow WX while #327 is open. Do not also rewrite wolf/archer 50/50 in this PR. Keep the live `isSummoner` → `decideSummonerAction` path (`WX` 16375–16376). Do **not** wait for `inferArchetype` to emit `"summoner"` — it never does (`enemyAI.ts` 447–477).  
**BALANCE_CONSIDERATIONS:** Band-0 packs should often have **zero** summoners. Do not put the slot on a boss. Do not raise `ENEMY_SUMMON_CAP`.  
**QA_REQUIREMENTS:** Packs of 1 never spawn a summoner-only unwinable board (player can still last-hostile); enemy summons still `side: "enemy"`; player-side kills still skip `applyRewards`; lifespan still decrements on the summon’s turn; no Doka write.  
**PRIORITY:** P1  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “Only one of them summons — do I spend this Inferno on the battery, or on the crowned golem?”
...
**WHY_IT_IMPROVES_STRALT:** Positioning is the combinatorial axis the brief asks for. The resolvers and tests exist; zero production callers. This is the first shippable slice of 013. It multiplies family (tide_shade later), modifiers (thorns / rift / 024 gravity), Barrier, and Mark without a new spell id and without Motoko.  
**SYSTEMS_AFFECTED:** `spellEngine.ts` / enemy-cast dispatch; occupancy (already); Swap still separate (MIMA-001 — do not fix Swap in this PR). **No damage-formula rewrite.** Open combat PRs (#379 / #417 / #432 / #441 / #467 / #489 / #496) currently race walk occupancy / cast execute — wait or union landing, do not open a fifth walk path.  
...
**IMPLEMENTATION_APPROACH:** Place the objective on an already-walkable floor cell after finalize (do **not** punch new corridors; `mapGen.ts` stays closed, especially while #331 is open). First shippable slice is 026 (`contest_shrine` only).  
...
**TITLE:** First side objective: `contest_shrine`  
**CATEGORY:** COMBAT  
**PLAYER_OPPORTUNITY:** Spend MP this fight to stand on a marked floor cell at victory, or ignore it and still clear the pack.  
**MECHANIC:** After `finalizePlayableLayout` (do **not** punch), pick one already-walkable floor cell that is not spawn ±3, not a portal, and not a dump cell. Stamp `objective: { type: "contest_shrine", cell }`. At last-hostile victory, if the player (not a summon) occupies that cell, add a small extra Doka/XP grant on the **existing** recap → `applyRewards` funnel with `PREAPPLIED_REWARD_MULTIPLIER`. Failure is optional — the fight can still be won. Do not add interrupt-channel / extract-tile types in this PR. Do not block the progression portal.  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable slice of 010. Overworld fights are still “clear hostiles.” A single marked cell makes positioning and tempo a decision independent of DPS, without `mapGen` rewrites, Motoko, or a new challenge predicate (020 stays `kill_leader_last` in `challengeCompletion.ts`).  
**SYSTEMS_AFFECTED:** New `engine/tacticalObjectives.ts` + tests; recap field; one WX wire after #327. **`mapGen.ts` stays closed** (#331 destack plus later destack/dump PRs). Do not double-count 006/020. Credits stay on `applyRewards`; one-shot id if the bonus can remint (`dokaPersist.ts`).  
**INFINITE_PROGRESSION_BEHAVIOUR:** The cell is relative to the live map, not a level cap. Later 010 types rotate with tier on the same helper. Reward stays inside current victory clamps — never a level-capped table.  
**IMPLEMENTATION_APPROACH:** Placement helper first (legal floor, solvability unchanged). HUD: stone shrine chip, not a new tile type. Death Realm / rest maps do not roll an objective. Do not place on lava/spikes (contest must be walkable without a forced Untouchable fail).  
**BALANCE_CONSIDERATIONS:** Optional. Bonus stays well under `applyRewards` caps (think portal-scale, not jackpot). Never require the shrine to finish a dungeon chain. Do not pay if the player died.  
**QA_REQUIREMENTS:** `evaluateSolvability` still passes; dungeon-chain snapshot still happens before `cleanupMap`; Death Realm timer still blocks portals/encounters; challenge HUD still stays after the first action; no Doka write on fail; player-summon standing on the cell does **not** count.  
**PRIORITY:** P1  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “Do I walk onto the shrine this turn and skip the Inferno, or clear first and miss the bonus?”
...
**WHY_IT_IMPROVES_STRALT:** Catalog is wide but shallow. MIMA also shows `applyPushback` / `applyAttract` unused — 018 is the first interaction verb. Hold SPELL_PROPOSALS Waves 1–5 until 003 exists. Do not add a fifth reflect (CDA-017).  
...
**WHY_IT_IMPROVES_STRALT:** The catalog and tests exist and do nothing. Wiring **one** existing slot is cheaper than inventing a thirteenth modifier or a 53rd `WF-*` row. Relative difficulty is vs **same-tier** content — compatible with no level cap.  
**SYSTEMS_AFFECTED:** `worldFeatures.ts` (already); new placement helper; WX one-line after finalize. **`mapGen.ts` stays closed** (1,937 lines; #331 destack). Do not append Wave 4/5 world-dynamics rows from #344 / #399 until one live feature exists.  
**INFINITE_PROGRESSION_BEHAVIOUR:** Rarity weights + relative bands, never “unlocks at level N”. Append-only feature table.  
**IMPLEMENTATION_APPROACH:** One slot type first (`tile`) — that is 027 (`WF-HAZ-EMBER_VEIN` only). Then `encounter`, then `event`. HP taxes are **fractions of current max HP** (already in the module).  
...
**TITLE:** First world feature: Ember Vein tile  
**CATEGORY:** WORLD  
**PLAYER_OPPORTUNITY:** Spend extra MP to walk around a glowing seam, or cut through and pay a fraction of current max HP — a pathing question the 22 modifiers do not ask.  
**MECHANIC:** After `finalizePlayableLayout` (do **not** punch corridors), roll at most **one** Wave-1 tile: `WF-HAZ-EMBER_VEIN` (`worldFeatures.ts` 164–193, `slot: "tile"`, `hpTaxPctOfMax: 0.04`). Place on an already-walkable floor cell that is not spawn ±3, not a portal, and not a dump cell (`isTileReservedForSpawnOrPortal`). Stepping on it taxes `hpTaxFromMax` (4% of **current max HP**, already in the module) through the existing walk-hazard commit (`recordInBattleChallengeDamage` / `updateCombatant`). Death Realm / rest maps do not roll. Do **not** honor `extraHazardCount` (catalog 3–6) in this PR — one cell is the slice. Do **not** roll encounter or event slots. Do not append a new `WF-*` id.  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable slice of 016. The catalog is 1,913 lines with tests and **zero** production callers. Wiring one common tile is cheaper than a 53rd feature row or a thirteenth modifier. Relative difficulty is vs same-tier content — no level cap. Same-hour Wave-N world sheets stay Hold.  
**SYSTEMS_AFFECTED:** New `engine/worldFeaturePlacement.ts` + tests (or a helper next to `worldFeatures.ts`); walk-hazard landing (reuse, do not fork); one WX import after #327. **`mapGen.ts` stays closed** (#331 destack). Credits stay on `applyRewards` only if a later event slot pays — Ember Vein is a tax, not a mint.  
**INFINITE_PROGRESSION_BEHAVIOUR:** Tax is a fraction of current max HP, so it stays meaningful at band 0 and band 20. Later 016 slots rotate encounter/event on the same placement helper. Never “unlocks at level N.”  
**IMPLEMENTATION_APPROACH:** Placement helper first; unit-test reserved cells, Death Realm skip, solvability unchanged. HUD: existing world-feature visual (`glyph` / `tileTint`), stone chip, not a new tile type in `mapGen`. Do not stack Titan's Vigor + Blood Moon ×1.25 + Ember Vein on band 0 as a required combo.  
**BALANCE_CONSIDERATIONS:** Vein must never be the only walkable cell in a corridor (`blocksWalk: false`). Do not replace lava. Challenge Untouchable / `no_hazard_steps` must see the tax. Do not pay Doka for crossing.  
**QA_REQUIREMENTS:** `evaluateSolvability` still passes; mouse **and** touch walks tax the same; dungeon-chain snapshot still happens before `cleanupMap`; Death Realm quiet; no Doka write; player-summon steps tax the summon via `updateCombatant`, not a second player-HP path.  
**PRIORITY:** P2  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “The ember seam is 4% max HP — do I spend two MP to walk around, or cut through and risk Untouchable?”

---

### EXP-2026-09-23-024

**TITLE:** Wire Gravity Well via existing attract  
**CATEGORY:** WORLD  
**PLAYER_OPPORTUNITY:** Spend extra MP (or a push/pull) to stay on the rim, or accept the slide toward center thorns / lava / the pack.  
**MECHANIC:** Honor `gravity_well` (`WX` 2324 `_isGravityWell` unused; `mapModifiers.ts` 280–286 empty hooks) by calling `applyAttract` (`occupancy.ts` 537) **one step toward map center** after a successful walk, using the same occupancy / wall / portal stop rules as a spell pull. Sync announce copy. Do **not** implement Fog in this PR. Do not re-implement Blood Moon / Mirror Field (they are live).  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable unused-modifier slice of 009. Gravity is a positioning verb — the brief’s preferred axis — and the resolver already exists. Shipping it trains the player to read the chip again. Blood Moon conversion (022) can wait; Gravity is actually a no-op today.  
**SYSTEMS_AFFECTED:** Walk landing helper (not a new damage path); `mapModifiers.ts` announce; tests in occupancy. **Wait for walk-occupancy freeze** (open #379 / #417 / #432 / #441) or union onto the shared landing helper. 018 can land first so spell pull and gravity share one attract call.  
**INFINITE_PROGRESSION_BEHAVIOUR:** Pull distance stays 1. High tier can stack Gravity with Thorned / Void Rift (already combinatorial) instead of scaling the pull with player level.  
**IMPLEMENTATION_APPROACH:** One hook on committed walk, not on preview. Enemy and summon walks must pay the same slide (Frozen/Slime already charge walk MP — do not skip those). Barrier blocks the step.  
**BALANCE_CONSIDERATIONS:** Do not slide into instant-kill without a telegraph (center-of-map lava is rare; still record challenge damage). Do not also halve MP. Fog stays a later 009 slice.  
**QA_REQUIREMENTS:** Mouse and touch walks both slide; Death Realm stays quiet; challenge `no_hazard_steps` still sees the landed tile; no Doka write; portals / occupancy still block.  
**PRIORITY:** P2  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “Gravity will slide me onto the thorn lane — do I walk one extra tile around, or spend Inferno from here?”
...
**MECHANIC:** Implement the two **still-unused** flags: `gravity_well` (024) and `fog_of_war` (UI vision radius). Differentiate `slime_flood` vs `frozen_terrain` (155–173 currently both `onMpCost * 2`). Do **not** re-implement `mirror_field` (already 20% ST reflect in `spellEngine.ts` 901–907) or `blood_moon` as another damage multiplier (already ×1.25 at 895 — convert via 022). Sync `mapModifiers.ts` announce text with live engine so the chip stops lying.  
...
**SYSTEMS_AFFECTED:** `mapModifiers.ts` registry + announce strings; `MapModifiersPanel.tsx`; challenge `modifier_survivor`. Gravity Well should call the same attract helper as 018 / 024.  
...
**IMPLEMENTATION_APPROACH:** One unused id per change. 024 first. 022 handles Blood Moon. 015 handles Titan's Vigor. Do not add a fifth reflect.  
...
**MECHANIC:** Implement unused `ENEMY_AI_TIER_GATES` **inside** `decideEnemyAction` (`engine/enemyAI.ts`), not as more WX branches. Lethal lookahead / LoS reposition / backline protect are **global on** (`gameConstants.ts` 224–258) — gate them by `enemy.aiTier` so low-level fights stay readable. Follow [`ENEMY_AI_EVOLUTION.md`](../ENEMY_AI_EVOLUTION.md); do not rewrite the 2,580-line module in one PR. Do not grow it for Wave 4/5 AI sheets (#351 / #416).  
...
**INFINITE_PROGRESSION_BEHAVIOUR:** `computeAITier` already maps unbounded level → 1..10. After 900, keep tier 10 and add **pack** coordination (004 / 023) instead of a tier 11 damage stat. Do not “fix” the 30% random roll in the first AI PR (master-roadmap hold).  
**IMPLEMENTATION_APPROACH:** One gate per PR, with a deterministic `rng` fixture. First gate is 028 (`chokepointCamp`). Do not edit turn-advance in WX.  
...
**TITLE:** First AI gate: `chokepointCamp`  
**CATEGORY:** AI  
**PLAYER_OPPORTUNITY:** Read a high-`aiTier` pack camping a corridor and spend AP/MP to break the camp — while low-tier packs still walk in and telegraph.  
**MECHANIC:** Honor `ENEMY_AI_TIER_GATES.chokepointCamp` (`gameConstants.ts` 205, threshold **3**) inside `decideEnemyAction`. When `enemy.aiTier >= 3` and a 1-tile-wide corridor exists between the enemy and the player (occupancy already knows walls / portals), prefer an idle or hold-tile action on that choke over a random wander. When `aiTier < 3`, keep today’s approach. Extract the predicate to `engine/aiTierGates.ts` (`shouldCampChokepoint(aiTier, ctx)`) so `enemyAI.ts` (2,580 lines) does not grow a content table. Do **not** implement `instantKill` / `groupTactics` / `escapeRoute` in this PR. Do not “fix” the 30% random `computeAITier` roll.  
**WHY_IT_IMPROVES_STRALT:** This is the first shippable slice of 005. The brief prefers AI sophistication over HP. The knobs exist (`ENEMY_AI_TIER_GATES`) and have **zero** production readers except a comment at `enemyAI.ts` 1422. Gating one existing behaviour by `aiTier` creates a progression read without new art, Motoko, or a Wave-N AI sheet.  
**SYSTEMS_AFFECTED:** New `engine/aiTierGates.ts` + tests; one call from `decideEnemyAction`. Do not grow WX. Do not rewrite lethal lookahead / LoS reposition (those are globally on — later 005 remainder gates them off at low tier).  
**INFINITE_PROGRESSION_BEHAVIOUR:** `computeAITier` already maps unbounded level → 1..10. After 900, keep tier 10 and add pack coordination (004 / 023) instead of a tier 11 damage stat. Choke-camping stays valid at any player level because `aiTier` is relative to enemy level.  
**IMPLEMENTATION_APPROACH:** Pure helper first with a fixture map (corridor vs open room). Then one branch in `decideEnemyAction`. Same `(enemy, ctx, rng)` must stay deterministic. Summoner path (`decideSummonerAction`) is out of scope.  
**BALANCE_CONSIDERATIONS:** Camping must not soft-lock the player behind an unbreakable wall of bodies (Barrier / pull / 018 still answer it). Do not also grant extra HP. Band-0 / `aiTier` 1–2 packs must still approach.  
**QA_REQUIREMENTS:** Same `(enemy, ctx, rng)` ⇒ same `EnemyAction` when `aiTier` is held fixed; summons still decrement lifespan on their own turn; Death Realm / rest idle wander unchanged; no Doka write; occupancy still refuses stacked tiles.  
**PRIORITY:** P2  
**STATUS:** PROPOSED  
...
**What new decision does this create?** “Tier-3+ will hold the corridor — do I Barrier it, pull the camper off with Collapse, or take the long ice walk?”
...
**SYSTEMS_AFFECTED:** New `engine/bossRushPairHooks.ts` + tests; one-line dispatch from existing boss-turn / death pipeline. Do not retune jackpot Doka/XP. Do not grow `useBossSystem` if the hook table can live beside `bossKits.ts`. Hold Wave 5–6 / Table C–D (#367 / #406) until Room 0 is live.  
...
**WHY_IT_IMPROVES_STRALT:** Blood Moon is live and is the same anti-pattern as Titan's Vigor: a DAMAGE reskin wearing a world-event name. CDA names it. Converting it is cheaper than inventing Fog, and it stops high-level design from treating modifiers as stat sticks. Adopted from #397.  
...
**IMPLEMENTATION_APPROACH:** One PR, one hook. Do not retune Mirror Field, Titan's Vigor, or Glass Realm in the same change. 024 (Gravity) is a different unused id — do not combine.  
**BALANCE_CONSIDERATIONS:** If converting to extra elite roll, wait until 025/002 exists or no-op the elite and use enrage instead. Do not stack with 015 on band 0. Fury potion 1.25 stays independent.  
...
**BALANCE_CONSIDERATIONS:** Wound-once must still lose to a second lethal (lava + strike). Do not combine with 016 extreme or 022 on band 0. If 025 `shielded` is live, do **not** also give every Titan combatant the same cheat-death — pick the extra-elite-roll option instead.  
...
- Wave 2 / 3 / 4 / 5 / 6 / 7 / **8** families, spell ids, encounter rooms, Rush tables, or `WF-*` rows **before** 021, 014, 019+F, 018, and 003. Open PRs #342 / #344 / #347 / #348 / #349 / #351 / #367 / #371 / #396 / #399 / #401 / #405 / #406 / #411 / #416 / **#452** / **#454** / **#457** / **#459** / **#463** / **#474** / **#479** / **#480** / **#518** / **#525** / **#558** / **#563** are **Hold**.
...
- Restacking `WorldExploration.tsx` / `challengeCompletion.ts` while **#327** (or twin **#370**) is open, or `mapGen.ts` while **#331** is open.
- 019 without PREREQ-F (queens become Blood-Mend healers).
- 021 without PREREQ-H (family chassis overlay that still lies about paper HP **and** RES/SP).
- 025 as +HP / extra `calcEnemyMaxHp`. Shield is one cheat-death, then kill.
- 026 as a `mapGen` punch, a required portal block, or a second challenge predicate.
- Rewriting Enemy Register from lore into a live bestiary.
- Fighting #366 on `EXPANSION_PROPOSALS_2026-09-02.md`, #397 on the 09-22 catalog, #453 on the 09-23 catalog, #510 on the 09-24 catalog, or rewriting README while older open PRs own it.
- Reminting 019–026 under new dates.
- 027 as a `mapGen` punch, a new `WF-*` id, an encounter/event slot, or the catalog’s `extraHazardCount` 3–6 spray. One Ember Vein cell.
- 028 as `instantKill`, a rewrite of `decideEnemyAction`, or a Wave-N AI sheet. One gate, extracted helper.

---

## Recommended implementation order

1. **021 Family-preferred chassis + PREREQ-H** — `spawnPolicy.ts` only; readable identity this week; stop the paper-HP **and** overlay RES/SP lie; no #327 race.
...
5. **025 First elite token `shielded`** — after 021; `spawnPolicy` + `eliteModifiers.ts`; one cheat-death, not +HP.
6. **023 One summoner slot** — after #327 releases the battle-start loop. First composition decision. Keep `isSummoner` flag (inference never returns `summoner`).
7. **020 `kill_leader_last`** — after #327 releases `challengeCompletion.ts`.
8. **018 Push/pull on existing ids** — occupancy already tested; creates positioning combos without Motoko.
9. **024 Gravity Well** — after walk-occupancy freeze / 018, so attract is shared.
10. **027 Ember Vein** — first live `WF-*`; existing floor cell; no `mapGen` punch; freeze further world-dynamics rows.
11. **026 `contest_shrine`** — after #327; existing floor cell; no `mapGen` punch.
12. **028 `chokepointCamp`** — extracted `aiTierGates` helper; do not grow `enemyAI.ts` with Wave-N tables.
13. **003 Observed-spell discovery** — after PREREQ-J (later EOP file + deploy confirmation). Needs 001 so there is something worth observing.
14. **002 Elite remainder** + **004 Pack composition remainder** — once `shielded` and the summoner slot exist, extra tokens and roles multiply them.
15. **008** as the scoreboard for 001/003. **016** remainder (encounter/event slots) after 027. **009** fog + slime≠ice. **017** then **011**. **022 / 015 / 005 remainder / 007** last.

Combinatorial target once 021+014+019+001+023+025+026+027+028+002+003+004+009/016+018+024 are live:

```
family × preferred chassis × relative band × piece variant × aiTier × kit band
  × elite token (shielded first) × pack role (incl. one summoner slot)
  × map archetype × map modifier (incl. gravity attract) × world feature (Ember Vein first)
  × displacement verb × optional shrine contest × choke-camp read
```
