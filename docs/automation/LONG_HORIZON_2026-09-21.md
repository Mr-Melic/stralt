# Long-Horizon Infinite Progression — 2026-09-21

Stralt has no character level cap in data or persist. This run re-simulated the **live formulas** at HEAD `0f5363f` (post-#332). Players are **not** assumed to reach these levels quickly. The mid-teens already exhaust intended XP income. Observed max level (unknown here) is not a cap.

Stress levels: 1, 10, 15, 25, 48, 50, 78, 100, 250, 325, 500, 1000, 2500, 5000, 1018, 1019, **10000, 50000, 100000**.

**Telemetry:** none. No encounter / duration / death / discovery series. Calibration is skipped. Real play would only check the synthetic model, not stop the horizon.

**This run does not redesign progression.** Findings are ACTION_IDs only. Prior `LHIPS-2026-08-31-001..014`, `LHIPS-2026-09-01-001..004`, and `LHIPS-2026-09-02-001..002` stay NEW unless a formula moved. This file adds IDs only for failures the earlier harness never measured (frozen INIT, CHC saturation).

Harness: `src/frontend/src/utils/longHorizonSim.ts`  
Ledger: [`ACTION_IDS_LHIPS_2026-09-21.md`](./ACTION_IDS_LHIPS_2026-09-21.md)
...
| Enemy combat HP | `floor(50 * (1 + (L-1)*0.05))` (`WorldExploration.tsx` 3606–3611) |
| Player live max HP | `floor(100 * (1 + (L-1)*0.05))` (`WorldExploration.tsx` 3400–3406) |
| Player formula HP | `round(100 * 1.05^(L-1))` — unused for HP; JSON-null (IEEE Inf) at 50_000+ |
| Player battle AP/MP | unbounded `getPlayerBaseStats` (`WorldExploration.tsx` 12105–12116) |
| Player battle INIT | persisted `characterStats.init` (`WorldExploration.tsx` 11948); create value 10 (`startingChampionStats.ts` 15) |
...
## What moved since 2026-09-02

| Prior ID | Drift |
| :--- | :--- |
| LHIPS-2026-09-01-003 | Persist AP/MP is no longer a flat 20 at every level. `persistApWriteCap` (`adminGuard.mo` 649–658) is official formula then `MAX_PERSISTED_AP` 20, with grandfathering. Battle init still uses **unbounded** `getPlayerBaseStats` (`WorldExploration.tsx` 12105–12116). Formula AP 21 at 325 vs persist 20 remains. Combat ignores the stored 20. Not re-filed. |
| LHIPS-2026-08-31-003 | Confirmed 100% under-level at **100000** as well as 10000 / 50000. `maxTier` formula unchanged. |
| LHIPS-2026-08-31-007 | `setCurrentZoneTier` line drifted (WX 4680); still unused by kits. Object `levelZone` still yields zone-0 kits. |
| (new measurements) | Frozen create INIT vs scaling enemy INIT. Enemy CHC saturates to 100%. |

Unchanged and still NEW: XP wall 001; HUD sat 09-01-001; IEEE 002; RES/HP/AI/kits/discovery/summoner/boss/challenge 004–011; jackpot + 100k/500k 09-01-002; leftover Nat / pow2 09-01-004; GameKey 10M 09-02-001; spell `10*2^n` 09-02-002; no telemetry 014.

Line numbers in WorldExploration / `main.mo` drifted; XP/spawn/kill-XP/kit/jackpot formulas did not.
...
| 50 | 5.63e16 | MAX_SAFE | ~2,700 | ~2.1e13 |
...
| 10000 | Infinity | MAX_SAFE | ~59,640 | never |
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 11.4 | 1 | **78** | 0 | 0.69 | 0.31 |
| 10 | 11.0 | 1 | 78 | 0 | 0.71 | 0.29 |
| 25 | 26.8 | 1 | 97 | 0.28 | 0.44 | 0.28 |
| 100 | 95.3 | 21 | 164 | 0.27 | 0.46 | 0.27 |
| 500 | 495.5 | 421 | 569 | 0.28 | 0.45 | 0.28 |
| 1000 | 991.9 | 923 | **1020** | 0.29 | 0.58 | 0.13 |
| 2500 | 993.3 | 931 | 1020 | **1.00** | 0 | 0 |
| 10000 | ~994 | 931 | 1020 | **1.00** | 0 | 0 |
| 50000 | ~993 | 931 | 1020 | **1.00** | 0 | 0 |
| 100000 | ~993 | 931 | 1020 | **1.00** | 0 | 0 |

Dungeon boost is `+ boost * tierSize` (max +30 at depth 5) on top of this window. Live dungeon `maxDepth` is still rolled 3–5 (`WorldExploration.tsx` 6324). 100k confirms the 999 ceiling does not grow with the player.
...
| Level | Live player HP | Formula (unused) HP | Victory HP floor | Enemy live HP (at player L) | Persist AP cap | Formula AP |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 100 | 100 | 60 | 50 | 8 | 8 |
| 10 | 145 | 155 | **150 (> max)** | 72 | 8 | 8 |
| 25 | 220 | 323 | 300 | 110 | 9 | 9 |
| 50 | 345 | 1,092 | 550 | 172 | 10 | 10 |
| 100 | 595 | 12,524 | 1,050 | 297 | 12 | 12 |
| 325 | 1,720 | 7.33e8 | 3,300 | 860 | **20** | **21** |
| 500 | 2,595 | 3.75e12 | 5,050 | 1,297 | 20 | 28 |
| 1000 | 5,095 | 1.47e23 | 10,050 | 2,547 | 20 | 48 |
| 10000 | 50,095 | overflow-scale | 100,050 | 25,047* | 20 | 408 |
| 100000 | 500,095 | JSON-null | 1,000,050 | 250,047* | 20 | 4008 |

\*Live enemy HP at the **player** level. Actual spawned enemies cap near 1020 (~2,547 HP) once the player is past ~1000.
...
`calcScaledDamage` still ignores caster level (`combatMath.ts` 130–137) — player offense grows only with spell upgrades (+3%/level). Family 30% (`spawnPolicy.ts` `FAMILY_VARIANT_CHANCE`) is still overwritten at battle start (WX 11873–11902).

**LHIPS-2026-08-31-004, 005 (still).** **LHIPS-2026-09-21-002 (CHC).**

---

## Initiative (new measurement)

Create-time INIT is 10. Battle uses `characterStats.init`. `saveBattleStats` writes `_minNat(initiative, stored)` — INIT cannot rise. Enemy INIT is `roll(3, 6+level*1.2, piece.mult)`.

Share of 3-enemy packs where player INIT is strictly higher than every enemy (4000 samples; enemy levels from `pickEnemyLevelFromTiers`):
...
Level-1 packs already roll mean enemy ~11 (003), so first-turn share is ~17% in the tutorial band. Same-band collapse is visible by 15. Sample rook at enemy 80: INIT 76 vs player 10.

**LHIPS-2026-09-21-001.**
...
All 32 frontend spells are `starterSpells` and are `baseSpells` at create (`WorldExploration.tsx` 2395–2408). `shouldIncludeBackendSpellInLibrary` (`adminSafety.ts` 712–719) only hides **retired** catalog rows. No rarity, no duplicate-discovery path.
...
Combat boss HP stays 350 (Pale Archbishop sample) at every player level including 100000. Guide `1.08^5` shows 514. Solvable at all simulated levels as a time sink, not as a scaled threat, once player HP grows and enemy kits stay in band 0. Dungeon extra-enemy / tier-boost / Doka-multiplier tables stop at depth 5. Live chain length is rolled 3–5. Highest catalog boss XP multiplier is 6 (`bossDefaults.ts` 755). Challenge under-30 / under-50 damage still fails on a single placeholder hit at 14 / 24.
...
| Unused exponential HP as Number | JSON-null / Inf at 50_000+ |
...
**LHIPS-2026-09-01-001, 003, 004 (still).** **LHIPS-2026-09-02-001, 002 (still).** **LHIPS-2026-09-21-001.**
...
- No reopen of unchanged 2026-08-31, 2026-09-01, or 2026-09-02 IDs.
