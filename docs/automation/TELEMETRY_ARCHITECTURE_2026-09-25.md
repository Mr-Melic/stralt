# Gameplay telemetry architecture — 2026-09-25
...
**Prior designs (unmerged, same SHA):** PRs **#352** (09-21), **#422** (09-22), **#450** (09-23), **#527** (09-24). On `main` the last merged design is [`TELEMETRY_ARCHITECTURE_2026-09-02.md`](./TELEMETRY_ARCHITECTURE_2026-09-02.md). Ledger: [`ACTION_IDS_GTAD_2026-09-25.md`](./ACTION_IDS_GTAD_2026-09-25.md).  
...
The 2026-08-31 through 09-24 **fail-open increment sidecar** is **reaffirmed**. This run does **not** invent a second architecture. `main` has **not moved** since the 09-21 cron (`0f5363f`). EVENT_SOURCE line numbers below were **re-read** on this SHA (battle start **12097–12099**, recap **12517**, dual death **13261** vs **13316**, flee **18935**, GameKey helper **275–279**, recap dismiss five `onClose` sites). They match 09-21 through 09-24.

This file patches three implementer hazards that 09-24 left stale:

1. **Open-PR queue grew again.** Oldest-first merge order is still **#327** then **#331**, then **#333–#579** — **247** still-open PRs targeting `main` at `open-pr-stack-compat.sh --self` (createdAt ascending, 2026-09-25 ~00:24 UTC; GitHub search later in the same run showed **249**). Unmerged GTAD designs are **#352** (09-21), **#422** (09-22), **#450** (09-23), and **#527** (09-24). Do not hitchhike telemetry maps, bindgen, or sidecar call sites onto any of them. Union README index rows; unique 09-25 files must not overwrite #352/#422/#450/#527 unique files. Pre-existing queue breaks (#389 WX, #392 ChatPanel, later combat/map/persist PRs) are not this PR’s unique delta.
2. **Queued persist PRs still are not EVENT_SOURCE.** 09-24 deferred Q-018 against **#375 / #377 / #387 / #391 / #426 / #431 / #435 / #439 / #445 / #482 / #488 / #490 / #493 / #497 / #499 / #508**. Those remain open. Additional persist PRs opened after the 09-24 cron: **#532** (skip `saveBattleStats` wipe after unseeded `handleBattleEnd` keep), **#540** (skip wipe after unseeded portal keep), **#545** (skip wipe after unseeded GameKey keep), **#552** (skip wipe after seeded GameKey keep), **#577** (keep feat/session caches across version wipe). On **main**, `noteUnconfirmedCredit` is still only the one-shot settle path (`dokaPersist.ts` **217–239**). No `unseededKeepWriteSkip` / `unseededVictoryKeepWriteSkip` helpers exist on `main` (repo search empty). Do not invent spend-note writers. Do not treat unmerged branch helpers as live increment sites.
3. **Recap dismiss five-path list is still correct** (re-read `PostBattleRecap.tsx` 2026-09-25): Escape **67–68**, backdrop **93–96**, overlay Enter/Space **98–111**, close **208**, Continue **609**. Increment **once**. Sibling TADD **#559** skipped a fifth Health matrix because these APIs are still absent (09-24 TADD **#504** skipped the fourth). Sibling TBC **#556** remains `WAITING_FOR_TELEMETRY` (prior WAITING reports **#333/#395/#462/#502**). World-dynamics **#503** (wave-7) and **#578** (wave-8) are still catalog-only (N-007); neither is a WX importer.

`GTAD-2026-09-01-001`…`014` remain the Phase 0/1 implementation tickets. Use **this file’s `EVENT_SOURCE` line numbers**. New IDs this run are stack-fence / extra persist-deferral only. **No new metric IDs.**

Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

---

## 1. What changed since 2026-09-24

| Topic | 2026-09-24 (PR #527, SHA `0f5363f`) | 2026-09-25 (this cron, same SHA) |
| :--- | :--- | :--- |
...
| Open sibling PRs | **190** open through **#520** | **247** open through **#579** (`open-pr-stack-compat.sh --self`). Oldest still **#327** then **#331**. GTAD siblings **#352** + **#422** + **#450** + **#527**. TBC WAITING **#333/#395/#462/#502/#556**. Dashboard **#345/#424/#455/#504/#559** (09-25 TADD **skipped** a fifth Health matrix). |
| Persist PRs (not EVENT_SOURCE) | #356/#375/#377/#385/#387/#388/#391 + #426/#431/#435/#439/#445 + #482/#488/#490/#493/#497/#499/#508 | Those still open, **plus** #532/#540/#545/#552/#577. Q-018 still deferred. No `unseeded*KeepWriteSkip` on `main`. |
...
| One-shot / absolute skip | Q-015 / Q-016 | Unchanged. `settleOneShotAfterCredit` **144–152**; `resolveOneShotCreditSettle` **194–215**; `shouldSkipAbsoluteDokaWrite` **202–212**. |
...
| Dual death | Two entries | Unchanged. `_handlePlayerDeath` **13261**; HP-watch **13316+** still does **not** call it (recap **13355**). Flee **18935** does. |
| Family / summoner | `spawnPolicy` 30% + live summoner roll | Unchanged. `applyFamilyVariantsToRoster` **289**; overwrite `sp`/`sr`/`init`/`res`/`chc` **11873–11903**; summoner **11932–11943**. `FAMILY_VARIANT_CHANCE` **0.3** (`spawnPolicy.ts` **35**). |
| World features | Catalog only; WX does not import | Unchanged on `main`. Open **#503** (wave-7) and **#578** (wave-8) add catalogs + tests; still **no** `WorldExploration.tsx` importer. N-007 stays dormant. |
...
| `APP_VERSION` | v163 | Still **v163** (`App.tsx` **14**). Version-gate keep list on `main` is still spawn/level-up + `*_inventory` only (`versionGate.ts` **9–11**). Unpaid-death key keep is **#508**, not `main`. |
...
10. Hitchhike onto still-open **#327** / **#331** or any later still-open persist/combat PR (**#340**, **#356**, **#375**, **#377**, **#379**, **#387**, **#389**, **#391**, **#426**, **#431**, **#435**, **#439**, **#445**, **#482**, **#488**, **#490**, **#493**, **#497**, **#499**, **#501**, **#508**, **#532**, **#540**, **#545**, **#552**, **#577**, …). Dedicated human-approved PR after those land, or a human-unioned restack. Unique 09-25 files must not overwrite unmerged **#352** / **#422** / **#450** / **#527**.
...
### 3.4 Recap dismiss correction (still 2026-09-22; re-verified 2026-09-23, 2026-09-24, and 2026-09-25)
...
**Motoko / EOP note:** add telemetry maps in a **new** `src/backend/migrations/YYYYMMDD_HHMMSS.mo` that sorts **after** `20260901_000000`. `OldActor = {}`. `NewActor` = empty maps + zero epoch. Bump `mops.toml` `check-limit` to **6**. Run `python3 scripts/check-eop-stables.py` and `bash scripts/caffeine-import-gate.sh backend` (check-stable vs `.old` = Caffeine Aug-31 signature **and** every `snapshots/deployed/*.most` + `empty-canister.most`; `unsupported/` must still fail). Do not land this in the same PR as open **#327** / **#331** or later persist/combat PRs. Do not restack unique 09-25 docs by concatenating a second copy of a helper into a sibling file.
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; rows marked **NEW 2026-09-01**, **NEW 2026-09-02**, **NEW 2026-09-21**, **NEW 2026-09-22**, or **NEW 2026-09-23** are additive. This run adds **no new metric IDs** — only queue/persist-deferral evidence (09-25).
...
EVENT_SOURCE: Tag both entry points. Flee **18935**; lava/spike HP-watch **13316+**; combat/DoT via `_handlePlayerDeath` callers. Enum only: `combat_melee|combat_spell|dot|lava|spikes|flee|other`.  
...
| Charts for `worldFeatures` / formations / discovery / Enemy Register as if live | Catalog-only or lore. N-007 / N-008. Open **#503** / **#578** are still not WX importers. |
...
| Editing frozen `NewActor`s or hitchhiking telemetry onto #327 / #331 / later persist-combat PRs (now through **#579**) | EOP + stack-compat. Overwriting #352/#422/#450/#527 unique files is also forbidden. |
...
| Implementing Q-018 against unmerged #375/#377/#387/#391/#426/#431/#435/#439/#445/#482/#488/#490/#493/#497/#499/#508/#532/#540/#545/#552/#577 | EVENT_SOURCE must be on `main`. |
...
12. After older queue items merge, **re-read EVENT_SOURCE line numbers** before implementing. This catalog is valid at `0f5363f` only. The 247-PR queue will move WX/persist lines the moment **#327** / **#331** land.
...
| Balance / Content Analyst (`2786666f`) | Stay `WAITING_FOR_TELEMETRY` (open **#556** already says so; prior **#333/#395/#462/#502**). | Use Phase 2 dimensions; never invent OVERPERFORMING from source. Enemy Register is lore (N-008). |
| Dashboard Designer (`4b026695`) | Snapshot panels + honest “not measured.” Shop series = GameKey. H14 audit log is ops, not battles. Open **#559** correctly skipped a fifth Health matrix (09-24 **#504** skipped the fourth). | Bind to `adminGetTelemetrySnapshot` / progression snapshot only. |
...
Related: `AQA-2026-08-30-012` (still NEW; shop site patched in 09-21). `GTAD-2026-09-01-*` remain the implementation tickets; **read this file for line numbers, GameKey helper, one-shot, recap-dismiss, EOP, and the 247-PR stack fence.** `GTAD-2026-09-02-*`, `GTAD-2026-09-21-*` (PR #352), `GTAD-2026-09-22-*` (PR #422), `GTAD-2026-09-23-*` (PR #450), and `GTAD-2026-09-24-*` (PR #527) are still NEW additive deltas. `GTAD-2026-09-25-*` are this run’s deltas only.
