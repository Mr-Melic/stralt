...
Live spawn/admin code has not moved since 09-21 (`0f5363f`). New IDs exist because **queued sibling catalogs and occupancy PRs after #534** (elite waves 7–8, FSN drop 7, EED Ley/Fan/Pit/Font + Gale/Twin/Pincer rooms, white-gateway re-legalize, portal-seeded destack, occupied-alcove dump punch, jackpot `complete(9)` order, SpellSummonFields UI) would be forked if the owner pack were implemented from 09-24 alone.

No WDD wave-8 PR existed in this cron window. **Do not invent `catalogWave: 8` or `WF-*` ids.** `enabledWaves` stays any non-empty subset of `{1,2,3,4,5,6,7}` until a later WDD brief lands. Same for Rush Table G: do not invent `G0`–`G3`.
...
| #344 / #399 / #454 / #503 WDD | Waves 4–7 `WF-*` on the **same** `WORLD_FEATURES` array. `CatalogWave` becomes `1\|2\|3\|4\|5\|6\|7` after those land. On `main` it is still `1\|2\|3`. **No wave 8 this window.** |
| #348 / #401 / #459 / **#537** FSN | Drops 4–6, then **drop 7** (Face Court / Gait Snare / Vault File / Span Gate — Wave 6 family combinations). |
| #347 / #396 / **#479** / **#519** EED | Tide/File/Clock, Wick/Rime/Smoke/Plus, then **Ley/Fan/Pit/Font** and **Gale/Twin/Pincer** `ENC-*` rooms. 09-24-003 bounded rooms as day-1…5 / drop-1…6 and did not name #479 / #519. |
| #349 / #405 / #452 / **#535** / **#558** elite | Waves 4–6 family sheets, then **Wave 7** (14 families) and **Wave 8** (17 families). |
| #367 / #406 / #474 / #518 boss bible | Tables C–F. **No Table G this window.** |
...
| `WDEAD-2026-09-24-001`…`007` | Yes (queued #534) | Waves 6–7 / drop 6 / Tables E–F / elite wave 6 / preferred-room snap / relative objectives. Stay `NEW`. Do not re-issue 09-24-007 if `DEFAULT_CHALLENGES` is still absolute (`challengeCompletion.ts` 44–109). |
...
## 4. Encounter / formation catalogs — drop 7 plus two EED days
...
- #479 (`EED-2026-09-23-001`) — Ley / Fan / Pit / Font `ENC-*` rooms (current-MP spend, 90° wedge, isolate-pet, LoS pits, mercy-font, enter-push). Filed **after** #451; 09-24 did not name it.
...
- #537 (FSN drop 7) — Face Court, Gait Snare, Vault File, Span Gate, Span Plug, Cadence Choir/Mute, Brand Cover, Lintel Coup, Pin Pit, … packing **Wave 6** families from #452. Wave 7–8 families (`#535` / `#558`) stay deferred — do not concatenate them onto drop 7.
...
Queued #536: clearing room 9 used to `completeRun` → `resetRunState` → `abortBossRush` **before** `complete(9)`, so master complete / run count / `highestRoomCompleted=10` never landed. Owner sequencing VALIDATE:

- Live jackpot is still namespace room 9. Activating Tables B–F must not skip or rewrite `complete(9)`.
- Do not encode E/F as canister `roomIndex` 10–17.
- Do not invent Table G this run.
...
`WDEAD-2026-09-24-005` named Wave 6 only. FSN drop 7 packs Wave **6** families, not 7–8. Do not append Wave 7/8 ids onto live `FAMILY_TYPES`. Restack union ≠ concatenate.
...
CONTENT
  ...
  World Events   eligibility (rest / deathRealm / whiteSanctuary) · rarity · hazards
                 · elites · grants · hooks · catalogWave 1–7 (no invented 8)
...
57. Encounter / formation ids resolve to EED #479 / #519 and FSN drop 7 as well as earlier sheets. Unknown `formationId` fails VALIDATE. Wave 7–8 family ids are not on drop-7 sheets.
58. Rush sequencing still distinguishes 0–9 / B / C / D / E / F. Activating extra tables does not skip `complete(9)` and does not use canister `roomIndex > 9`. No invented Table G.
...
- Do not invent WDD wave 8 or Rush Table G.
...
| WDEAD-2026-09-25-003 | Ingest EED Ley/Fan/Pit/Font + Gale/Twin/Pincer and FSN drop 7 — do not fork arrays | P1 |
| WDEAD-2026-09-25-004 | Rush extra tables must not skip jackpot `complete(9)` and must not use `roomIndex > 9` | P1 |
