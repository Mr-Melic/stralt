# Content Diversity & Repetition Audit — 2026-09-25
...
**Prior auditor HEAD:** `0f5363f` (2026-09-24). Every 09-24 check is **still true**; line numbers did not move.

ACTION_IDs: [`ACTION_IDS_CONTENT_DIVERSITY_2026-09-25.md`](./ACTION_IDS_CONTENT_DIVERSITY_2026-09-25.md).

On `main`: [`ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md`](./ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md) (`CDA-2026-09-02-001` … `015`).

Unpublished ledgers (still **OPEN**): GitHub **PR #359** (09-21, `016`–`018`), **PR #403** (09-22, copies `016`–`018` + `019`), **PR #465** (09-23, copies `016`–`019` + `020`), **PR #521** (09-24, copies `016`–`020` + `021`). This file **reaffirms** those ids and **mints** `CDA-2026-09-25-022`. Do not mint twins.

Sibling design that already exists — **do not re-propose as new families / rooms / spells**:

| Catalog | Status on this HEAD / flock |
| :--- | :--- |
| Enemy family sheets Wave 1–7 | `ENEMY_ELITE_EVOLUTION_*` — PROPOSED. Wave 6 = open **#452** (docs). Wave 7 = open **#535** (docs, after CDA-021) |
| Named formations | `docs/design/ENEMY_FORMATIONS_*` — PROPOSED, unwired. Drop 6 = **#459**. Drop 7 = **#537** (docs, after CDA-021) |
| ENC-* rooms | `docs/encounters/ENCOUNTER_EVOLUTION_*` — PROPOSED. Ley/Fan/Pit/Font = **#479**. Gale/Twin/Pincer = **#519** |
| Spell verbs (push, attract, root, trap, delayed execute, …) | `SPELL_PROPOSALS_*` — PROPOSED. Wave 6 = **#463** / SDE **#480**. Wave 7 SDE = **#533** (docs, after CDA-021) |
...
**Do not recommend** HP/damage reskins, Register-only families (Crimson Spawn / Shadow Lurker / Storm Caller), a fifth reflect, a 53rd (or 117th) `WF-*`, Wave-7 `EnemyFamily` members, extra `BOSS_RUSH_ROOMS`, or a 33rd innate `starterSpells` row.
...
## 0. Prior-run checks (2026-09-24 → this HEAD)

`origin/main` did not move. 09-24 next-run questions, re-run 2026-09-25:

| # | Question | 2026-09-25 | Evidence |
| :--- | :--- | :--- | :--- |
...
| 10 | Did `worldFeatures.ts` grow past 52 unused rows **on main**? | **No on main.** Open flock would. | Main 52. **#503** still 116 ids. **#454** Wave-6 +1436 |
...
| 12 | Did PR #359, #403, or #465 merge (016–020 on `main`)? | **No.** #521 also still open. | `gh` search: 359 / 403 / 465 / 521 `state: OPEN` |
| 13 | Did `BOSS_RUSH_ROOMS.length` grow past 10? | **No.** | `useBossRush.ts` roomIndex 0–9 |
| 14 | Did Wave-6/7 names enter `EnemyFamily`? | **No.** | `gameTypes.ts` 12–20: original seven + `default`. No `isElite` / `isChampion` |
| 15 | Did PR #503 merge (52 → 116 `WF-*`)? | **No.** Still open. | `gh api` files: `worldFeatures.ts` +1914 / −3 |

What **did** change since 09-24 is the **open flock after CDA-021**, not live combat. Same-day 2026-09-24 automations that opened **after** PR #521 (00:21Z) authored Wave-7 elite sheets (**#535**, docs), Wave-7 SDE (**#533**, docs), and formations drop 7 (**#537**, docs). CDA-021 already froze Wave-7 **WF-\*** TypeScript (**#503**). Those three later PRs are docs today; treating them as a green light to add `EnemyFamily` members, `starterSpells` rows, or spawn `formationId` is the only new actionable gap (`CDA-2026-09-25-022`).
...
Live: four quadrants around (8,8) then fill, Chebyshev ≥ 4 (WX, `SPAWN_MIN_CHEBYSHEV` 4). Battle start re-scatters ≥ 2 from allies, ≥ 3 from player (11869–11891). Named formation catalogs (including drop 7 **#537**) and `worldFeatures.ts` encounter slots are unwired.

### 1.6 Bosses

19 `BOSS_IDS` (`bossTypes.ts` 390–410). `BossAbility` enums are **unique per later boss** — that axis is actually diverse. Kits are not: they remix the same frontend ids (`spell-shadow-veil` ×7, `spell-cursed-wound` ×6, `spell-frost-nova` ×6, `spell-swap` ×5).
...
- Wave-7 elite sheets (**#535**) = more unread family names for the same unbound sticker.
...
## 5. Open flock (proposed, not live) — 2026-09-25
...
| ---: | :--- | :--- | :--- |
| **#503** | 2026-09-24 00:08 | `worldFeatures.ts` +1914 (52 → **116** `WF-*`) under a “docs” title | **021**; violates 018 / 020 |
...
| **#535** | 2026-09-24 00:28 (**after** CDA-021) | Wave 7 elite sheets (docs only, +732) | **022** — docs OK; not `EnemyFamily` |
| **#533** | 2026-09-24 00:26 (**after** CDA-021) | Wave-7 SDE (docs only) | **022** — docs OK; not `starterSpells` |
| **#537** | 2026-09-24 00:35 (**after** CDA-021) | Formations drop 7 (docs only, +1320) | **022** — docs OK; not spawn `formationId` |
...
| #525 | 2026-09-24 | Wave 7 tactical spell proposals (docs) | 012 / 015 / 017 — do not append to `starterSpells` |

---

## 6. What to build (only if it creates a new player decision)

Do **not** add families, ENC rooms, Wave-2–7 spells, `WF-*` rows, or named formation ids in spawn until the seven live families survive battle start and select kit/AI. Those catalogs already exist as PROPOSED.
...
- Authoring Wave-8 elite / SDE / formation catalogs. Unused Wave-1–7 sheets already oversupply names. Wave-8 Rush Table F already exists as **#518** (docs).
- Landing **#503** (or #454 / #399 / #344) as “just docs” while they append unread `WF-*` rows.
- Landing **#535 / #533 / #537** as `EnemyFamily` members, `starterSpells` rows, or spawn `formationId`.
