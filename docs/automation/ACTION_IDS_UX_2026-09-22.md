# ACTION_ID catalog — 2026-09-22 UX audit

Stable IDs for the daily player-journey auditor. STATUS values: NEW | OPEN | IMPLEMENTED_THIS_RUN | IMPLEMENTED | SUPERSEDED.

Reuse IDs from [`ACTION_IDS_UX_2026-09-02.md`](./ACTION_IDS_UX_2026-09-02.md) and PR #372’s 2026-09-21 catalog when the finding is the same. Do not mint twins.
...
```
ACTION_ID: UX-SELECTION-PBV-HEADING
TITLE: Character select heading matches champion language
CATEGORY: action-discoverability
PRIORITY: P2
CONFIDENCE: HIGH
FILES_OR_SYSTEMS: src/frontend/src/components/CharacterSelection.tsx
CURRENT_BEHAVIOUR: H1 was “Choose your Paper Baby Vampire!” after landing chrome already says ÆSTRALTØ.
DESIRED_BEHAVIOUR: Carved-stone “Choose your champion” — same gradient, not SaaS “Select an account.”
EVIDENCE: CharacterSelection.tsx h1 vs LandingPage ÆSTRALTØ; DESIGN.md chess/champion tone.
RECOMMENDED_ACTION: Heading copy only. GameFlow wordmark and index.html title stay a product-name call (UX-LAUNCH-TAB-TITLE).
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: LOW
VALIDATION_REQUIRED: Selection still lists three slots; Play/Create unchanged.
STATUS: IMPLEMENTED_THIS_RUN
```

```
ACTION_ID: UX-SELECTION-XP-THIS-LEVEL
TITLE: Slot XP bar names leftover experience
CATEGORY: reward-clarity
PRIORITY: P2
CONFIDENCE: HIGH
FILES_OR_SYSTEMS: src/frontend/src/components/CharacterSelection.tsx XpBar
CURRENT_BEHAVIOUR: Slot cards showed leftover/needed as “N / N XP” with no “this level.”
DESIRED_BEHAVIOUR: “N / N this level” plus hover “Leftover experience in this level, not a lifetime total.” Keep Dofus purple numerals.
EVIDENCE: XpBar uses xpHudProgress leftover; changelog already describes leftover XP.
RECOMMENDED_ACTION: Display-only. Do not change xpHudProgress math or recolor the bar to gold.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: LOW
VALIDATION_REQUIRED: Slot XP still matches leftover/needed; Play still hydrates the same character.
STATUS: IMPLEMENTED_THIS_RUN
```

```
ACTION_ID: UX-SELECTION-PLAY-HINT
TITLE: Play says where the player goes next
CATEGORY: action-discoverability
PRIORITY: P3
CONFIDENCE: HIGH
FILES_OR_SYSTEMS: src/frontend/src/components/CharacterSelection.tsx
CURRENT_BEHAVIOUR: Play had no title/aria beyond the visible word.
DESIRED_BEHAVIOUR: title “Enter the realm”; aria-label “Play — enter the realm.”
EVIDENCE: character_selection.play_button.
RECOMMENDED_ACTION: Copy only. Do not change onPlayCharacter.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: LOW
VALIDATION_REQUIRED: Play still loads WorldExploration for that slot.
STATUS: IMPLEMENTED_THIS_RUN
```

```
ACTION_ID: UX-SELECTION-ERROR-ALERT
TITLE: Slot-load errors announced to assistive tech
CATEGORY: feedback
PRIORITY: P3
CONFIDENCE: HIGH
FILES_OR_SYSTEMS: src/frontend/src/components/CharacterSelection.tsx
CURRENT_BEHAVIOUR: Failed-to-load copy had no role.
DESIRED_BEHAVIOUR: role="alert" on character_selection.error_state (same pattern as landing login).
EVIDENCE: CharacterSelection isError branch.
RECOMMENDED_ACTION: Add role + data-ocid. Retry still refetches slots.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: LOW
VALIDATION_REQUIRED: Retry still calls refetch().
STATUS: IMPLEMENTED_THIS_RUN
```

```
ACTION_ID: UX-GAMEOVER-PENALTY-COPY
TITLE: Game Over always states the 20/40 death rule
CATEGORY: death-explanation
PRIORITY: P1
CONFIDENCE: HIGH
FILES_OR_SYSTEMS: src/frontend/src/components/GameOverModal.tsx
CURRENT_BEHAVIOUR: 20%/40% lived only on amount chips, so a 0 leftover XP / 0 Doka death hid the rule. Chips said “XP (20% penalty)” not leftover.
DESIRED_BEHAVIOUR: Always-visible rule: “Death takes 20% leftover XP and 40% Doka.” Chips show amounts only when > 0.
EVIDENCE: GameOverModal chip gate `(xpLost > 0) || (dokaLost > 0)`; lava path still skips this modal (UX-DEATH-DUAL-MODAL).
RECOMMENDED_ACTION: Copy only. Do not rewire persistDeathPenalty or deathGuards.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: LOW
VALIDATION_REQUIRED: Combat Game Over with a loss shows amounts + rule; zero-loss still shows the rule; Enter the Death Realm still respawns.
STATUS: IMPLEMENTED_THIS_RUN
```
...
## Queued in older open PR #372 (do not duplicate this run)
...
CURRENT_BEHAVIOUR: <title> and OG tags are Paper Baby Vampires; GameFlow select/forge h1 is Paper Baby Vampires; landing chrome is ÆSTRALTØ. Selection h1 no longer uses the leftover nickname (UX-SELECTION-PBV-HEADING).
