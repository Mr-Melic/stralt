# Gameplay telemetry architecture — 2026-09-22
...
**Prior design (unmerged):** PR **#352** [`TELEMETRY_ARCHITECTURE_2026-09-21.md`](https://github.com/Mr-Melic/stralt/blob/cursor/gameplay-telemetry-architecture-0427/docs/automation/TELEMETRY_ARCHITECTURE_2026-09-21.md) on the same SHA. On `main` the last merged design is [`TELEMETRY_ARCHITECTURE_2026-09-02.md`](./TELEMETRY_ARCHITECTURE_2026-09-02.md). Ledger: [`ACTION_IDS_GTAD_2026-09-22.md`](./ACTION_IDS_GTAD_2026-09-22.md).  
...
`src/` hits remain: an empty WorldExploration intent comment (`WX` **16442–16446**), `longHorizonSim.telemetry.available = false` (**532–536**), and an optional combatant `intelligence` field (`gameTypes.ts` **307** — AI flavour, **not** an admin tab). No `recordTelemetryIncrements`. No `telemetrySidecar`. No `telemetryLifetime` map. GitHub code search for those symbols on this repo is empty.

The 2026-08-31 / 09-01 / 09-02 / 09-21 **fail-open increment sidecar** is **reaffirmed**. This run does **not** invent a second architecture. `main` has not moved since the 09-21 cron (`0f5363f`). This file patches three implementer hazards that 09-21 left stale:

1. **Open-PR queue exploded.** 09-21 fenced only **#327** / **#331**. Oldest-first merge order is still those two, then **79** still-open PRs through **#409** (docs, persist, combat, admin). Unmerged 09-21 design is **#352**. Do not hitchhike telemetry maps, bindgen, or sidecar call sites onto any of them. Union README index rows; do not overwrite siblings. Pre-existing queue breaks (#389 WX, #392 ChatPanel) are not this PR’s unique delta.
2. **Recap dismiss is five `onClose` sites, not four.** 09-21 labeled `PostBattleRecap.tsx` **111** as backdrop. Line **111** is overlay **Enter/Space**. Backdrop click is **93–96**. Also Escape **67–68**, close **208**, Continue **609**. Increment **once**.
3. **Queued persist PRs are not EVENT_SOURCE.** `#375` / `#377` / `#387` / `#391` propose throw-after-credit/debit notes. On **main**, `noteUnconfirmedCredit` is already used from one-shot settle (`dokaPersist.ts` **217–239**). Q-018 is deferred until those helpers exist on `main`. Do not invent spend-note writers in a telemetry PR.

`GTAD-2026-09-01-001`…`014` remain the Phase 0/1 implementation tickets. Use **this file’s `EVENT_SOURCE` line numbers**. New IDs this run are stack-fence / recap-dismiss / deferred Q-018 only.

Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

---

## 1. What changed since 2026-09-21

| Topic | 2026-09-21 (PR #352, SHA `0f5363f`) | 2026-09-22 (this cron, same SHA) |
| :--- | :--- | :--- |
| Production telemetry | None | **Still none.** Same two `src/` hits + combatant `intelligence?: number`. |
| `main` HEAD | `0f5363f` | **Unchanged.** Line numbers below match a fresh read of this SHA. |
| Open sibling PRs | #327 then #331 | **#327**, **#331**, then #333–#409 (**79** open). #352 = 09-21 GTAD docs. #333/#395 TBC WAITING, #345 dashboard matrix. Persist PRs #356/#375/#377/#385/#387/#388/#391 must not be treated as live increment sites. |
| Recap dismiss | Four sites; **111** called backdrop | **Five** sites. Backdrop **93–96**; Enter/Space **98–111**. |
...
| EOP | New later file after frozen `20260901`; `check-limit` 5→6 | Unchanged. `.old` stays Caffeine Aug-31 (no GameKey). #259 hitchhike warning remains obsolete. |
...
| Identifiable dumps | Leaderboard, OQL email, `getAllCharacters` | Same. `getAllCharacters` **532**. `getLeaderboard` **3390**. OQL `include Expose` **3498**; `gameKeyRequests.email` **3788**. |

Merged to `main` since 09-02: persist/combat/map/admin-safety PRs through **#332**. **No increment sidecar shipped.** Prior telemetry PRs remain docs only.
...
10. Hitchhike onto still-open **#327** / **#331** or any later still-open persist/combat PR (**#340**, **#356**, **#375**, **#377**, **#379**, **#387**, **#389**, **#391**, …). Dedicated human-approved PR after those land, or a human-unioned restack.
...
### 3.4 Recap dismiss correction (new this run)
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; rows marked **NEW 2026-09-01**, **NEW 2026-09-02**, **NEW 2026-09-21**, or **NEW 2026-09-22** are additive.
...
| Editing frozen `NewActor`s or hitchhiking telemetry onto #327 / #331 / later persist-combat PRs | EOP + stack-compat. |
...
| Implementing Q-018 against unmerged #375/#377/#387/#391 | EVENT_SOURCE must be on `main`. |
...
9. Do not land telemetry stables in the same PR as open **#327** (Striker / WX / challenge) or **#331** (portal destack / WX / mapGen / persist) or later persist/combat PRs unless a human explicitly unions them. Unmerged 09-21 design is **#352** — unique 09-22 files do not overwrite it.
...
12. After older queue items merge, **re-read EVENT_SOURCE line numbers** before implementing. This catalog is valid at `0f5363f` only.
