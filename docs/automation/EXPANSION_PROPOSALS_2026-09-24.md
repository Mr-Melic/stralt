# Stralt expansion catalog — 2026-09-24

**Author:** Expansion Director (cron `0 */24 * * *`, automation `3f31b18f-a492-11f1-a7d1-d6b4613131ce`)  
**This run:** seventh catalog. Re-rank against `origin/main` `0f5363f` (`Merge pull request #332` — accepted-challenge HUD). Author-date on that tip is **2026-09-03**. Calendar: 2026-09-24.  
**Prior living catalog on `main`:** [`EXPANSION_PROPOSALS_2026-09-02.md`](./EXPANSION_PROPOSALS_2026-09-02.md) (`HEAD` then `58302bc`).  
**Unmerged sibling catalogs:** PR **#366** (09-21; 019/020), **#397** (09-22; 021/022), **#453** (09-23; 023/024) — same `HEAD`, still drafts. This file **adopts** 019–024 rather than reminting them.  
...
All seventeen 08-31 / 09-01 / 09-02 cards remain **PROPOSED**. 019–024 remain **PROPOSED**. None shipped. None superseded. New IDs in this file are additive first-ship slices of already-ranked cards.

`README.md` still indexes the 09-01 catalog. Older open **#333** (TBC), **#346** (product-docs sync), **#420** / **#477** (docs sync) own that file. This run does not touch `README.md` / `AGENTS.md` / `ARCHITECTURE.md` / `WorldExploration.tsx` / `mapGen.ts` / `challengeCompletion.ts` / the 09-02 catalog (#366) / the 09-22 catalog (#397) / the 09-23 catalog (#453).
...
`main` did **not** move. #366 / #397 / #453 ranked the same `0f5363f`. This run is a re-rank against a still-growing open-PR queue (now through **#501**), plus two first-ship slices of 002 and 010 that do not need a new family / spell / Wave-N sheet.
...
| Open queue grew again | Oldest-first: **#327** Striker AoE (`WX` / targeting / `challengeCompletion`), **#331** portal destack (`mapGen`). Twin **#370** restacks the same Striker gate. Then **#333–#501** (~170 drafts from 09-21 / 09-22 / 09-23 flocks). Do not restack those files. |
| #366 / #397 / #453 still drafts | 09-21 / 09-22 / 09-23 catalogs are not on `main`. Adopt 019–024. Do not fight #366 on `EXPANSION_PROPOSALS_2026-09-02.md`, #397 on the 09-22 file, or #453 on the 09-23 file. |
...
| Family paper stats lie twice | Overlay still multiplies HP/damage (`spawnPolicy.ts` 261–272). Battle start then **also** overwrites RES/SP/CHC via `computeEnemyStats` (`WX` 11872, 11898–11902) and HP via `calcEnemyMaxHp` (`WX` 11970–11974). CDA 09-23 reconfirmed. PREREQ-H is identity-only, not “keep the overlay numbers.” |
| `inferArchetype` never returns `summoner` | Union at `enemyAI.ts` 86–93 includes `summoner`; inference (`447–477`) never emits it. Live summoner path is the `isSummoner` flag (`WX` 16375–16376 `decideSummonerAction`). 023 must keep that flag — do not rely on kit inference. |
| 019 without PREREQ-F is a trap | Band-1 queens gain `starter-heal` (`healAmount: 12`) and `inferArchetype` (`enemyAI.ts` 447–452) classifies them as healers. **Ship 019 with F in the same PR.** |
...
| Telemetry still none | TBC #333 / #395 / **#462**: `WAITING_FOR_TELEMETRY`. 0 collectors / 0 rows. Do not claim live demand. Do not retune XP, Doka, or spawn weights. |
| Engineering-health on `main` | Last landed: `MASTER_ROADMAP.md` / LONG_HORIZON / CDA / PXA dated **2026-09-02**. 09-21…09-23 MTD (#338 / #402 / **#448**), CDA (#359 / #403 / **#465**), PX (#343 / #393 / **#481**), LHIPS (#357 / #407 / **#475**) are still drafts. Their code claims that match this `HEAD` are accepted. |
...
| Summoner is still a per-body coin flip | `WX` **11932–11942**: `0.12 + playerLevel * 0.02`, 50/50 wolf or archer. Can mark every enemy a summoner. Leader is highest-level body (`WX` 12010–12014) with crown chrome already live. |
| Push / pull still unused | `applyPushback` / `applyAttract` at `occupancy.ts` **482 / 537**. Callers: tests only. |
| `worldFeatures` still tests-only | 1913 lines, 52 `WF-*` rows. No WX import. Freeze new rows (#344 / #399 / **#454**) until one is wired. |
| `combinedMechanic` still copy | `useBossRush.ts` 19–135. Zero engine readers. Rush table still 10 rooms. `weeping_pawn_2` still missing from `BOSS_IDS`. |
...
| Wave 2–6 elite / family sheets (09-01 / 09-02 + open **#349** / **#405** / **#452**) | More family ids | **Hold** until 001/014/018/019/021 exist |
...
| [`ENEMY_AI_EVOLUTION.md`](../ENEMY_AI_EVOLUTION.md) + 09-01 / 09-02 + open **#351** / **#416** / **#457** | Unbounded AI modules | 005 |
...
| [`WORLD_DYNAMICS.md`](../WORLD_DYNAMICS.md) + `engine/worldFeatures.ts` + open **#344** / **#399** / **#454** | Rarity / relative-difficulty overlay | 009, 016, 022, 024 |
...
| [`MASTER_ROADMAP.md`](./MASTER_ROADMAP.md) (on `main`: 09-02) + open **#338** / **#402** / **#448** | P0 integrity before expansion PRs | Process, not content |
| [`PX_COHERENCE_AUDIT_2026-09-02.md`](./PX_COHERENCE_AUDIT_2026-09-02.md) + open **#343** / **#393** / **#481** | Register / modifier / discovery honesty | 014 QA; Register is lore |
| [`LONG_HORIZON_2026-09-02.md`](./LONG_HORIZON_2026-09-02.md) + open **#357** / **#407** / **#475** | XP wall; kit NaN; summoner saturate | PREREQ-A/B/C; do not retune curve |
| [`CONTENT_DIVERSITY_AUDIT_2026-09-02.md`](./CONTENT_DIVERSITY_AUDIT_2026-09-02.md) + open **#359** / **#403** / **#465** | Live spawn is sticker + NaN kits; Blood Moon live; no elites | Confirms 001/014/019/021/022/023/025 |
| [`TELEMETRY_BALANCE_2026-09-02.md`](./TELEMETRY_BALANCE_2026-09-02.md) + open **#333** / **#395** / **#462** | STATUS WAITING_FOR_TELEMETRY | No demand claims |
...
| Rank | EXPANSION_ID | Title | Category | SCORE | Priority | vs 09-23 |
| ---: | :--- | :--- | :--- | ---: | :--- | :--- |
...
| 7 | EXP-2026-09-24-025 | First elite token: `shielded` | ENEMIES | 27 | P1 | **new** — first ship slice of 002 |
...
| 11 | EXP-2026-09-23-023 | One summoner slot per pack | ENEMIES / AI | 25 | P1 | adopted |
...
| 14 | EXP-2026-09-24-026 | First side objective: `contest_shrine` | COMBAT | 23 | P1 | **new** — first ship slice of 010 |
| 15 | EXP-2026-08-31-008 | Uncapped achievement / mastery ladder | PROGRESSION | 23 | P2 | same |
| 16 | EXP-2026-08-31-013 | Explicit spell-interaction layer | COMBAT | 23 | P2 | 018 first |
| 17 | EXP-2026-09-01-016 | Wire `worldFeatures` overlay (no mapGen rewrite) | WORLD | 23 | P2 | same; freeze new `WF-*` |
| 18 | EXP-2026-09-23-024 | Wire Gravity Well via existing attract | WORLD | 22 | P2 | adopted |
| 19 | EXP-2026-08-31-009 | Finish unused modifiers + combo-only world rules | WORLD | 21 | P2 | same; 024 can land first |
| 20 | EXP-2026-08-31-005 | AI sophistication ladder (unused gates) | AI | 21 | P2 | same |
| 21 | EXP-2026-08-31-007 | Dungeon room types without rewriting mapGen | DUNGEONS | 21 | P2 | wait for #331 |
| 22 | EXP-2026-09-02-017 | Honor Boss Rush `combinedMechanic` | BOSSES | 20 | P2 | same |
| 23 | EXP-2026-08-31-011 | Boss mechanic-pool scaling (not more HP) | BOSSES | 17 | P3 | same |
| 24 | EXP-2026-09-22-022 | Convert Blood Moon off ×1.25 damage | WORLD | 16 | P3 | same |
| 25 | EXP-2026-09-01-015 | Convert Titan's Vigor off +1000 HP | WORLD | 15 | P3 | same |

**Ship-next (not the same as SCORE):** **021 + PREREQ-H** → **014** → **PREREQ-A + 019 + PREREQ-F (one PR)** → **001** → **025** (after 021, `spawnPolicy` + new `eliteModifiers.ts`) → **023** (after #327) → **020** (after #327) → **018** → **024** (after walk freeze / 018) → **026** (after #327; no `mapGen` punch) → **003** (PREREQ-J) → 002/004 remainder.

SCORE ranks opportunity size. Ship-next ranks what creates a player decision **this week** without Motoko schema, without growing `enemyAI.ts`, and without racing open WX / mapGen / challenge / walk-occupancy PRs.

021 before 019 because band-0 kits already differ by piece type: coupling family → preferred chassis works **today**, lives in `spawnPolicy.ts`, and does not wait on #327. Bundle PREREQ-H in that same PR — `applyEnemyFamilyStats` is the paper-HP liar **and** battle start already discards overlay RES/SP. 019 before full 001 because the piece-kit table already exists and currently cannot fire — but 019 **without F** turns late queens into healers, so A+F+019 is one change. 014 still creates family identity independent of kits. 025 is the cheapest elite decision (one shielded body, not +HP) and can stamp in `spawnPolicy` after 021 makes chassis readable; do not wait for the full 002 table. 023 is the cheapest composition decision (one summoner, not a mid-40s coin-flip swarm) but its WX call site waits on #327. 026 is the cheapest fight-side-goal (contest an existing floor cell) and must not punch corridors.
...
- Wave 2 / 3 / 4 / 5 / 6 / 7 families, spell ids, encounter rooms, Rush tables, or `WF-*` rows **before** 021, 014, 019+F, 018, and 003. Open PRs #342 / #344 / #347 / #348 / #349 / #351 / #367 / #371 / #396 / #399 / #401 / #405 / #406 / #411 / #416 / **#452** / **#454** / **#457** / **#459** / **#463** / **#474** / **#479** / **#480** are **Hold**.
...
- Fighting #366 on `EXPANSION_PROPOSALS_2026-09-02.md`, #397 on the 09-22 catalog, #453 on the 09-23 catalog, or rewriting README while #333 / #346 / #420 / **#477** own it.
- Reminting 019–024 under new dates.
...
10. **026 `contest_shrine`** — after #327; existing floor cell; no `mapGen` punch.
11. **003 Observed-spell discovery** — after PREREQ-J (later EOP file + deploy confirmation). Needs 001 so there is something worth observing.
12. **002 Elite remainder** + **004 Pack composition remainder** — once `shielded` and the summoner slot exist, extra tokens and roles multiply them.
13. **008** as the scoreboard for 001/003. **016** next for world variety (one live `WF-*`, freeze the rest). **009** fog + slime≠ice. **017** then **011**. **022 / 015 / 005 / 007** last.

Combinatorial target once 021+014+019+001+023+025+026+002+003+004+009/016+018+024 are live:
...
family × preferred chassis × relative band × piece variant × aiTier × kit band
  ...
  × map archetype × map modifier (incl. gravity attract) × world feature
  × displacement verb × optional shrine contest
```
