# ACTION_IDs — 2026-09-22 Admin Feature & Drift Auditor

Durable ledger. Reuses AFDA-2026-08-31-* / AFDA-2026-09-01-020 / AFDA-2026-09-02-* / AFDA-2026-09-21-* for the same underlying problems.
...
HEAD `0f5363f` (origin/main). Open PR #334 still carries 2026-09-21 Tiers leftover honesty; this run unions that AdminDashboard delta and adds unique copy. Did not edit WorldExploration (older open PRs #327/#331 and many siblings overlap that file). Did not restack #341 AUX (built-in spell Retire) into this file — 3-way merge should keep both.

Tiny honesty this run (`AdminDashboard.tsx` only):
- Union #334: Tiers save no longer requires 100%; leftover ±3+ preview; Settings ban list is Shop; Boss Rush multiplier unused.
- Unique: Tiers CatalogNote names computeAITier bands + ENEMY_KITS; Settings names live Ground Doka on Map Modifiers; Bosses intro no longer claims "until a backend writer exists"; Achievements CatalogNote states live canister contract.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: `useBossQueries.ts` 1–31 and `useAdminQueries.ts` 508–557 still read/write `localStorage.pbv_boss_configs`. `WorldExploration.tsx` still loads the same key. `main.mo` still has `setBossConfig`, `deleteBossConfig`, `getAllBossConfigs`. Frontend `BossConfig` (`bossTypes.ts` 91–110) has `iconEmoji`, `loreText`, `chc`; Motoko (`admin.mo` 317–330) has `defeated`, `adminNotes`, no `chc`. Bosses tab this run states localStorage + schema mismatch (`AdminDashboard.tsx` ~7882–7888); it no longer claims a backend writer is missing. Duplicate hooks in both query files. Canister seed uses retired spell ids (025).
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: `computeAITier` (`combatMath.ts` 36–51) maps enemy level onto hard bands 10/30/60/100/150/250/400/600/900 then randomly re-rolls 1–10 at 30%. WorldExploration assigns `aiTier` at spawn (~5823, ~5865). `ENEMY_KITS` (`enemyAI.ts` 163–185) scale by levelZone with starter-frost / spell-inferno ids. Tiers CatalogNote this run names the bands (`AdminDashboard.tsx` ~3965–3975). Not a player career cap.
...
PRIORITY: P3
CONFIDENCE: HIGH
EVIDENCE: `WorldExploration.tsx` 2166 / 2220 and `AchievementsPanel.tsx` call `useGetAchievementConfigs`. Conditions are the closed `KNOWN_ACHIEVEMENT_CONDITIONS` list (`adminSafety.ts` 306–322). Wallet/level feats defer until `applyRewards` (`shouldDeferAchievementUnlockUntilRewardsPersist`). This run added a CatalogNote (`AdminDashboard.tsx` ~6724–6730). Challenges remain code-only (007).
SYSTEMS_AFFECTED: Achievements tab; recap unlocks
CURRENT_BEHAVIOUR: Saving an achievement with a known condition is live. Unknown conditions are rejected. Challenges still have no tab.
