# Gameplay telemetry architecture — 2026-09-23
...
**Prior designs (unmerged, same SHA):** PR **#352** (09-21) and PR **#422** (09-22). On `main` the last merged design is [`TELEMETRY_ARCHITECTURE_2026-09-02.md`](./TELEMETRY_ARCHITECTURE_2026-09-02.md). Ledger: [`ACTION_IDS_GTAD_2026-09-23.md`](./ACTION_IDS_GTAD_2026-09-23.md).  
...
The 2026-08-31 / 09-01 / 09-02 / 09-21 / 09-22 **fail-open increment sidecar** is **reaffirmed**. This run does **not** invent a second architecture. `main` has **not moved** since the 09-21 cron (`0f5363f`). EVENT_SOURCE line numbers below were **re-read** on this SHA (battle start **12097–12099**, recap **12517**, dual death **13261** vs **13316**, flee **18935**, GameKey helper **275–279**). They match 09-22.

This file patches three implementer hazards that 09-22 left stale:

1. **Open-PR queue grew again.** Oldest-first merge order is still **#327** then **#331**, then **#333–#446** — **116** still-open PRs targeting `main` (createdAt ascending, GitHub list 2026-09-23 00:03 UTC). Unmerged GTAD designs are **#352** (09-21) and **#422** (09-22). Do not hitchhike telemetry maps, bindgen, or sidecar call sites onto any of them. Union README index rows; unique 09-23 files must not overwrite #352/#422 unique files. Pre-existing queue breaks (#389 WX, #392 ChatPanel, later combat/map PRs) are not this PR’s unique delta.
2. **Queued persist PRs still are not EVENT_SOURCE.** 09-22 deferred Q-018 against **#375 / #377 / #387 / #391**. Those remain open. Additional persist PRs opened after the 09-22 cron: **#426** (skipBeforeEach death recut), **#431** / **#439** (unseeded one-shot / victory HUD Doka), **#435** (flushed GameKey/feat HUD vs first hydrate), **#445** (respawn HP after unpaid-death remount). On **main**, `noteUnconfirmedCredit` is still only the one-shot settle path (`dokaPersist.ts` **217–239**). Do not invent spend-note writers. Do not treat unmerged branch helpers as live increment sites.
3. **Recap dismiss five-path list is still correct** (re-read `PostBattleRecap.tsx`): Escape **67–68**, backdrop **93–96**, overlay Enter/Space **98–111**, close **208**, Continue **609**. Increment **once**.

`GTAD-2026-09-01-001`…`014` remain the Phase 0/1 implementation tickets. Use **this file’s `EVENT_SOURCE` line numbers**. New IDs this run are stack-fence / extra persist-deferral only.

Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

---

## 1. What changed since 2026-09-22

| Topic | 2026-09-22 (PR #422, SHA `0f5363f`) | 2026-09-23 (this cron, same SHA) |
| :--- | :--- | :--- |
...
| Open sibling PRs | **79** open through **#409** | **116** open through **#446**. Oldest still **#327** then **#331**. GTAD siblings **#352** + **#422**. TBC WAITING **#333/#395**. Dashboard **#345/#424**. |
| Persist PRs (not EVENT_SOURCE) | #356/#375/#377/#385/#387/#388/#391 | Those still open, **plus** #426/#431/#435/#439/#445. Q-018 still deferred. |
| Recap dismiss | Five `onClose` sites | **Re-verified.** Same five paths. |
...
| EOP | New later file after frozen `20260901`; `check-limit` 5→6 | Unchanged. `.old` stays Caffeine Aug-31 (no GameKey). |
| Dual death | Two entries | Unchanged. `_handlePlayerDeath` **13261**; HP-watch **13316–13407** still does **not** call it. Flee **18935** does. |
| Family / summoner | `spawnPolicy` 30% + live summoner roll | Unchanged. `applyFamilyVariantsToRoster` **289–296**; overwrite `sp`/`sr`/`init`/`res`/`chc` **11873–11903**; summoner **11932–11943**. |
| `WorldExploration.tsx` | 19,213 lines | **19,213**. `setInBattle(true)` / `BATTLE_START` **12097–12099**. Safety 2s **12167–12170**. |
| Admin tabs | No intelligence / Health | Unchanged (`gameTypes.ts` **483–498**; labels **5611–5625**). Purchases = GameKey inbox. |
| `APP_VERSION` | v163 | Still **v163** (`App.tsx` **14**). |
...
10. Hitchhike onto still-open **#327** / **#331** or any later still-open persist/combat PR (**#340**, **#356**, **#375**, **#377**, **#379**, **#387**, **#389**, **#391**, **#426**, **#431**, **#435**, **#439**, **#445**, …). Dedicated human-approved PR after those land, or a human-unioned restack. Unique 09-23 files must not overwrite unmerged **#352** / **#422**.
...
### 3.4 Recap dismiss correction (still 2026-09-22; re-verified 2026-09-23)
...
**Motoko / EOP note:** add telemetry maps in a **new** `src/backend/migrations/YYYYMMDD_HHMMSS.mo` that sorts **after** `20260901_000000`. `OldActor = {}`. `NewActor` = empty maps + zero epoch. Bump `mops.toml` `check-limit` to **6**. Run `python3 scripts/check-eop-stables.py` and `bash scripts/caffeine-import-gate.sh backend` (check-stable vs `.old` = Caffeine Aug-31 signature **and** every `snapshots/deployed/*.most` + `empty-canister.most`; `unsupported/` must still fail). Do not land this in the same PR as open **#327** / **#331** or later persist/combat PRs. Do not restack unique 09-23 docs by concatenating a second copy of a helper into a sibling file.
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; rows marked **NEW 2026-09-01**, **NEW 2026-09-02**, **NEW 2026-09-21**, **NEW 2026-09-22**, or **NEW 2026-09-23** are additive. This run adds **no new metric IDs** — only queue/persist-deferral evidence.
...
PURPOSE: NEW 2026-09-22; **deferral expanded 2026-09-23**. Open persist PRs (#375 keep unconfirmed Doka through spends, #377 GameKey/claim throw-after-credit, #387 rename/upgrade throw-after-debit, #391 unpaid death before GameKey lock credit) would add `noteUnconfirmedCredit` / spend-note calls after the canister already mutated. After 09-22: **#426** skipBeforeEach death recut, **#431/#439** unseeded HUD Doka, **#435** flushed GameKey/feat vs first hydrate, **#445** respawn HP after unpaid-death remount. On **main today**, `noteUnconfirmedCredit` is already used from one-shot settle (`dokaPersist.ts` **217–239**). That live path is Q-015 keep + Q-016 skip — **do not double-count it here**.  
EVENT_SOURCE: **None on `main` beyond Q-015/Q-016.** After those PRs merge: fire-and-forget **after** the note helper returns, off the lock. Enum only: `credit_throw|spend_throw`. Do not invent the helpers in the telemetry PR. Do not treat unmerged branch line numbers as EVENT_SOURCE.  
...
| Charts for `worldFeatures` / formations / discovery / Enemy Register as if live | Catalog-only or lore. N-007 / N-008. |
...
| Editing frozen `NewActor`s or hitchhiking telemetry onto #327 / #331 / later persist-combat PRs (now through **#446**) | EOP + stack-compat. Overwriting #352/#422 unique files is also forbidden. |
...
| Implementing Q-018 against unmerged #375/#377/#387/#391/#426/#431/#435/#439/#445 | EVENT_SOURCE must be on `main`. |
...
12. After older queue items merge, **re-read EVENT_SOURCE line numbers** before implementing. This catalog is valid at `0f5363f` only. The 116-PR queue will move WX/persist lines the moment **#327** / **#331** land.
...
| Balance / Content Analyst (`2786666f`) | Stay `WAITING_FOR_TELEMETRY`. | Use Phase 2 dimensions; never invent OVERPERFORMING from source. Enemy Register is lore (N-008). |
...
| Expansion / discovery / formations / world-events | Design docs stay design. | Add metric keys only when a persist/event source exists. E-008 summoner roll is live; `elite_patrol` and Enemy Register are not. |

Related: `AQA-2026-08-30-012` (still NEW; shop site patched in 09-21). `GTAD-2026-09-01-*` remain the implementation tickets; **read this file for line numbers, GameKey helper, one-shot, recap-dismiss, EOP, and the 116-PR stack fence.** `GTAD-2026-09-02-*`, `GTAD-2026-09-21-*` (PR #352), and `GTAD-2026-09-22-*` (PR #422) are still NEW additive deltas. `GTAD-2026-09-23-*` are this run’s deltas only.
