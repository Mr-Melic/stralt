# Emergent Build & Meta Analysis — 2026-09-26
...
**Prior unmerged passes (same HEAD, draft PRs only):** 2026-09-21 `#365`, 2026-09-22 `#414`, 2026-09-23 `#456`, 2026-09-24 `#522`, 2026-09-25 `#584`  
...
ACTION_IDs: [`ACTION_IDS_EBMA_2026-09-26.md`](./ACTION_IDS_EBMA_2026-09-26.md).
...
New this pass (queued after `#584`, still not on `main`): duration-1 AP/MP never touches the live pool (**018**), control-mode summon walk/cast ignores Slow/Haste (**019**), Sentinel kit Shield stamps the caster (**020**).

---

## 0. What changed since 2026-09-25 / last merge

`origin/main` is still `0f5363f`. Combat gates cited on 2026-09-25 still match. This pass re-read the live files and the queued combat-restore PRs opened **after** `#584`.
...
| **020** | `#597` Sentinel Shield on the clicked ally summon | `resolveSpellCast` stamps `targetId: caster.id` |

Combat-rule-parity occupant tickets (`#601`, `#607`) are **not** combo balance. Do not open EBMA IDs that those PRs already own.
...
| Duration-1 AP/MP | `WX` `applyActiveEffect` 1871–1922 vs walk/cast `currentBattleMp`; turn start `processActiveEffects` 1927–2017 **then** restore 14350–14365 | Haste (+2 MP, duration 1) never bumps the live pool. Next player turn ticks the row to 0 before restore. Swift Boots **does** bump (`WX` 3579–3581). Enemy Drain Courage AP −1 (duration 1) expires the same way |
...
**RECOMMENDED_RESPONSE:** Same-source refresh or a small per-target / per-name cap. Keep Poison + Venom + Inferno as three different types that can coexist. Do not flatten DoT identity. Do not nerf Timestep because a restored once-per-battle dump is still one extra recast. See `EBMA-2026-09-26-001`.
...
**RECOMMENDED_RESPONSE:** Player alive-cap (2–3) and a short summon-spell cooldown. Keep five identities. See `EBMA-2026-09-26-002`. Do not also give AI summons ticking DoTs until 001 lands (`006`). Do not close ally-buff targeting as a substitute for a cap. Do not treat 019/020 as a flood nerf — those restore advertised kit identity.
...
**LIVE SEQUENCE:** Drain heals call `onPlayerHealed` with the actual `healAmt` (`WX` 9502–9508). Flag flips. **005 stays closed.** Wisp cannot currently restore player HP (three independent kills: enemy-only control target, `buffStat` return before `healAmount`, recorded delta 0). That is 015, a restore, not a no-heal exploit.
...
**COMBO_SEQUENCE (controlled Archer):** Player-controlled Archer casts Slow through `resolveSpellCast` 529–548. The MP debuff **does** land on the effect list. It does **not** reduce the victim summon’s walk/cast pool (019). AI Archer Slow still goes through `executeSummonAction` `applyEffect` without `stat`/`modifier` (cosmetic).

**ACCESS_REQUIREMENTS:** Starters. 8-slot opportunity cost on the bar; Archer kit is a summon slot.

**WHY_IT_IS_STRONG:** The player bar is not. The cards advertise control that the player path cannot deliver. The **intended** Slow identity currently lives only on a controlled Archer’s effect list, which is interesting synergy and should be kept if 002 caps the flood — and 019 should make that Slow actually shorten walks.
...
**LIVE SEQUENCE:** `buildEnemyKit(..., currentMap.levelZone)` stays zone 0. Bishops have Frost only. Multiple Frosts **replace** by `effectName`. Live denial is −1 MP for 1 turn **on the effect list**. Duration-1 Frost is the same tick-before-restore class as 018, so even that −1 often never reaches `currentBattleMp`. Enemy AI Slow from an Archer summoner is cosmetic.

**ACCESS_REQUIREMENTS:** Live: any bishop pack. Latent: zone ≥ 1 kits + enemy Archer summon + 018/019 live-pool wiring.

**WHY_IT_IS_STRONG (latent):** Player mobility is the positional game. −3 MP on a 4-MP pool is near-root.

**COUNTERPLAY (latent):** Kill the bishop / archer; Haste (+2 MP, 1 turn, after 018); Timestep (once, after 017); Null Field.

**RECOMMENDED_RESPONSE:** Do **not** nerf Frost or Slow. Pass a numeric zone into `buildEnemyKit` (`009`). If that ships, the same-stat AP/MP stack cap in 004 covers both sides. 018 must land before duration-1 Frost is treated as real counterplay.
...
**COMBO_SEQUENCE:** Both modifiers roll independently. Sacrifice uses `dealDamage` → `enemyTakesDamage` → `applyDamageDealt` (`WX` 3499). Main-bar nukes skip that hook.
...
**RECOMMENDED_RESPONSE:** Monitor. Do not nerf Sacrifice or the modifiers unless play data shows every Glass+Titan map is a skip-or-Sacrifice binary. See `008`. Occupant-required Sacrifice (`#607`) is CRP, not a number change.
...
**COMBO_SEQUENCE:** Enrage → Mark tile → nuke (Shadow Strike 35 fits the leftover 3 AP). Ally-Enrage on a Wolf is a second payload on the summon’s turn. Enrage is `buffStat: "dmg"` duration 2, so `getStatModifier` sees it on the **damage** path immediately — unlike Haste, it does not depend on the AP/MP pool.
...
**RECOMMENDED_RESPONSE:** Preserve. Do not touch Mark, Enrage, or player-bar ally-buff targeting because the product is large.

---

### 3.10 Shield + Iron Skin (+ Sentinel / ally target) — **STRONG_BUT_HEALTHY** (player bar) / **UNDERPOWERED** (Sentinel kit)

**COMPONENTS:** Both `buffStat: "res"`, `buffModifier: 1.3`, different `effectName` → multiplicative 1.69× RES for 3 turns. Player Shield/Iron Skin **do** land on an allied summon via `resolvePlayerCast` 676–718. Sentinel kit Shield/Iron Skin go through `resolveSpellCast`, which stamps `targetId: caster.id` (507–526), so the clicked Wolf never gets the 1.3×.

**WHY_IT_IS_STRONG (player bar):** Durable, not immortal. Costs 5 AP. Null Field suppresses. Duration is short.

**WHY_IT_IS_WEAK (kit):** A Sentinel that spends 2 AP on Shield buffs itself, not the body it was summoned to guard. That is advertised guardian identity failing, not a nerf candidate.

**RECOMMENDED_RESPONSE:** Preserve player-bar 1.69×. Restore kit targeting (`020` / `#597`). Do not close ally targeting to “fix” 3.2.

---

### 3.11 Timestep + Haste + Rally — **UNDERPOWERED** (Timestep/Mirror/Haste inert) / **STRONG_BUT_HEALTHY** after 017+018

**COMPONENTS:** Timestep (once, 0 AP, `self`+`buff`, no `buffStat`) + Mirror (`self`+`defense`+`isMirror`) + Haste (+2 MP, duration 1) + Rally / Blood Mend (heal + advertised CHC).

**LIVE SEQUENCE:** Highlight already allows the caster tile. `isShieldSpell` swallows Timestep. Mirror’s `activateMirror` is inside the enemy-target loop. Haste writes an `ActiveEffect` and returns `"cast"`; walk/cast spend `currentBattleMp`, which is unchanged. Next player turn `processActiveEffects` expires the duration-1 row **before** restore 14350–14365. Swift Boots is the existence proof that same-turn MP bumps are intended (`WX` 3579–3581). Blood Mend / Rally CHC never reaches the crit roll (`012`).

**WHY_IT_IS_STRONG (intended):** One extra full bar per fight is a real decision, not a loop. Combined with 3.1 it is an amplifier (one extra Poison dump); the loop is still the missing DoT cap, not Timestep. Haste this-turn is a 2-AP walk extension, not a second Timestep.

**RECOMMENDED_RESPONSE:** Restore Timestep and Mirror (`017`). Restore Haste onto the live MP pool (`018`). **Do not land a second copy if `#496` / `#596` merge.** Do not nerf Timestep after restore. Optionally wire CHC (`012`). Do not use inert Timestep as a `hard_3` brake (`016`).
...
### 3.15 Ally-buff summons (Enrage / Shield / Haste on kits) — **STRONG_BUT_HEALTHY** (player-bar Enrage/Shield) / **UNDERPOWERED** (Haste + kit Shield)

**COMPONENTS:** `resolvePlayerCast` ally branch (676–718) + Enrage / Shield / Iron Skin / Haste + any player-side summon.

**LIVE SEQUENCE:** Player-bar Enrage and Shield **do** land on the clicked ally (dmg/res are read from `getStatModifier` on the damage path). Player-bar Haste lands on the effect list and does not extend that summon’s walk (019) or the player’s walk if self-cast (018). Sentinel kit Shield does not land on the clicked ally (020).

**RECOMMENDED_RESPONSE:** Preserve player-bar Enrage/Shield on kits. Restore Haste onto the live pool (018/019). Restore kit targeting (020). Do not close ally targeting to “fix” 3.2.
...
**LIVE SEQUENCE:** Control-cast always passes `targetEnemy` (`WX` 9849–9869). `resolveSpellCast` takes `buffStat` and returns before `healAmount`. `ctx.heal` only mutates character HP for player ids. Recorded no-heal amount is 0.
...
### 3.19 Duration-1 AP/MP never touches the live pool — **UNDERPOWERED** (restore) — *new this pass*

**COMPONENTS:** Haste (`buffStat: "mp"`, `buffModifier: 2`, `buffDuration: 1`, `spellData.ts` 314–331) + enemy Drain Courage (`debuffStat: "ap"`, −1, duration 1) + player turn-start tick-then-restore. Contrast: Swift Boots (`WX` 3579–3581) and Enrage (dmg, duration 2).

**COMBO_SEQUENCE:**

1. Cast Haste on self (2 AP via the shield/ally branch). An `ActiveEffect` is written. `currentBattleMp` is unchanged, so leftover walk MP is still the unbuffed 4.
2. End the turn. Next player turn: `processActiveEffects("player")` (`WX` 14275) runs `tickNonDotEffects` (1997–2017) and drops duration 1 → 0 **before** restore 14350–14365 reads `getStatModifier("player", "mp")`.
3. Enemy Drain Courage (if an enemy actually applies it) is the same class: duration 1 expires at the start of the player turn the copy says it should reduce.

**ACCESS_REQUIREMENTS:** Starter Haste. Drain Courage is a starter; player-bar Drain Courage currently does not write `debuffStat` (004), so the live hole is enemy-cast Drain Courage and player Haste.

**WHY_IT_IS_STRONG:** It is not. The card advertises “+2 MP for 1 turn.” The live pool never sees it. 09-25 classified ally Haste as STRONG_BUT_HEALTHY from metadata; the queued `#596` (opened after `#584`) is the existence proof that walk/cast ignore the effect list.

**COUNTERPLAY:** N/A (the player is the one missing the tool).

**RELATIVE_PROGRESSION_CONTEXT:** 2 AP spent for flavor text. Swift Boots (BuffShop) is the only honest same-turn MP bump.

**PERSISTENCE/ECONOMY_IMPACT:** None. This is a restore of advertised identity, not a nerf. After restore, Haste is a 2-AP walk extension — STRONG_BUT_HEALTHY, not a second Timestep. Do not nerf Haste numbers.

**RECOMMENDED_RESPONSE:** Bump the live AP/MP pool when a duration-1 additive resource effect is applied (Haste this turn; Drain Courage leftover AP next turn if tick order stays). Keep Swift Boots as the item analog. **Do not land a second copy if `#596` merges** — treat that PR as the player-Haste vehicle, then confirm duration-1 **AP debuffs** still reduce the next player AP pool. See `EBMA-2026-09-26-018`.

---

### 3.20 Slow / Haste vs summon walk-cast pool — **UNDERPOWERED** (restore) — *new this pass*

**COMPONENTS:** Control-mode `summonTurnBudget` (`summonControlCast.ts` 356–363) + Archer Slow (live on the effect list via `resolveSpellCast`) + player-bar ally Haste on a Wolf.

**COMBO_SEQUENCE:**

1. Controlled Archer Slow hits a hostile summon or the player-side Wolf is Slowed. `getStatModifier` is −2.
2. That unit’s next control-mode turn sets `currentMp = maxMp` (`WX` 14520–14529) and walks the full budget.
3. Ally Haste (duration 1) on a Wolf expires in `processActiveEffects` before the budget, so the advertised +2 MP never arrives. `#596` only bumps the **player** pool.

**ACCESS_REQUIREMENTS:** Starter Archer + Haste + any player-side summon.

**WHY_IT_IS_STRONG:** It is not. 3.5’s “controlled Archer Slow is live” is only half-true: the badge is real; the walk is not. This is the positional half of the control kit.

**COUNTERPLAY:** N/A until restored. After restore, kill the Archer / wait out duration 2.

**RELATIVE_PROGRESSION_CONTEXT:** Same as 3.5. Do not nerf Slow −2.

**PERSISTENCE/ECONOMY_IMPACT:** None.

**RECOMMENDED_RESPONSE:** Feed remaining AP/MP modifiers into the control-mode budget (same semantics as player restore). Duration-1 ally Haste on a summon must survive long enough to grant one budget. **Do not land a second copy if `#598` merges.** See `EBMA-2026-09-26-019`. Pair with 004 (player-bar Slow still does not write the effect) and 018 (player pool).

---

### 3.21 Sentinel kit Shield on the clicked ally — **UNDERPOWERED** (restore) — *new this pass*

**COMPONENTS:** Sentinel `summonKit: ["starter-shield", "spell-iron-skin"]` (`spellData.ts` 593) + `resolveSpellCast` buff branch (`spellEngine.ts` 507–526) + control-cast always passing `targetEnemy` (`WX` 9849–9869).

**COMBO_SEQUENCE:** Click an allied Wolf, spend Sentinel AP on Shield. RES 1.3× is written on the **Sentinel**. `getStatModifier(wolfId, "res")` stays 1. Self-click and Blood Mend stay on the caster by design.

**ACCESS_REQUIREMENTS:** Starter Sentinel. Player-bar Shield on the same Wolf **does** work (`resolvePlayerCast` 676–718) — this hole is kit-path only.

**WHY_IT_IS_STRONG:** It is not. Guardian identity is the interesting synergy; the kit currently cannot perform it.

**COUNTERPLAY:** N/A.

**RELATIVE_PROGRESSION_CONTEXT:** Sentinel spell-level buys HP/AP that still cannot guard an ally.

**PERSISTENCE/ECONOMY_IMPACT:** None.

**RECOMMENDED_RESPONSE:** Stamp `targetId` from the clicked ally on kit buffs whose `targetType` is `"ally"`. Keep self-click and heal-on-caster. **Do not land a second copy if `#597` merges.** Not `#550` (heal-after-buff). See `EBMA-2026-09-26-020`. Do not treat this as a flood nerf.
...
| Shield + Iron Skin (player bar / ally) | STRONG_BUT_HEALTHY | No |
| Ally Enrage on kits (player bar) | STRONG_BUT_HEALTHY | No |
...
| Controlled Archer Poison | STRONG_BUT_HEALTHY | Keep; cap DoT (001) |
...
| Haste this-turn / duration-1 AP-MP | UNDERPOWERED | Restore live pool (`#596`) |
| Slow/Haste vs summon walk pool | UNDERPOWERED | Restore budget (`#598`) |
| Sentinel kit Shield on ally | UNDERPOWERED | Restore kit target (`#597`) |
...
- Mark, Enrage, Timestep-once **identity**, Mirror identity, Barrier geometry, Swap’s teleport identity, player-bar ally-buff targeting.
- Haste +2 / Slow −2 **numbers**. Restore the pool; do not shrink the card.
...
| — | **New 018** | Duration-1 AP/MP live pool; `#596` is the Haste vehicle |
| — | **New 019** | Summon walk/cast pool; `#598` is the vehicle |
| — | **New 020** | Sentinel kit ally target; `#597` is the vehicle |

Mending Mist on the player is still unwired (`playerTurnStartModifierTarget` → `undefined`). Do not open a ticket that `#443` already owns.
...
| Permanent control | Player-bar denial dead; latent after 009+018 |
| AP/MP denial chains | Latent 3.6; live Archer Slow badge without walk (3.20); duration-1 resource effects inert (3.19) |
...
| Defensive immortality | Shield+Iron Skin duration-gated (healthy on player bar); Sentinel kit Shield mis-targeted; Mirror inert until 017 |
