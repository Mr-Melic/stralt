...
WorldExploration React pass (086–093): report-only; no WX/RAF edits.
Implemented this increment: PERF-2026-09-23-094, 095, 096.
Reported only: 097–101.
...
ChatPanel 096 hunk is disjoint from #350 (poll / width drag) and #392 (focus / click-trace / blob revoke).
...
| ID | Issue | Impact | Autonomy | Status |
|---|---|---|---|---|
| 086 | filteredBackendSpells → ownedSpells churn every setState | CPU / frame pacing / mobile | SAFE | NEW |
| 087 | canAttackNearest LoS IIFE every render | CPU / input latency / mobile | HUMAN | NEW |
| 088 | spellCooldowns Object.fromEntries every render | CPU / GC | SAFE | NEW |
| 089 | Ungated console.log on click/victory/battle-start | CPU / input latency / mobile | SAFE | NEW |
| 090 | cleanupMap misses mark/range/loot-claim/coin state | Memory / correctness bleed | HUMAN | NEW |
| 091 | Dead cameraVersion setState on portal | CPU on map transition | SAFE | NEW |
| 092 | isDesktop resize undebounced setState | CPU on resize / mobile rotate | SAFE | NEW |
| 093 | normalizedSpellPool map every render | CPU / GC | SAFE | NEW |
| 094 | useIsMobile resize setState without equality | CPU / frame pacing / mobile | SAFE | IMPLEMENTED |
| 095 | App profile query focus refetch | CPU on tab return | SAFE | IMPLEMENTED |
| 096 | ChatPanel copies debug ring on mount | CPU / memory | SAFE | IMPLEMENTED |
| 097 | Battle HUD caret infinite CSS + drop-shadow | compositor during combat | HUMAN | NEW |
| 098 | Barrier tower 18 save/restore per tile per frame | CPU / frame pacing | HUMAN | NEW |
| 099 | Spellbook kit `allSpells.find` per card | CPU while modal open | SAFE | NEW |
| 100 | Spellbook range preview unmemoized 9×9 grid | CPU while modal open | SAFE | NEW |
| 101 | Query `withTimeout` dangling 10s timers | memory (small) | SAFE | NEW |

---

ACTION_ID: PERF-2026-09-23-094
TITLE: Skip useIsMobile setState when resize does not cross the breakpoint
CATEGORY: runtime-performance
PRIORITY: high
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/hooks/use-mobile.tsx; src/frontend/src/engine/viewportActivity.ts; src/frontend/src/components/WorldExploration.tsx (consumer)
CURRENT_BEHAVIOUR: `useIsMobile` started as `false` then setState on mount, and every `resize` called `setIsMobile(width < 768)` even when the boolean was unchanged. Mobile URL-bar chrome fires resize often; WorldExploration is the live consumer, so those events re-rendered the ~19k-line world tree under the canvas RAF.
DESIRED_BEHAVIOUR: Seed from `window.innerWidth`. Commit React state only when the boolean flips. Distinct from PERF-092 (WX `isDesktop` > 1024 listener).
EVIDENCE: use-mobile.tsx `setIsMobile(window.innerWidth < breakpoint)` with no equality guard; WorldExploration.tsx calls `useIsMobile()`.
RECOMMENDED_ACTION: Keep `shouldCommitIsMobile`. Do not change the 768px Continue warning. Optional follow-up: same equality guard on WX `isDesktop` (092).
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Rotation that crosses 768 must still flip camera/zoom. Starting `true` on a phone avoids one extra mount render rather than changing layout rules.
VALIDATION_REQUIRED: node --experimental-strip-types --test src/frontend/src/engine/viewportActivity.test.ts; typecheck; narrow viewport still uses mobile camera; dragging a desktop window that stays >768 does not re-render the world tree.
STATUS: IMPLEMENTED

---

ACTION_ID: PERF-2026-09-23-095
TITLE: Stop App-level profile query from refetching on window focus
CATEGORY: runtime-performance
PRIORITY: medium
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/hooks/useCharacterQueries.ts (useGetCallerUserProfile); src/frontend/src/App.tsx
CURRENT_BEHAVIOUR: App always mounts `useGetCallerUserProfile` (`staleTime: 30000`, default `refetchOnWindowFocus`). After 30s, alt-tab back into a live world decoded the profile on the main thread and re-rendered App → GameFlow. WorldExploration is memo'd and does not take the profile object, but Candid + GameFlow reconcile still ran under the canvas. Distinct from PERF-061 (WX catalogs), PERF-009 (Doka), PERF-070 (global QueryClient default), PERF-085 (leaderboard).
DESIRED_BEHAVIOUR: `refetchOnWindowFocus: false` on this query. `saveCallerUserProfile` still invalidates. First mount / remount still loads.
EVIDENCE: useCharacterQueries.ts staleTime 30000 without focus gate; App.tsx always calls the hook.
RECOMMENDED_ACTION: Keep the per-query flag. Do not change persist-lock or Doka hydrate.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Profile edits in another tab lag until remount or mutation invalidate (same class as catalog 061).
VALIDATION_REQUIRED: Typecheck; enter world; alt-tab after 30s does not hitch; saving a profile still refreshes the header name.
STATUS: IMPLEMENTED

---

ACTION_ID: PERF-2026-09-23-096
TITLE: Do not copy the 2000-line debug ring into ChatPanel React state on mount
CATEGORY: runtime-performance
PRIORITY: high
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/components/ChatPanel.tsx (debug subscribe effect)
CURRENT_BEHAVIOUR: PERF-003 gated *live* subscribe updates on `activeChannel === "debug"`, but the same effect still `setDebugEntries([...getDebugLogBuffer()])` on mount. After a long session the module ring holds up to 2000 entries; re-entering the world copied them into React state. `filteredDebugEntries` then filtered that array on every ChatPanel render (battle-log props) even if Debug was never opened. Distinct from 003 (subscriber gate), 077 (prod buffer writes), 081 (folded JSX trees), 084 (click-trace tick).
DESIRED_BEHAVIOUR: Keep subscribe. Snapshot the buffer only when switching to the Debug channel (existing effect). Mount leaves `debugEntries` as `[]`.
EVIDENCE: ChatPanel.tsx subscribe effect called `setDebugEntries([...getDebugLogBuffer()])` before `subscribeDebugLogs`. Channel effect already snaps when `activeChannel === "debug"`.
RECOMMENDED_ACTION: Keep the channel snap. Do not drop Export (reads `getDebugLogBuffer()` directly).
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Opening Debug must still show history from before the tab was selected. Unread on Debug is already 0.
VALIDATION_REQUIRED: Typecheck; play then open Debug — history is present; folded chat during battle does not allocate a 2000-row filter; Export .txt still works.
STATUS: IMPLEMENTED

---

ACTION_ID: PERF-2026-09-23-097
TITLE: Battle HUD caret runs infinite CSS bounce plus drop-shadow during combat
CATEGORY: runtime-performance
PRIORITY: low
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/index.css (.stone-battle-caret); src/frontend/src/components/BattleUIPanel.tsx
CURRENT_BEHAVIOUR: The active-turn caret uses `animation: bounceCaret 1s ease-in-out infinite` and `filter: drop-shadow(...)`. BattleUIPanel is mounted for the whole world session (068); the caret is visible whenever `isActive`. Continuous compositor work plus a filter on the live HUD, concurrent with the world canvas. Distinct from landing `chessDrift` (072).
DESIRED_BEHAVIOUR: Pause while `document.hidden` / `prefers-reduced-motion`, or a static marker; keep turn readability.
EVIDENCE: index.css `.stone-battle-caret`; BattleUIPanel `{isActive && <div className="stone-battle-caret" />}`.
RECOMMENDED_ACTION: Visual-approved substitute. index.css is also in #339 / #423 — restack if implementing.
AUTONOMY:
- HUMAN_APPROVAL_REQUIRED
REGRESSION_RISK: Active-turn affordance is a combat readability cue.
VALIDATION_REQUIRED: Side-by-side battle HUD; reduced-motion; hidden tab compositor cost.
STATUS: NEW

---

ACTION_ID: PERF-2026-09-23-098
...
CURRENT_BEHAVIOUR: Each barrier tile stacks 6 layers × 3 faces. Every face does `ctx.save` / fill / optional dim fill with a fresh rgba string / stroke / `restore`. With several barriers this is dozens of state flips per RAF on the world canvas. Distinct from PERF-035 (entity foot shadows), PERF-066 (pixel bake), PERF-022 (barrier Set rebuild).
DESIRED_BEHAVIOUR: Hoist dim fillStyles, reduce save/restore, or blit a cached tower ImageBitmap keyed by tile size/DPR.
EVIDENCE: drawBarrierLayer three save/restore blocks; tower loop BARRIER_LAYERS = 6.
...
ACTION_ID: PERF-2026-09-23-099
TITLE: Spellbook summon cards scan the full catalog per kit id on every render
CATEGORY: runtime-performance
PRIORITY: low
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/components/SpellbookModal.tsx (SummonAbilitiesBlock ~261–264)
CURRENT_BEHAVIOUR: Each summon card maps `summonKit` ids through `allSpells.find`. Open spellbook over the live world; parent re-renders redo O(catalog × kit) scans per visible summon. Distinct from PERF-023 (battle SummonControlPanel IIFE).
DESIRED_BEHAVIOUR: Memo kit resolution keyed on `allSpells` identity + kit ids + spell level, or a Map index built once per catalog.
EVIDENCE: SpellbookModal.tsx `kitIds.map((id) => allSpells.find((s) => s.id === id))`.
RECOMMENDED_ACTION: Extract a helper. SpellbookModal is in #441/#425/#389/#372 — union, do not overwrite.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Wrong kit after catalog invalidation or upgrade if the memo key omits spellLevels.
VALIDATION_REQUIRED: Open spellbook on a summon; kit names/AP match live spawn; upgrade a summon spell; preview stats update.
STATUS: NEW

---

ACTION_ID: PERF-2026-09-23-100
TITLE: Spellbook range preview rebuilds a 9×9 tile grid and targeting Set on every parent render
CATEGORY: runtime-performance
PRIORITY: low
CONFIDENCE: high
FILES_OR_SYSTEMS: src/frontend/src/components/SpellbookModal.tsx (computePreviewCells, RangePatternGrid)
CURRENT_BEHAVIOUR: `RangePatternGrid` calls `computePreviewCells` during render: allocates a 9×9 floor grid, empty enemy list, barrier Map, then `computeTargetableTiles`. Selected-spell preview is open over the world; WX re-renders redo that work. Distinct from 076 (world floor string keys).
DESIRED_BEHAVIOUR: `useMemo` keyed on spell id / range / targeting fields; keep geometry identical to live `computeTargetableTiles`.
EVIDENCE: RangePatternGrid `const hitSet = computePreviewCells(spell)` with no memo.
RECOMMENDED_ACTION: Memoize. Do not change advertised range vs live gates (#441).
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Preview cells can desync from canvas range if memo misses aoe/hitTiles.
VALIDATION_REQUIRED: Select several spell types (self, enemy, aoe); preview matches in-world highlights.
STATUS: NEW

---

ACTION_ID: PERF-2026-09-23-101
TITLE: Query withTimeout leaves a 10s timer running after the actor resolves
CATEGORY: runtime-performance
PRIORITY: low
CONFIDENCE: medium
FILES_OR_SYSTEMS: src/frontend/src/hooks/useCharacterQueries.ts; useAdminQueries.ts; useSpellQueries.ts; useShopQueries.ts (`withTimeout`)
CURRENT_BEHAVIOUR: `Promise.race` against `setTimeout(..., 10000)` never clears the timer when the actor wins. Each catalog/profile fetch leaves a timer that rejects a dangling promise at 10s. Not a large leak, but world entry fires several queries.
DESIRED_BEHAVIOUR: `clearTimeout` when the actor promise settles. Shared helper so the four copies stay in sync.
EVIDENCE: Identical `withTimeout` in four hook files; no clearTimeout.
RECOMMENDED_ACTION: Extract one helper. useAdminQueries / useSpellQueries are in older PRs — union if implementing.
AUTONOMY:
- SAFE_TO_AUTO_IMPLEMENT
REGRESSION_RISK: Timeouts must still reject hung replica calls so UI does not spin forever.
VALIDATION_REQUIRED: Slow-network fixture or fake timer: resolved queries do not reject later; hung queries still time out at 10s.
STATUS: NEW
