# Enemy & Boss Admin — 2026-09-25 re-audit

**Author:** Enemy & Boss Admin Content Designer  
**Automation:** `299b70f5-a498-11f1-a7d1-d6b4613131ce`  
**Verified against:** `origin/main` @ `0f5363f` (merge of PR #332; **same SHA as the 2026-09-21 through 2026-09-24 EBA re-audits**)  
...
**Prior ledgers not yet on `main`:** [`ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-21.md`](https://github.com/Mr-Melic/stralt/pull/360) (`EBA-2026-09-21-001`) on open draft PR [#360](https://github.com/Mr-Melic/stralt/pull/360); zero-ID ledgers on [#404](https://github.com/Mr-Melic/stralt/pull/404) (09-22), [#449](https://github.com/Mr-Melic/stralt/pull/449) (09-23), and [#507](https://github.com/Mr-Melic/stralt/pull/507) (09-24)  
**This run:** documentation only. No production, RAF, map generation, turn, or damage-math code. This PR does **not** edit the 08-31 contract file (PR #360 owns that 09-21 pointer). Unique `*2026-09-25*` filenames only.
...
`origin/main` has not moved since the 2026-09-21 EBA re-audit (`0f5363f`). Spot-checks on 2026-09-25 match the 09-21 / 09-22 / 09-23 / 09-24 line numbers. The 2026-08-31 design remains the owner-studio contract.
...
**No new ACTION_IDs this run.** Do not re-file `EBA-2026-08-31-*`, `EBA-2026-09-01-*`, `EBA-2026-09-02-*`, `EBA-2026-09-21-001` (still covered by open PR #360), or the zero-ID 09-22 / 09-23 / 09-24 ledgers (PRs #404 / #449 / #507). Consume `SDA-*`, `VAL-*`, `AFDA-002` / `011`, `WDEAD-*`, `AEE-*`, Enemy Elite Evolution, Enemy Formations / EBMA.

Open-PR queue at audit time: **226** still-open PRs targeting `main` (GitHub search `is:pr is:open base:main`). Oldest `createdAt` first: [#327](https://github.com/Mr-Melic/stralt/pull/327), [#331](https://github.com/Mr-Melic/stralt/pull/331), then the 2026-09-21 wave starting at [#333](https://github.com/Mr-Melic/stralt/pull/333). EBA siblings already in that queue: [#360](https://github.com/Mr-Melic/stralt/pull/360), [#404](https://github.com/Mr-Melic/stralt/pull/404), [#449](https://github.com/Mr-Melic/stralt/pull/449), [#507](https://github.com/Mr-Melic/stralt/pull/507). This run adds unique `docs/automation/*2026-09-25*` files so it stays merge-clean after those older siblings.

---

## 1. What still drives live encounters (re-verified 2026-09-25)
...
| Spawn roster | Chess piece + `pickEnemyLevelFromTiers` (WX 5770–5772) + placeholder `level*8+20` HP, then `applyFamilyVariantsToRoster`. Tables live in `spawnPolicy.ts` (`FAMILY_VARIANT_CHANCE = 0.3` at 35; `DUNGEON_SPAWN_DEPTH_CAP = 5` at 29). | EBA-001, 007, 008; **EBA-2026-09-21-001** (PR #360) |
| Closed level bands | `newEnemy()` / `newRegion()` default `levelMax` to **9999** (`AdminDashboard.tsx` 137–157). Editor Level Min/Max with `ELIGIBILITY_BAND_HINT` (248–249, rendered at 813): “New drafts default max to 9999 so high-level play still matches.” `AdminGuard` **requires** the closed band (`adminGuard.mo` 273–277, 297–301). Live region match is still `level <= Number(r.levelMax)`. | EBA-002; 09-01-001; 09-02-001; consume WDEAD |
...
| Boss scaling | Spawn `level + 5` and `Math.min(50, res/sp)`. `getBossEffectiveStats` is Boss Guide + `longHorizonSim` only. | EBA-011 |
...
| AI inference | `inferArchetype` (`enemyAI.ts`): healer from heal flags, flanker from `pieceType === "knight"`, berserker from `family.includes("berserk")`. | EBA-009; consume AEE |
...
| Portal assignments | Canister `setBossPortalAssignment` exists. Frontend `useSetBossPortalAssignment` is a no-op invalidate. World picks a random `BOSS_IDS` entry. | EBA-023 |
| Hard delete | `adminDeleteEnemyConfig` is `Map.remove` (`main.mo` 798–805). No `adminRollbackEnemyConfig` / `adminRollbackBossConfig`. Bosses tab holds unused `_deleteBossConfig`. | EBA-021; 09-01-003; 018 |
...
Same as 09-21 through 09-24, still true on this SHA:

- `generateEnemies` `tryPlaceEnemy` still calls `computeEnemyStats(enemyLevel, pieceType, …)` where `pieceType` is the **player** chess piece. The spawned unit stores `pieceType: randomPieceType`. Battle start reseeds with `e.pieceType`. When EBA-001 / 09-21-001 bind `EnemyDefinition.pieceType` through `spawnPolicy`, pass the **unit** piece. Do not retune damage math.
...
- AP/MP ≤ 20 remains a **combat-budget** clamp (`adminGuard.mo` 270–271). `initStat > 100` (272) and enemy HP 1–100000 (267–268) are document-size rails. `summonUnitDef.level > 99` is Motoko **and** `adminSafety.ts`; `getSummonBaseStats` scales from spellLevel + hpScale, not `unitDef.level` (**09-02-002 OPEN**). Do not raise 99 → 999.
- Tiers tab `SAMPLE_LEVELS = [1, 10, 25, 50, 100, 200, 500]` — consume WDEAD. Death Realm fallbacks still stamp `maxLevel: 5`. Consume WDEAD; do not “fix” kit width by reading that object (EBA-013).
- `computeAITier` still bands through 900 then a flat tier 10 (`combatMath.ts` 36–47). Sophistication may plateau; **rolled enemy level must not** (EBA-003). Mixin `admin-api.mo` still writes via `AdminLib.setEnemyConfig` with **no** `AdminGuard` and **no** audit; `main.mo` does not include the mixin (**09-01-005 OPEN**). `longHorizonSim.ts` still has a private `buildEnemyKit` (**09-01-004 OPEN**).
...
| 002 `summonUnitDef.level ≤ 99` is a template-size rail | OPEN — Motoko **and** `adminSafety.ts`. Do not raise 99 → 999. |
...
### EBA-2026-09-22 / EBA-2026-09-23 / EBA-2026-09-24

No IDs. Ledgers on [PR #404](https://github.com/Mr-Melic/stralt/pull/404), [PR #449](https://github.com/Mr-Melic/stralt/pull/449), and [PR #507](https://github.com/Mr-Melic/stralt/pull/507). Do not re-file empty records for those dates.
...
**None.** Ledger: [`ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-25.md`](./ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-25.md).
