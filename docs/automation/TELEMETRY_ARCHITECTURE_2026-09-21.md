# Gameplay telemetry architecture — 2026-09-21
...
**Prior design:** [`TELEMETRY_ARCHITECTURE_2026-09-02.md`](./TELEMETRY_ARCHITECTURE_2026-09-02.md). Ledger: [`ACTION_IDS_GTAD_2026-09-21.md`](./ACTION_IDS_GTAD_2026-09-21.md).  
...
`src/` hits remain: an empty WorldExploration intent comment, `longHorizonSim.telemetry.available = false`, and an optional combatant `intelligence` field (AI flavour, **not** an admin tab). No `recordTelemetryIncrements`. No `telemetrySidecar`. No `telemetryLifetime` map.

The 2026-08-31 / 09-01 / 09-02 **fail-open increment sidecar** is **reaffirmed**. This run does **not** invent a second architecture. It patches evidence after **158 commits** since `58302bc` and corrects four implementer hazards:

1. **GameKey commit helper moved.** Paid Doka still credits only via `redeemGameKey` / `redeemGameKeyThroughPersist`. The increment predicate is now `shouldCommitGameKeyRedeem(walletSeeded, granted)` (`shopPurchase.ts` **275–279**), **not** `shouldCommitShopCredit` (legacy remount / `processPendingPurchases` path). A Phase 1 hook that still keys off shop-credit helper names will miss every real mint.
2. **One-shot transport-keep is a live persist class.** `settleOneShotAfterCredit` (`dokaPersist.ts` **144–152**) returns `commit|release|keep`. Seeded keep can confirm via `confirmKeptOneShotCredit` (**173–192**). Absolute `saveBattleStats` must skip a stale pre-credit snapshot (`shouldSkipAbsoluteDokaWrite`, `progressPersist.ts` **202–212**). Quality series Q-015 / Q-016; never a second mint writer.
3. **EOP fence replaced the #259 hitchhike warning.** GameKey stables are **frozen** on `20260901_000000`. Open PR **#259 is gone**. New `telemetryLifetime` / `telemetryDay` maps need a **new later** chain file after `20260901` (`20260921+`, `OldActor = {}`), `check-limit` bumped from **5 → 6**, `python3 scripts/check-eop-stables.py`, and `bash scripts/caffeine-import-gate.sh backend`. Never edit a shipped `NewActor`. Never blank or hand-write `.old` (Caffeine-owned Aug-31 signature, **no GameKey**).
4. **Dual death is still two entries.** `_handlePlayerDeath` (`WX` **13261**) comment claims HP-watch routes through it. The HP-watch effect (**13316+**) still calls `persistDeathPenalty` itself and **does not** call `_handlePlayerDeath`. Tag both. Do not “unify” death math to make a metric easier.

`GTAD-2026-09-01-001`…`014` remain the Phase 0/1 implementation tickets. Use **this file’s `EVENT_SOURCE` line numbers**, not 09-01 or 09-02. New IDs this run are GameKey-helper / one-shot / EOP / dual-death / stack-fence deltas only.

Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

---

## 1. What changed since 2026-09-02

| Topic | 2026-09-02 (`58302bc`) | 2026-09-21 (`0f5363f`) |
| :--- | :--- | :--- |
| Production telemetry | None | **Still none.** Same two `src/` hits + combatant `intelligence?: number` (`gameTypes.ts` **307**). |
| `WorldExploration.tsx` | 19,253 lines; battle start **12212** | **19,213** lines. `setInBattle(true)` / `BATTLE_START` **12097–12099**. Safety 2s `setInBattle(true)` **12167–12170**. |
| Victory recap-then-persist | Recap **12645**; catch **12739** | Recap **12517–12519**; catch **12611–12616**. Computation-error recap **12629–12632**. |
| Boss Rush persist catch | **12990** | **12863–12868** (`"BossRush reward persist failed"`). Room-clear persist **12813**. |
| `_handlePlayerDeath` | **13360**. Comment listed HP-watch as a routed caller | **13261**. Comment still lists HP-watch. **HP-watch still does not call it** (**13316–13407**). Function now also `resetRunState` + `cleanupBattle` + Game Over. |
| Flee | **18976** → death | **18918–18935** → `_handlePlayerDeath`. Run confirm unchanged. |
| Intent comment | **16530** (empty) | **16442–16446** (empty). **Do not persist `action.intent` strings.** |
| Family 30% roll | Inline `WX` **5940** | Extracted: `applyFamilyVariantsToRoster` (`spawnPolicy.ts` **289–296**, `FAMILY_VARIANT_CHANCE = 0.3` **35**). WX call **5862–5866**. Battle start still overwrites `sp`/`sr`/`init`/`res`/`chc` via `computeEnemyStats` (**11873–11903**). |
| Shop mint | `shouldCommitShopCredit` after redeem | Redeem still `DokaGameKeyShop` → `redeemGameKeyThroughPersist` (**184** / `shopPurchase.ts` **294**). Commit = **`shouldCommitGameKeyRedeem`**. Remount still calls no-op `creditPendingPurchasesThroughPersist` (**1494**). `processPendingPurchases` still returns **0** (`main.mo` **1338–1348**). |
| One-shot credits | Claim + keep-on-transport mentioned | Live settle machine: `settleOneShotAfterCredit` / `confirmKeptOneShotCredit` / `resolveOneShotCreditSettle` / `shouldSkipAbsoluteDokaWrite`. Persist lock tracks `unconfirmedWalletCredit` + `beforeEach` unpaid-death flush. |
| EOP / GameKey maps | Open PR #259 owned GameKey stables | **`20260901_000000` frozen** (GameKey maps). `check-limit = 5`. `.old` = Caffeine Aug-31, 42 stables, **no GameKey**. Telemetry maps = **new later file**, not an edit of 20260831/20260901. |
| Open sibling PRs | #259 GameKey EOP | **#327** (Striker AoE; WX + challenge + targeting) then **#331** (portal destack; WX + mapGen + persist). Do not hitchhike telemetry onto those. |
| `ownedSpells` | Catalog grant `WX` **2373** | Still catalog grant **2412–2440**; `shouldIncludeBackendSpellInLibrary` now `adminSafety.ts` **712–719**. Not discovery. |
| Recap dismiss | Escape **68** / backdrop **95** / Enter **99** | Escape **67–68**, backdrop **111**, close button **208**, Continue **609**. Dedup once per recap. |
| Identifiable dumps | Leaderboard, OQL (email), `getAllCharacters` | Same. `getAllCharacters` **532**. `getLeaderboard` **3390**. OQL `include Expose` **3498**; `gameKeyRequests.email` **3788**. AdminDashboard still unused for `getAllCharacters`. |
| Enemy summoner roll | `WX` **12047** | **11932–11943**. Constants unchanged (`gameConstants.ts` **298–299**). |
| Unpaid death replay | `flushPendingDeathPenalty` **572** | Still **572–589**. Persist lock may `beforeEach` this flush — increment **after** it returns, off the lock. |
| World features / formations / observe→win | Not wired | Still **not** imported by `WorldExploration.tsx`. `worldFeatures.ts` tests only. |
| Enemy Register | Not called out | Flavor lore (`enemyRegisterCopy.ts`). **Not** an encounter or bestiary series (N-008). |
| Admin tabs | No intelligence / Health | Unchanged (`gameTypes.ts` **483–498**). Purchases = `AdminGameKeyPurchases`. |
| `APP_VERSION` | v163 | Still **v163**. |
| `CharacterStats` | `main.mo` **144–157** | **147–160**. Still 12 fields. Telemetry must not ride `updateCharacter`. |
| `getAdminAuditLog` | Bound; no dashboard caller | Bound `backend.ts` **839 / 2435**; `main.mo` **3486**. Still no AdminDashboard caller. Ops ring, not gameplay. |

Merged since `58302bc`: persist/combat/map/admin-safety/security PRs, GameKey EOP on `20260901`, stack-compat CI already on main. **No increment sidecar shipped.** Prior telemetry PRs remain docs only.
...
| Boss Rush catch **12863** | `"BossRush reward persist failed"`. | Same pattern. Recap is built after the fire-and-forget persist (**12875+**). |
...
| `chatMessages` | In-memory, cap 200, dropped on upgrade. `sendMessage` binds `playerName` from `userProfiles`. | **Do not collect message content.** |
| Admin dashboard | Catalog CRUD + Purchases GameKey inbox. | Not battle outcomes. No `intelligence` tab. |
...
| Combatant `intelligence?` | `gameTypes.ts` **307**. WX spawn stub **16159** sets `0`. | AI flavour field. **Not** an analytics tab. |
...
| Recap dismiss | `PostBattleRecap` `onClose` (four UI paths; increment once) |
...
10. Hitchhike onto still-open **#327** / **#331** (WX, `mapGen`, `challengeCompletion`, `dokaPersist`, `progressPersist`).
...
### 3.2 Shop-credit correction (updated 2026-09-21)
...
### 3.3 One-shot / absolute-write correction (new this run)
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; rows marked **NEW 2026-09-01**, **NEW 2026-09-02**, or **NEW 2026-09-21** are additive.
...
EVENT_SOURCE: `handleBattleEnd(true)` after recap is queued (**12517**), **not** inside `resolveBattleRewards`. Boss Rush: after recap is shown in the room-clear path; “victory paid” still **after** persist returns (**12813** / catch **12863**).  
...
PURPOSE: NEW 2026-09-02. Battle start flags `isSummoner` with `ENEMY_SUMMONER_CHANCE_BASE + level * PER_LEVEL_ZONE` (`WX` **11932–11943**; `gameConstants.ts` **298–299**).  
...
PURPOSE: UX cancellation. Discrete handlers (`PostBattleRecap.tsx` Escape **67–68** / backdrop **111** / close **208** / Continue **609** → `onClose`).  
...
PURPOSE: Paid Doka actually landed. **Updated 2026-09-21:** increment predicate is `shouldCommitGameKeyRedeem`, not `shouldCommitShopCredit`.  
...
| Editing frozen `NewActor`s or hitchhiking telemetry onto #327 / #331 | EOP + stack-compat. |
...
9. Do not land telemetry stables in the same PR as open **#327** (Striker / WX / challenge) or **#331** (portal destack / WX / mapGen / persist) unless a human explicitly unions them.
...
Related: `AQA-2026-08-30-012` (still NEW; shop site patched here). `GTAD-2026-09-01-*` remain the implementation tickets; **read this file for line numbers, GameKey helper, one-shot, and EOP rules.** `GTAD-2026-09-02-*` are still NEW additive deltas. `GTAD-2026-09-21-*` are this run’s deltas only.
