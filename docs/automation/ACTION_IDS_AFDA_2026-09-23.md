# ACTION_IDs — 2026-09-23 Admin Feature & Drift Auditor

Durable ledger. Reuses AFDA-2026-08-31-* / AFDA-2026-09-01-020 / AFDA-2026-09-02-* / AFDA-2026-09-21-* / AFDA-2026-09-22-* for the same underlying problems.
...
HEAD `0f5363f` (origin/main). Open AdminDashboard siblings (oldest first): #334 Tiers leftover, #341/#413 built-in spell Retire, #415 computeAITier / Ground Doka / live feats / boss schema. This run does **not** restack those hunks. Unique honesty: Names live CatalogNote; Shop leftover ShopPackage sentence.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: Adapter still in `adminContract.ts` `toBackendSpellConfig` / `fromBackendSpellConfig`. Bindgen `SpellConfig` (`backend.ts` 118+) includes summon fields plus `multiTarget`, `hitsAllies`, `cooldown`. Combat still reads `hitsMultiple`. Frontend-only mechanic flags still drop (018). Spells CatalogNote `AdminDashboard.tsx` 3640–3645.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: `useBossQueries.ts` 1–31 and `useAdminQueries.ts` 508–557 still read/write `localStorage.pbv_boss_configs`. `WorldExploration.tsx` ~6486 loads the same key. `main.mo` still has `setBossConfig`, `deleteBossConfig`, `getAllBossConfigs`. Frontend `BossConfig` (`bossTypes.ts` 91–110) has `iconEmoji`, `loreText`, `chc`; Motoko (`admin.mo` 317–330) has `defeated`, `adminNotes`, no `chc`. Main Bosses intro still says “until a backend writer exists” (`AdminDashboard.tsx` ~7837–7839); honest schema copy is queued in #415. Canister seed uses retired spell ids (025).
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: Live player shop is GameKey (`DokaGameKeyShop.tsx`). Admin Shop CatalogNote this run states leftover `ShopPackage` CRUD / `initiatePurchase` (`AdminDashboard.tsx` ~6915–6921). Canister still has `adminSetShopPackage` / `adminDeleteShopPackage` / `getShopPackages` / `initiatePurchase`. Do not delete until a deployed DID prove-out.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: Admin writes `room_N_enabled` / `room_N_reward` to localStorage + `adminSetBossRushConfig`. `useBossRush.ts` parses `rewardMultiplier` into unused state. Rooms come from `BOSS_RUSH_ROOMS` (room 9 `weeping_pawn_2`). Main CatalogNote still says “only parsed.rewardMultiplier is read” (`AdminDashboard.tsx` ~7141–7146); unused-state honesty is queued in #334/#415.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: Admin still dual-writes `paperVertexPalette` and `pbv_color_palette` (`AdminDashboard.tsx` VisualsTab handleSave ~4733–4734). World hydrates `getColorPalette` into `pbv_color_palette`.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: `ownedSpells` hydrates via `shouldIncludeBackendSpellInLibrary` (`adminSafety.ts`: `usableByPlayer` or already owned). Still no `minLevel` check. Admin still edits `minLevel`. Spells CatalogNote `AdminDashboard.tsx` 3640–3645.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: `computeAITier` (`combatMath.ts` 36–51) maps enemy level onto hard bands 10/30/60/100/150/250/400/600/900 then randomly re-rolls 1–10 at 30%. WorldExploration assigns `aiTier` at spawn (~5823, ~5865). `ENEMY_KITS` stay code-owned. Tiers CatalogNote naming the bands is queued in #415. Not a player career cap.
...
PRIORITY: P3
CONFIDENCE: HIGH
EVIDENCE: `WorldExploration.tsx` 2170 hydrates `useGetEnemyNames`; generateEnemies shuffles that pool and assigns unique names per map (5753–5804), falling back to `DEFAULT_ANCIENT_NAMES` (5667). Enemies tab is labeled catalog-only (006). Names subtitle only said “max 1 per enemy per map”. This run added a CatalogNote (`AdminDashboard.tsx` names tab).
SYSTEMS_AFFECTED: Enemy Names tab; overworld spawn labels
CURRENT_BEHAVIOUR: Adding/deleting a name changes the next map’s labels. Empty pool uses the hardcoded fallback list.
