# ACTION_IDs — 2026-09-25 Admin Feature & Drift Auditor

Durable ledger. Reuses AFDA-2026-08-31-* / AFDA-2026-09-01-020 / AFDA-2026-09-02-* / AFDA-2026-09-21-* / AFDA-2026-09-22-* / AFDA-2026-09-23-* for the same underlying problems. New this run: AFDA-2026-09-25-030.
...
HEAD `0f5363f` (origin/main). Open AdminDashboard siblings (oldest first): #413 retired spells, #415 computeAITier / Ground Doka-on-Settings / live feats / boss schema / Tiers leftover, #457 Names live + Shop leftover ShopPackage, #470 name-pool / Boss Rush confirm, #531 Visuals unused palette, #539 live-publish confirms, #564 summon editor. This run does **not** restack those hunks. Unique honesty: Ground Doka live contract on the Map Modifiers panel; Ads live landing slots.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: Live player shop is GameKey (`DokaGameKeyShop.tsx`). Admin Shop CatalogNote on main still omits leftover `ShopPackage` CRUD / `initiatePurchase` / `setShopPaymentLink` (`AdminDashboard.tsx` 6926–6930); leftover sentence is queued in #457. Canister still has `adminSetShopPackage` / `getShopPackages` / `initiatePurchase` / `getShopPaymentLinks`. No frontend caller except bindgen/mocks. Do not delete until a deployed DID prove-out.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: `useGetEnemyConfigs` (`useSpellQueries.ts` 111) is admin-only. Spawn uses `pickEnemyLevelFromTiers` + `getEnemyBaseStats`. Admin `EnemyConfig` is hp/ap/mp/initStat/levelMin/levelMax/regions/spriteUrl — not `types/common.mo` combat template. WorldExploration has no `getEnemyConfigs` / `spriteUrl` reader. Enemies tab CatalogNote `AdminDashboard.tsx` 2117–2122. Enemy **names** from `getEnemyNames` ARE used at spawn (WX 2170) — see 029. Do not delete CRUD.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: Admin writes `room_N_enabled` / `room_N_reward` to localStorage `bossRushConfig` + `adminSetBossRushConfig`. `useBossRush.ts` only applies `parsed.rewardMultiplier` into unused `_rewardMultiplier`. Admin JSON never writes `rewardMultiplier`. Rooms come from `BOSS_RUSH_ROOMS` (room 9 `weeping_pawn_2`). Main CatalogNote still says “only parsed.rewardMultiplier is read” (`AdminDashboard.tsx` 7152–7157); unused-state honesty is queued in #415.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: Dropdown is `listAdminModifierTypeOptions()` plus `LEGACY_MODIFIER_TYPES` (`AdminDashboard.tsx` 4943–4956). Live registry still has 22 ids (`MAP_MODIFIERS` in `mapModifiers.ts` 152–471). Legacy `lava_fields` / `ice_fields` / `spike_pit` / `custom` remain selectable. Motoko `defaultMapModifiers` (`admin.mo` 136–154) still seeds only slime_flood / paper_windstorm.
...
PRIORITY: P1
CONFIDENCE: HIGH
EVIDENCE: Motoko `SpellConfig` (`admin.mo` 92–127) requires summon fields + `cooldown`. Generated `backend.ts` 118–152 includes that block. `getAdminAuditLog` exists on `main.mo` and `backend.ts`. Remaining: bindgen lists `isCallerAdmin` / `assignCallerUserRole` (`backend.ts` 709, 1042) which are absent from `main.mo`. README says do not hand-edit bindgen; regenerate with `pnpm bindgen`.
...
PRIORITY: P2
CONFIDENCE: HIGH
EVIDENCE: Canister default is 60/20/10/5 = 95 (`main.mo` 1710–1718). Motoko comment says values need not sum to 100 (`types/admin.mo` 156). `validateTierSpawnConfig` does not require sum 100 (`adminGuard.mo`). `pickEnemyLevelFromTiers` (`combatMath.ts` 72–98) spends same/adj/twoAway and puts leftover into ±3+; `threeOrMorePercent` is unused (`_threeMore`). `levelVarianceChance` is rolled at 15% and is not on Candid. Main Tiers tab still requires total === 100 (`AdminDashboard.tsx` 3834, 3842–3844, 3928). Honesty + leftover preview is queued in #415.
...
PRIORITY: P3
CONFIDENCE: HIGH
EVIDENCE: `WorldExploration.tsx` 2170 hydrates `useGetEnemyNames`; generateEnemies assigns unique names per map, falling back to `DEFAULT_ANCIENT_NAMES`. Enemies tab is labeled catalog-only (006). Names subtitle only said “max 1 per enemy per map” (`AdminDashboard.tsx` ~7329). Live CatalogNote is queued in #457.
...
PRIORITY: P3
CONFIDENCE: HIGH
EVIDENCE: `LandingPage.tsx` 279–296 hydrates `getAdBoxes` and renders slots when an image URL is set (~706–719). Admin Ads tab writes `adminSetAdBox` / `adminClearAdBox`. Help copy mentioned landing-page slots but did not name the live query. This run added a CatalogNote (`AdminDashboard.tsx` 6890–6894). Custom ad art is optional; play does not require ads.
SYSTEMS_AFFECTED: Admin Ads tab; landing page
CURRENT_BEHAVIOUR: Saving a box with both URLs appears on the landing page. Empty image+link stays hidden.
AUTHORITATIVE_BEHAVIOUR: Admin copy must say which catalogs the live client actually reads.
RECOMMENDED_ACTION: Keep CRUD. Do not require ad artwork. Do not treat unused empty slots as license to delete methods.
AUTONOMY: IMPLEMENT for copy (this run)
DEPENDENCIES: None
REGRESSION_RISK: LOW
