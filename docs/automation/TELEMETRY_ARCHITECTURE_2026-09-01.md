# Gameplay telemetry architecture — 2026-09-01

**Director:** Gameplay Telemetry Architecture Director  
**Automation:** `047ac8a1-a4a0-11f1-a7d1-d6b4613131ce` (cron `0 */72 * * *`)  
**HEAD inspected:** `dd275aa` (`Merge pull request #182 from Mr-Melic/stralt` — Caffeine import gates)  
**Prior design:** [`TELEMETRY_ARCHITECTURE_2026-08-31.md`](./TELEMETRY_ARCHITECTURE_2026-08-31.md) (PR #130, merged). Ledger: [`ACTION_IDS_GTAD_2026-09-01.md`](./ACTION_IDS_GTAD_2026-09-01.md).  
...
There is still **no analytics store, no increment API, and no admin Intelligence tab**.  
The 2026-08-31 sidecar design is **reaffirmed**. This run updates evidence (line numbers, new privacy hazards, AQA persist-lock wording) and adds a few metrics. It does **not** invent a second architecture.

`AQA-2026-08-30-012`, `GTAD-2026-08-31-001`…`011`, and the dashboard/balance WAITING reports are still unimplemented. Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

---

## 1. What changed since 2026-08-31

| Topic | 2026-08-31 | 2026-09-01 (`dd275aa`) |
| :--- | :--- | :--- |
| Production telemetry | None | **Still none.** Grep in `src/` hits a WX comment + `longHorizonSim.telemetry.available = false`. |
| `WorldExploration.tsx` | ~19.6k lines; battle start ~12363 | **20,063** lines. Battle start `setInBattle(true)` **12661**; `BATTLE_START` **12663**. |
| Victory recap-then-persist | ~12731 / catch ~12797 | Recap first **13009**; persist catch **13157–13162** (`"Reward persistence failed (non-blocking)"`). |
| `_handlePlayerDeath` | ~13321 | **13754**. Comment claims HP-watch also routes here; **lava/spike HP-watch (13809+) still does not call it.** |
| Flee | `onEndBattle` ~18871 | **19297–19314** → `_handlePlayerDeath` (run confirm unchanged). |
| Intent comment | ~16457 | **16877–16881** (empty block). |
| `ownedSpells` | starters ∪ all backend configs | Base ∪ backend rows passing `shouldIncludeBackendSpellInLibrary` (`adminSafety.ts` **311–317**). `usableByPlayer !== false` still grants the **entire player catalog**. Not discovery. |
| Recap dismiss | “defer if no handler” | `PostBattleRecap.tsx` has discrete `onClose` (Escape **72**, backdrop **93**, Enter/Space **99**, Continue buttons). **Q-005 is now instrumentable.** |
| Identifiable dump | Leaderboard + OQL | **Plus** `getAllCharacters` (`main.mo` **558–563**): `#admin` returns `[(Principal, CharacterSlots)]` including **names**. AdminDashboard does **not** call it. Intelligence must never download this and aggregate in the browser. |
| AQA-012 wording | Asked for counters | Still says they “must enqueue on `createProgressPersist` or be query-only.” **That enqueue clause is wrong** and would regress the wallet lock. Query-only snapshots OK. Increments **after** persist returns, **off** the lock. |
| `EnemyFamily` | Mentioned as pieceType | Closed enum on overworld roster; 30% variant roll (`WX` **6447–6537**). Battle start spreads `family` and keeps family HP/damage; **overwrites `res`/`sp`/`init`/`chc` via `computeEnemyStats` (12456–12467)**. Count family encounters; do not treat family W/L as proof those overwritten stats are live. |
| World features / elites / formations / observe→win discovery | Design docs | Still **not wired**. `worldFeatures.ts` has no production importer. Do not emit `elite_patrol` as if it fires. |
| Admin tabs | Config CRUD | Unchanged (`gameTypes.ts` **483–498**). No `intelligence` / Health. |
| Import gate | Informal | `pnpm typecheck` + `pnpm check`; Motoko changes require `mops check` / `caffeine check` (`.github/workflows/caffeine-import-gate.yml`). |
| `APP_VERSION` | v163 | Still **v163**. |
| Leaderboard / OQL / chat | ~2527 / ~2620 / ~1741 | `getLeaderboard` **3005**; OQL `Expose` **~3110**; `chatMessages` **2245**. |

Merged since prior HEAD `22503b5`: combat/map/economy/CI PRs (#171–#182). **No increment sidecar shipped.** Prior telemetry PRs (#130 architecture, #119 dashboard, #122 balance WAITING) are docs only.
...
| :--- | :--- | :--- |
| `src/frontend/src/debug/debugLogger.ts` | Local ring buffer (2000). Shift+D. Console dev-only. | Session-local. Not aggregated. |
| `debug/clickTrace.ts` + `debugExport.ts` | Click-geometry / PDF. | Dev-only. Never a production funnel. |
| `WX` **16877–16881** | Empty “caller-side telemetry” around `action.intent`. | No emit. |
| `WX` **419+**, **12613**, **2511** | `[SPELLBAR-BISECT]` counters. | Local debug throttle. |
| `data/pieceArt.ts` **40–55**, **864+** | Throttled `logPatternLookupFailed` → `king.front`. | Local `SUMMON` log. |
| `handleBattleEnd` catch **13157** | `"Reward persistence failed (non-blocking)"`. Recap already shown. | Fail-open **pattern**, not a counter. |
| `getLeaderboard` (`main.mo` **3005**) | Top 50: `principalId`, `playerName`, `level`, `killCount`, `achievementsCompleted`. | Identifiable game feature. **Do not reuse as Intelligence.** |
| `getAllCharacters` (`main.mo` **558**) | Full slots + names + principals. Unused by AdminDashboard. | Identifiable dump. **Phase 0 must scan server-side and return buckets only.** |
| OQL `Expose` (`main.mo` **~3110**) | `characterSlots` / `dokaBalances` / `userProfiles` / `dungeonRecords` are `ownedBy("owner")`. | Controller can list principals. **Do not add a telemetry entity with an owner column.** |
| `PurchaseRecord` (`types/admin.mo` **202–217**) | Name, email, address, postal, proof URL. | Payment ops only. **Never join into gameplay analytics.** |
| `chatMessages` (`main.mo` **2245**) | In-memory, cap 200, dropped on upgrade. | **Do not collect message content.** |
| Admin dashboard | Enemies, Regions, Sprites, Visuals, Spells, Settings, Tiers, Modifiers, Purchases, Achievements, Names, Bosses, Ads, Shop, Boss Rush. | Catalog counts only. |
| `useSaveKillCount` | Hook in `useLeaderboardQueries.ts` **43**. | **Zero TSX callers.** `killCount` is not battle count. |
| `longHorizonSim.ts` **432–436** | `telemetry.available: false`. | Correct. Do not calibrate from source. |
| Quality audit `AQA-2026-08-30-012` | persist-ok/fail, victory paid, death-penalty, recap open/dismiss, shop credit. | Still unimplemented. |
...
| :--- | :--- |
| Battle start | `WX` **12661–12663** `setInBattle(true)` + `onDebugLog("BATTLE_START")` |
| Victory | `handleBattleEnd(true)` **12826**; recap first **13009**; persist in `try/catch` **13157** |
| Defeat (non-death) | `handleBattleEnd(false)` ~13164 (rare) |
| Combat / DoT / flee death | `_handlePlayerDeath` **13754**. Flee **19297** calls this. |
| Lava / spike death | HP-watch **13809+** — still **does not** call `_handlePlayerDeath` |
...
| Dungeon step | `snapshotDungeonChain` **before** `cleanupMap` (**6961**), then `decideDungeonChainPortal` (**6966**) |
...
| Family variant | `generateEnemies` 30% roll **6525–6537**; `family` spread at battle start |
| Pattern fallback | `drawCombatant` → `king.front` + throttle |
| Recap dismiss | `PostBattleRecap` `onClose` |
| Reward persist fail | catch **13157** (non-blocking) |
...
7. Download `getAllCharacters` / OQL owner rows / `getLeaderboard` into the Intelligence UI and “aggregate in the client.”
...
### 3.1 Correcting `AQA-2026-08-30-012`
...
| Swallow increment failures | Block `applyRewards` / `saveBattleStats` if telemetry fails |

The persist-lock warning in AQA was about **not inventing a second wallet path**. Increments are not wallet writes. Putting them on the lock can stall or reorder `applyRewards` vs `saveBattleStats` — the class AQA already feared.
...
### 4.1 Why not OQL / leaderboard / getAllCharacters / debug export

- **OQL** returns `owner` principals. Using it as analytics is identifiable surveillance.
- **`getLeaderboard`** is a player-facing top-50 with `principalId`.
- **`getAllCharacters`** is an admin dump of every name and slot. Intelligence aggregation must happen **inside** an `#admin` query that returns histograms only.
...
Client: call **after** recap is shown / death penalty applied / persist `try` settled. Spell **ids**, not names. Coarse buckets for HP, turns, level delta.
...
**Key cardinality budget:** catalog-sized dimensions only — `spellId`, `pieceType`, `EnemyFamily` enum, `bossId`, `challengeId`, `modifierType`, `deathCause` enum, `achievementId`. No free-text. No map ids that include player names.
...
| :--- | :--- | :--- |
| **0 — Snapshot aggregates** | `#admin` queries that scan existing maps and return **buckets only**. No combat writes. | Immediate population signal. Zero persist-lock risk. |
| **1 — Outcome + quality increments** | Sidecar + `recordTelemetryIncrements` + persist ok/fail, victory paid, death penalty, recap open/**dismiss**, shop credit, battles started / victory / defeat / flee. | Answers AQA-012. Fail-open. |
| **2 — Combat / spell / enemy / content dimensions** | Turns, remain-HP, death cause, family W/L, level-delta, spell unique-set, dungeon/rush/challenge/modifier. | Balancing. |
...
3. OQL `characterSlots.owner` / `userProfiles.name`
4. `PurchaseRecord` customer fields
5. Admin ban list (ops, not Intelligence)
6. Debug overlay buffer (local)
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; new rows are marked **NEW 2026-09-01**.
...
EVENT_SOURCE: `WorldExploration.tsx` **12661–12663** after `setInBattle(true)` / `onDebugLog("BATTLE_START")`. One increment per successful battle commit, including Boss Rush rooms.  
...
EVENT_SOURCE: `handleBattleEnd(true)` after recap is queued (**13009**), **not** inside `resolveBattleRewards`. Boss Rush: `handleBossRushRoomClear` after recap.  
...
EVENT_SOURCE: `_handlePlayerDeath` **13754** when `inBattleRef` is true **and** the caller is not the flee button.  
AGGREGATION: Counter; dim `mode`.  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Combat difficulty vs hazard difficulty (see C-007).  
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low.  
IMPLEMENTATION_COMPLEXITY: Medium — must tag the caller (flee vs combat vs DoT) without changing death math. The linchpin comment is slightly wrong: HP-watch is still a second entry.  
STATUS: PROPOSED  

METRIC_ID: GTAD-C-004  
NAME: Flees  
PURPOSE: Distinguish “I quit this fight” from “I died”. Flee **is** death (`onEndBattle` **19297** → `_handlePlayerDeath`).  
...
EVENT_SOURCE: Tag both entry points. Flee **19297**; lava/spike HP-watch **13809**; combat/DoT via `_handlePlayerDeath` callers. Enum only: `combat_melee|combat_spell|dot|lava|spikes|flee|other`.  
...
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Enemy / kit / tier-spawn edits. **Caveat:** battle start overwrites family `res`/`sp`/`init`/`chc` (`WX` **12456–12467**). Do not treat family W/L as proof those overwritten rows are live.  
...
EVENT_SOURCE: `handleBattleEnd` generation-guard / `deathTriggered` early return (**12840**); `cleanupBattle` while `inBattleRef` without an outcome increment; persist catch that still showed a recap.  
...
PURPOSE: There is **no** persisted spell-drop / first-seen store. `ownedSpells` (`WX` **2410–2438**) = starters ∪ backend rows with `usableByPlayer !== false` (or already on `spellLevelKeys` / `spellBarOrder`). `minLevel` is catalog metadata only. SDA-002 (owned/observed ids) is design-only.  
...
EVENT_SOURCE: WX `action.kind === "cast"` after **16885**; boss AI casts. Unique-set per battle.  
...
PURPOSE: NEW 2026-09-01. `shouldIncludeBackendSpellInLibrary` hides `usableByPlayer === false` unless already on keys/bar.  
...
PURPOSE: NEW 2026-09-01. The 30% `EnemyFamily` overlay (`WX` **6525–6537**) is a content knob.  
...
PURPOSE: Population shape without identities.  
EVENT_SOURCE: **Phase 0 admin snapshot** over `characterSlots` (occupied slots). Buckets `1|2_4|5_9|10_14|15_19|20_plus`. Recommend **all occupied slots**.  
...
EVENT_SOURCE: `applyRewards` `#ok` sites + `claimAchievementReward` + `processPendingPurchases` (**packageId / amount only**).  
...
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low if shop increments carry no customer fields.  
IMPLEMENTATION_COMPLEXITY: Low.  
STATUS: PROPOSED  

METRIC_ID: GTAD-P-005  
NAME: Doka spent (coarse)  
PURPOSE: Sink health. Spends are `upgradeSpell`, `saveBattleStats` (heal/shop/death), `renameCharacter`.  
...
EVENT_SOURCE: Phase 0 scan of `achievementProgress` grouped by `achievementId`: unlocked, claimed, unlocked-unclaimed.  
...
PURPOSE: NEW 2026-09-01. Quality Auditor could not estimate player count.  
...
EVENT_SOURCE: `decideDungeonChainPortal` after snapshot **6961–6968**; abort via `resetRunState` on death.  
...
EVENT_SOURCE: `persistBossRushRoomClear` success; `resetBossRush` / `abortBossRush`; `bossRushMasterComplete`.  
...
PURPOSE: NEW 2026-09-01. `engine/worldFeatures.ts` lists `elite_patrol` and peers. **No production importer.** `WORLD_DYNAMICS.md` is not wired into map gen. Formations / observe→win discovery are design-only.  
...
EVENT_SOURCE: After `readApplyRewardsOk` throws / `#err` — **outside** the persist-lock `enqueue` (existing `catch` **13157**).  
...
EVENT_SOURCE: `#ok` from `applyRewards` / `saveBattleStats` / `upgradeSpell` / `claimAchievementReward` / `processPendingPurchases` — after commit, off the lock.  
...
EVENT_SOURCE: `onShowBattleSummary` success (**13009** / room clear).  
...
PURPOSE: UX cancellation. **Now has discrete handlers** (`PostBattleRecap.tsx` Escape / backdrop / Enter / Continue → `onClose`).  
...
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low.  
IMPLEMENTATION_COMPLEXITY: Low (was “defer”; now Phase 1).  
STATUS: PROPOSED  
...
PURPOSE: Admin-configured URLs (player sprites, enemy `spriteUrl`, ad boxes). World draw is still pixel patterns — **do not count empty URL as CUSTOM_FALLBACK** (see dashboard design).  
EVENT_SOURCE: `onerror` only when a **non-empty** URL fails. **Do not send the URL.** Send `kind=enemy_sprite|player_sprite|ad_box`.  
...
PURPOSE: `drawCombatant` falls back to `king.front` (`pieceArt.ts` **40–55**, **864+**). This is missing `pieceType`/palette — a content bug — not a custom-URL fail.  
...
PURPOSE: NEW 2026-09-01. Telemetry health, not gameplay health.  
...
| Intelligence UI over `getLeaderboard`, OQL owner rows, or `getAllCharacters` | Identifiable. |
| Joining `PurchaseRecord` PII into gameplay charts | Different purpose. |
...
| Building spell-drop / observe→win **for** analytics | Inventing gameplay for a metric (SDA/SDE remain product decisions). |
...
- New tab key `intelligence` on `AdminDashboardState.tab` (lazy-loaded with the rest).
- **Backend `#admin` on every read.**
- Carved-stone / dark slate / crimson (`DESIGN.md`).
- Sections: Population (P-001/P-008/P-009), Economy (P-004/P-005), Combat (C-*), Spells (S-*), Content (N-*), Quality (Q-*).
- Empty state: “No increment sidecar yet — snapshot queries only” until Phase 1. **Never** show `0` battles as if measured.
- No principal search. No “inspect this player.”
...
2. Client sidecar **outside** `WorldExploration.tsx` (now **20,063** lines). Flush from `rewardResolver`, `deathPenalty`, `bossRushProgress`, `challengeCompletion`, `shopPurchase`, `spellUpgrade` **after they return**.
...
4. Bindgen after Candid change (`pnpm bindgen`). Telemetry must not ride on `updateCharacter`. `CharacterStats` stays 12 fields.
5. Import gate: `pnpm typecheck` && `pnpm check`. Motoko / mocks / `.old` → `mops check` or `caffeine check`. Unused locals and hook-deps are **errors**.
6. Do not modify RAF, map generation, turn logic, or damage math to “make metrics easier.”
7. Do not call `getAllCharacters` from the Intelligence tab.
...
| :--- | :--- | :--- |
| Quality Auditor (`976261d8`) | INCONCLUSIVE. Do **not** require persist-lock enqueue. | Cite `quality.persist.*`, `combat.outcome.*`, `quality.death_penalty.*`, `quality.recap.*`. Missing increment ≠ regression. |
...
| Dashboard Designer (`4b026695`) | Snapshot panels + honest “not measured.” | Bind to `adminGetTelemetrySnapshot` / progression snapshot only. |
| `longHorizonSim` | Keep `telemetry.available = false`. | Flip only when increment/snapshot APIs exist **and** are populated. |
| Expansion / discovery / formations / world-events | Design docs stay design. | Add metric keys only when a persist/event source exists. |

Related: `AQA-2026-08-30-012` (still NEW). `GTAD-2026-08-31-*` remain valid policy; **use `GTAD-2026-09-01-*` for implementation** (updated evidence + AQA correction + `getAllCharacters` fence).
