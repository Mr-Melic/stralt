# ACTION_IDs — Content Diversity & Repetition Auditor 2026-09-21

Durable ledger for implementers and the Report Action Orchestrator.  
Source of every record: Content Diversity & Repetition Auditor.  
Audit: [`CONTENT_DIVERSITY_AUDIT_2026-09-21.md`](./CONTENT_DIVERSITY_AUDIT_2026-09-21.md).  
HEAD: `0f5363f`. Gameplay / production code was **not** modified this run.

Prior ledger [`ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md`](./ACTION_IDS_CONTENT_DIVERSITY_2026-09-02.md) (`CDA-2026-09-02-001` … `015`) **landed on `main`**. Do **not** mint twins. Line numbers below replace the 09-02 citations. Do not mint twins of SDE / WDEAD / formation / family-sheet ids — those remain the implementer contract; CDA ids are the diversity gate.

Do not implement combat from this file unless a later human or orchestrator picks an ID.

---

## Status of CDA-2026-09-02-001 … 015

| ACTION_ID | STATUS | Note |
| :--- | :--- | :--- |
| CDA-2026-09-02-001 | OPEN | Overlay now in `spawnPolicy.ts`; wipe unchanged |
| CDA-2026-09-02-002 | OPEN | Still NaN. **Must ship with 006** (queens + Blood Mend → healer) |
| CDA-2026-09-02-003 | OPEN | No elite type; `WF-ELT-*` still unwired |
...
| CDA-2026-09-02-011 | OPEN | Slime ≡ Frozen. Blood Moon / Mirror Field are **live** in `spellEngine` (not placeholders). Gravity / Fog still empty. Replace Blood Moon’s ×1.25 DAMAGE; do not re-implement Mirror Field as another reflect |
| CDA-2026-09-02-012 | OPEN | Frontend 32 vs backend `defaultSpells()` six + ghost boss seeds |
| CDA-2026-09-02-013 | PARTIAL | Register chrome = FLAVOR LORE (`9e28cf9`). `EnemyFamily` union still seven + `default`. Per-row copy still lies — see CDA-2026-09-21-016 |
| CDA-2026-09-02-014 | OPEN | Dungeon extras still `[0,2,3,4,4,5]`; 52 `WF-*` still unused |
| CDA-2026-09-02-015 | OPEN | `applyPushback` / `applyAttract` still tests-only. Prefer POSITIONING here; REACTION is now a reflect cluster (CDA-2026-09-21-017) |

Reaffirmations (updated evidence; **not** new ids):
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Live placement is 4-quadrant + Chebyshev ≥ 4 (`WorldExploration.tsx` 5744–5763) then battle-start re-scatter ≥ 2 (`11869–11886`). `docs/design/ENEMY_FORMATIONS_2026-09-02.md` and `worldFeatures.ts` encounter slots are unwired. No `formationId` in spawn.  
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: Enemy Register chrome is FLAVOR LORE (`enemyRegisterCopy.ts` 6–15; `9e28cf9`). Rows still list Crimson Spawn / Shadow Lurker / Storm Caller (`EnemyRegister.tsx` 71–88) that are not `EnemyFamily`. Wave 1+2+3 sheets already propose 22+14+ ids. Shipping another sticker repeats CDA-001’s failure at larger scale.  
SYSTEMS_AFFECTED: `EnemyRegister.tsx`; `gameTypes.ts` EnemyFamily; elite evolution docs  
RECOMMENDED_ACTION: Implementers must not add `EnemyFamily` members or Register monsters until CDA-001 is live. Wave-2/3 families stay docs. NEW PLAYER DECISION: none until 001 exists — this id prevents a fake decision (“new name, same Strike pawn”).  
AUTONOMY: HUMAN_APPROVE  
...
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: WDEAD-2026-08-31-005 / 007; ENC-PROT-01/02; CDA-2026-09-02-009 if a challenge overlay is added; CDA-2026-09-21-018 freeze on more WF rows  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `applyPushback` / `applyAttract` (`occupancy.ts` 482, 537) have tests and no spell caller. Live POSITIONING is Swap only. REACTION is no longer empty: Mirror spell + Void 25% + Mirror Field 20% + boss Reflect Shield 30%. SPELL_PROPOSALS Wave 1 already reserved push, pull, delayed execute, root, real trap, absorb, redirect.  
...
## New this run (do not twin 001–015)
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Chrome is honest (`enemyRegisterCopy.ts` 11–15: “Flavor only — not the live spawn roster”). The seven live-family rows (`EnemyRegister.tsx` 28–70) still teach wall-phase, poison-on-hit, burning tiles, adjacent slow + regen, Weaken-from-range, magic immunity until physical. Live verbs are only ember melee burn, tide melee MP−1 (WX 16789–16821), void 25% reflect (`castHelpers.ts` 335–345). Crimson / Lurker / Storm (71–88) are not `EnemyFamily`.  
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: `worldFeatures.ts` holds **52** `WF-*` ids (Wave 1–3). Grep: imported only by `worldFeatures.test.ts`. WX does not import the module. 09-02 counted ~20 rows; Wave 2+3 added more unused catalog. CDA-014 already asks to wire **one** object. Another design wave would repeat CDA-013’s sticker flock on the encounter axis.  
SYSTEMS_AFFECTED: `engine/worldFeatures.ts`; WDD / ENC design docs; dungeon `generateEnemies`  
RECOMMENDED_ACTION: Implementers must not append `WF-*` rows until CDA-014 (or equivalent) imports one existing id (`WF-ELT-TOLL_KEEPER` or `WF-ZON-SHRINE_POOL` preferred). Wave-4 world-dynamics catalogs stay docs without new TypeScript rows. NEW PLAYER DECISION: none until one object is live — this id prevents a fake decision (“new shrine name, same kill-all pack”).  
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: CDA-2026-09-02-014 is the gate; do not invent a 53rd id to “teach” hold/escort  
