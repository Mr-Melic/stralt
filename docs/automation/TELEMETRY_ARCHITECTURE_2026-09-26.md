# Gameplay telemetry architecture — 2026-09-26
...
**Prior designs (unmerged, same SHA):** PRs **#352** (09-21), **#422** (09-22), **#450** (09-23), **#527** (09-24), **#583** (09-25). On `main` the last merged design is [`TELEMETRY_ARCHITECTURE_2026-09-02.md`](./TELEMETRY_ARCHITECTURE_2026-09-02.md). Ledger: [`ACTION_IDS_GTAD_2026-09-26.md`](./ACTION_IDS_GTAD_2026-09-26.md).  
...
`src/` hits remain: an empty WorldExploration intent comment (`WX` **16442–16446**), `longHorizonSim.telemetry.available = false` (**532–536**), and an optional combatant `intelligence` field (`gameTypes.ts` **307** — AI flavour, **not** an admin tab). No `recordTelemetryIncrements`. No `telemetrySidecar`. No `telemetryLifetime` map. Repo search for those symbols is empty.

The 2026-08-31 through 09-25 **fail-open increment sidecar** is **reaffirmed**. This run does **not** invent a second architecture. `main` has **not moved** since the 09-21 cron (`0f5363f`). EVENT_SOURCE line numbers below were **re-read** on this SHA (battle start **12097–12099**, recap **12517**, dual death **13261** vs **13316**, flee **18935**, plague-zone lethal **14326–14331**, GameKey helper **275–279**, recap dismiss five `onClose` sites). They match 09-25 except the plague-zone linchpin caption on C-007.

This file patches three implementer hazards that 09-25 left stale:

1. **Open-PR queue grew again.** Oldest-first merge order is still **#327** then **#331**, then **#333–#636** — **306** still-open PRs targeting `main` (createdAt ascending, GitHub list 2026-09-26 ~00:16 UTC). Unmerged GTAD designs are **#352** (09-21), **#422** (09-22), **#450** (09-23), **#527** (09-24), and **#583** (09-25). Do not hitchhike telemetry maps, bindgen, or sidecar call sites onto any of them. Unique 09-26 files must not overwrite those unique files. Pre-existing queue breaks (#389 WX, #392 ChatPanel, later combat/map/persist PRs) are not this PR’s unique delta.
2. **Queued persist PRs still are not EVENT_SOURCE.** 09-25 deferred Q-018 against **#375 / #377 / #387 / #391 / #426 / #431 / #435 / #439 / #445 / #482 / #488 / #490 / #493 / #497 / #499 / #508 / #532 / #540 / #545 / #552 / #577**. Those remain open. Additional persist PRs opened after the 09-25 cron: **#599** (skip `saveBattleStats` wipe after seeded portal keep), **#602** (skip rename and spell upgrade while Death Realm is pending), **#604** (skip feat claim and GameKey redeem while Death Realm is pending). On **main**, `noteUnconfirmedCredit` is still only the one-shot settle path (`dokaPersist.ts` **217–239**). No `unseeded*` skip helpers exist on `main`. Do not invent spend-note writers. Do not treat unmerged branch helpers as live increment sites.
3. **Plague-zone lethal is a linchpin caller, not HP-watch.** Re-read 2026-09-26: `isPlagueZone` tick (`WX` **14313–14332**) logs `[DEATH-BISECT] source: plague-zone` and calls `_handlePlayerDeath()`. Lava/spike deaths still use HP-watch (**13316+**) and still **do not** call the linchpin. C-007 enum gains `plague` (same METRIC_ID; not a new series). Recap dismiss five-path list is still correct: Escape **67–68**, backdrop **93–96**, overlay Enter/Space **98–111**, close **208**, Continue **609**. Increment **once**. Sibling TADD **#616** skipped a sixth Health matrix because these APIs are still absent. Sibling TBC **#609** remains `WAITING_FOR_TELEMETRY`.

`GTAD-2026-09-01-001`…`014` remain the Phase 0/1 implementation tickets. Use **this file’s `EVENT_SOURCE` line numbers**. New IDs this run are stack-fence / extra persist-deferral / plague-tag caption only. **No new metric IDs.**

Balance analyst and `longHorizonSim.telemetry.available` must stay **WAITING_FOR_TELEMETRY**.

## 1. What changed since 2026-09-25

| Topic | 2026-09-25 (PR #583, SHA `0f5363f`) | 2026-09-26 (this cron, same SHA) |
| :--- | :--- | :--- |
| Production telemetry | None | **Still none.** Same two `src/` hits + combatant `intelligence?: number`. Re-verified: no `recordTelemetryIncrements` / `telemetrySidecar` / `telemetryLifetime`. |
| `main` HEAD | `0f5363f` | **Unchanged.** Line numbers below match a fresh read of this SHA. |
| Open sibling PRs | **247** open through **#579** | **306** open through **#636**. Oldest still **#327** then **#331**. GTAD siblings **#352** + **#422** + **#450** + **#527** + **#583**. TBC WAITING **#609**. Dashboard **#616** skipped a sixth Health matrix. |
| Persist PRs (not EVENT_SOURCE) | #375…#577 as listed in 09-25-004 | Those still open, **plus** #599/#602/#604. Q-018 still deferred. No `unseeded*KeepWriteSkip` on `main`. |
| Recap dismiss | Five `onClose` sites | **Re-verified.** Same five paths (Escape 67–68, backdrop 93–96, Enter/Space 98–111, close 208, Continue 609). |
| Plague-zone death | Not called out as a linchpin caller | Lethal plague tick **does** call `_handlePlayerDeath` (`WX` **14326–14331**). C-007 enum adds `plague`. Lava/spike still HP-watch only. |
| Shop mint | `shouldCommitGameKeyRedeem` | Unchanged (`shopPurchase.ts` **275–279**; UI **184**). Remount `creditPendingPurchasesThroughPersist` **1494** still a dead writer (`processPendingPurchases` returns **0**, `main.mo` **1338–1348**). |
| One-shot / absolute skip | Q-015 / Q-016 | Unchanged. `settleOneShotAfterCredit` **144–152**; `shouldSkipAbsoluteDokaWrite` **202–212**. |
| EOP | New later file after frozen `20260901`; `check-limit` 5→6 | Unchanged. `.old` stays Caffeine Aug-31 (no GameKey). `mops.toml` **check-limit = 5**. |
| Dual death | Two persist entries | Unchanged **plus** plague as a third *tag* on the linchpin. `_handlePlayerDeath` **13261**; HP-watch **13316+** still does **not** call it (recap **13355**). Flee **18935** does. Plague **14331** does. |
| Family / summoner | `spawnPolicy` 30% + live summoner roll | Unchanged. `applyFamilyVariantsToRoster` **289**; overwrite `sp`/`sr`/`init`/`res`/`chc` **11873–11903**; summoner **11932–11943**. |
| World features | Catalog only; WX does not import | Unchanged on `main`. Open world-dynamics / encounter catalogs still **no** `WorldExploration.tsx` importer. N-007 stays dormant. |
| `WorldExploration.tsx` | 19,213 lines | **19,213**. `setInBattle(true)` / `BATTLE_START` **12097–12099**. Safety 2s **12167–12170**. Opening-turn count **12139–12145**. |
| Admin tabs | No intelligence / Health | Unchanged (`gameTypes.ts` **483–498**; labels **5611–5625**). Purchases = GameKey inbox. |
| `APP_VERSION` | v163 | Still **v163** (`App.tsx` **14**). Version-gate keep list on `main` is still spawn/level-up + `*_inventory` only (`versionGate.ts` **9–11**). |
| Identifiable dumps | Leaderboard, OQL email, `getAllCharacters` | Same. `getAllCharacters` **532**. `getLeaderboard` **3390**. OQL `include Expose` **3498**; `gameKeyRequests.email` **3788**. `CharacterStats` still 12 fields (**147–160**). |

Merged to `main` since 09-02: persist/combat/map/admin-safety PRs through **#332**. **No increment sidecar shipped.** Prior telemetry PRs remain docs only. **No `main` merge since 09-21.**
...
| :--- | :--- | :--- |
| `src/frontend/src/debug/debugLogger.ts` | Local ring buffer (`DEBUG_BUFFER_CAP` **2000**, **44**). Shift+D. Console dev-only. | Session-local. Not aggregated. |
| `debug/clickTrace.ts` | Click-geometry ring, cap **20** (`CLICK_TRACE_CAP` **58**). DEV-gated callers. | Dev-only. Never a production funnel. |
| `WX` **16442–16446** | Empty “caller-side telemetry” around `action.intent`. | No emit. Intent strings stay local TURN logs. |
| `data/pieceArt.ts` **40**, **876+** | Throttled `logPatternLookupFailed` → `king.front`. | Local `SUMMON` log. |
| `handleBattleEnd` catch **12611** | `"Reward persistence failed (non-blocking)"`. Recap already shown (**12517**). | Fail-open **pattern**, not a counter. |
| Boss Rush catch **12863** | `"BossRush reward persist failed"`. Recap after persist is fired (**12925**). | Same pattern. |
| `getLeaderboard` (`main.mo` **3390**) | Top 50: `principalId`, `playerName`, `level`, `killCount`, `achievementsCompleted`. | Identifiable game feature. **Do not reuse as Intelligence.** |
| `getAllCharacters` (`main.mo` **532**) | Full slots + names + principals. Unused by AdminDashboard. | Identifiable dump. **Phase 0 must scan server-side and return buckets only.** |
| OQL `Expose` (`main.mo` **3498**) | `characterSlots` / `dokaBalances` / `userProfiles` / `dungeonRecords` / **`gameKeyRequests` (email)** `ownedBy` owner. | Controller can list principals and **emails**. **Do not add a telemetry entity with an owner column.** |
...
| `chatMessages` | In-memory, cap 200, dropped on upgrade. `sendMessage` (`main.mo` **2610**) binds `playerName` from `userProfiles`. | **Do not collect message content.** |
| Admin dashboard | Catalog CRUD + Purchases GameKey inbox (`AdminDashboard.tsx` **5611–5625**). | Not battle outcomes. No `intelligence` tab. |
| `useSaveKillCount` | `useLeaderboardQueries.ts` **43**. | **Zero TSX callers.** `killCount` is not battle count. |
| `longHorizonSim.ts` **532–536** | `telemetry.available: false`. | Correct. Do not calibrate from source. |
| `calculateAndAwardDoka` (`main.mo` **3073–3076**) | Always `0`. | Not a mint. Do not chart. |
| `adminVisualStatus.ts` | World never loads `spriteUrl`. Empty URL is default pixel visual, not a custom-URL fail. | Q-009 stays dormant until a loader exists. |
| Combatant `intelligence?` | `gameTypes.ts` **307**. | AI flavour field. **Not** an analytics tab. |
| Quality audit `AQA-2026-08-30-012` | persist-ok/fail, victory paid, death-penalty, recap open/dismiss, shop credit. | Still unimplemented. Shop credit **site** is GameKey + `shouldCommitGameKeyRedeem`. |
...
| `gameKeyRequests` | Request id | **Counts by `status` only**. Never email. |
...
| :--- | :--- |
| Battle start | `WX` **12097–12099** `setInBattle(true)` + `onDebugLog("BATTLE_START")` |
| Victory | `handleBattleEnd(true)` **12262**; recap first **12517**; persist `try/catch` **12611** |
| Defeat (non-death) | `handleBattleEnd(false)` after the victory branch (rare vs HP-watch / `_handlePlayerDeath`) |
| Combat / DoT / flee death | `_handlePlayerDeath` **13261**. Flee **18935** calls this. |
| Plague-zone lethal | Player-turn plague tick **14326–14331** — **does** call `_handlePlayerDeath` (`source: plague-zone`). C-007 `plague`. |
| Lava / spike death | HP-watch **13316+** — still **does not** call `_handlePlayerDeath` |
| Death persist | `persistDeathPenalty` → `saveBattleStats` (20% XP / 40% Doka) |
| Unpaid death replay | `flushPendingDeathPenalty` (`deathPenalty.ts` **572**); may run as persist `beforeEach` |
| One-shot ground / shrine / dungeon bonus | `settleOneShotAfterCredit` after `applyRewards`; keep on transport miss |
| Absolute write skip | `shouldSkipAbsoluteDokaWrite` when `unconfirmedWalletCredit` and live ≤ committed |
| Boss Rush room | `persistBossRushRewardsThroughLock` **12813** + `applyRewards` on the persist lock |
| Dungeon step | `snapshotDungeonChain` **before** `cleanupMap` (**6288**), then `decideDungeonChainPortal` (**6293**) |
| Challenge complete | `isChallengeCompleted` inside `handleBattleEnd` (`challengeCompletion.ts` ids `easy_1`…`legendary_3`). Persist uses **refs**, not ChallengePanel visibility. |
...
| Family variant | `applyFamilyVariantsToRoster` (`spawnPolicy.ts` **289**; WX **5862**) |
| Summoner roll | Battle start **11932–11943** |
| Pattern fallback | `drawCombatant` → `king.front` + throttle |
| Recap dismiss | `PostBattleRecap` `onClose` (**five** UI paths; increment once) |
| Reward persist fail | catch **12611** / Boss Rush **12863** (non-blocking) |
| Shop mint | `redeemGameKeyThroughPersist` after `#ok` **and** `shouldCommitGameKeyRedeem` |
...
2. Enqueue on `createProgressPersist` (credits, `saveBattleStats`, GameKey redeem, and the unpaid-death `beforeEach` already share that lock).
...
9. Edit a frozen migration `NewActor` (`20260801` / `20260803_185500` / `20260827` / `20260831` / `20260901`) or rename/delete a recorded chain file.
10. Hitchhike onto still-open **#327** / **#331** or any later still-open persist/combat PR (through **#636**, including **#599** / **#602** / **#604**). Dedicated human-approved PR after those land, or a human-unioned restack. Unique 09-26 files must not overwrite unmerged **#352** / **#422** / **#450** / **#527** / **#583**.
...
| Allowed | Forbidden |
| :--- | :--- |
| Phase 0 **query-only** bucket scans | Enqueue increments **on** the persist lock (including `beforeEach`) |
| Fire-and-forget increment **after** the persist function returns | A second wallet / XP writer |
| Swallow increment failures | Block `applyRewards` / `saveBattleStats` / `redeemGameKey` / one-shot settle if telemetry fails |

### 3.2 Shop-credit correction (still 2026-09-21)

| Allowed increment site | Forbidden increment site |
| :--- | :--- |
| After `redeemGameKeyThroughPersist` returns with `shouldCommitGameKeyRedeem(seeded, granted)` | `creditPendingPurchasesThroughPersist` remount (`WX` **1494**) — canister always returns 0 |
| After redeem `#err`, enum reason only | Treating `shouldCommitShopCredit` as the GameKey predicate |
...
### 3.3 One-shot / absolute-write correction (still 2026-09-21)

| Allowed increment site | Forbidden |
| :--- | :--- |
| After `settleOneShotAfterCredit` / `resolveOneShotCreditSettle` returns (`commit\|release\|keep`) | Releasing a claim because the sidecar threw |
| After `shouldSkipAbsoluteDokaWrite` skips a heal/shop snapshot | Inventing a confirm-and-mint path for telemetry |
| After `flushPendingDeathPenalty` returns (Q-014) | Enqueueing the increment inside `beforeEach` |

### 3.4 Recap dismiss correction (still 2026-09-22; re-verified 2026-09-23 and 2026-09-24)

| Allowed | Forbidden |
| :--- | :--- |
| One `quality.recap.dismissed` when `onClose` runs for a given recap instance | Five increments from Escape + backdrop + Enter + close + Continue |
| Treating overlay Enter/Space (**98–111**) as a dismiss path | Calling line **111** “backdrop” (backdrop is **93–96**) |
...
- **OQL** returns `owner` principals and **emails** on `gameKeyRequests`.
...
- **`getAdminAuditLog`** is a last-100 **ops** ring with `adminPrincipal` (`main.mo` **3486**; bindgen **839 / 2435**). Dashboard Health H14 may count `action` after bindgen; Intelligence gameplay charts must not dump principals.
...
Client: call **after** recap is shown / death penalty applied / persist `try` settled / GameKey redeem returned / one-shot settle returned. Spell **ids**, not names. Coarse buckets for HP, turns, level delta.
...
| One-shot settle | Rare | **100%** | Lifetime + 28 UTC days |
...
**Key cardinality budget:** catalog-sized dimensions only — `spellId`, `pieceType`, `EnemyFamily` enum, `bossId`, `challengeId`, `modifierType`, `deathCause` enum, `achievementId`, GameKey `status` enum, one-shot `settle` enum. No free-text. No map ids that include player names. No emails. No GameKeys.
...
| **1 — Outcome + quality increments** | Sidecar + `recordTelemetryIncrements` + persist ok/fail, victory paid, death penalty, recap open/**dismiss**, **GameKey redeem committed**, one-shot settle, absolute-write skip, battles started / victory / defeat / flee. | Answers AQA-012 with a live shop site. Fail-open. |
...
**Motoko / EOP note:** add telemetry maps in a **new** `src/backend/migrations/YYYYMMDD_HHMMSS.mo` that sorts **after** `20260901_000000`. `OldActor = {}`. `NewActor` = empty maps + zero epoch. Bump `mops.toml` `check-limit` to **6**. Run `python3 scripts/check-eop-stables.py` and `bash scripts/caffeine-import-gate.sh backend` (check-stable vs `.old` = Caffeine Aug-31 signature **and** every `snapshots/deployed/*.most` + `empty-canister.most`; `unsupported/` must still fail). Do not land this in the same PR as open **#327** / **#331** or later persist/combat PRs. Do not restack unique 09-24 docs by concatenating a second copy of a helper into a sibling file.
...
| `quality.oneshot.keep` += 1 | Pickup coordinates, map seeds, claim ids |
...
Metric IDs `GTAD-C-001`… are **stable** from 2026-08-31; rows marked **NEW 2026-09-01**, **NEW 2026-09-02**, **NEW 2026-09-21**, **NEW 2026-09-22**, or **NEW 2026-09-23** are additive. This run adds **no new metric IDs** — only queue/persist-deferral evidence (09-24).
...
EVENT_SOURCE: `WorldExploration.tsx` **12097–12099** after `setInBattle(true)` / `onDebugLog("BATTLE_START")`. One increment per successful battle commit, including Boss Rush rooms. Do not double-count the 2s safety `setInBattle(true)` at **12167–12170**.  
...
EVENT_SOURCE: `handleBattleEnd(true)` after recap is queued (**12517**), **not** inside `resolveBattleRewards`. Boss Rush: after recap is shown in the room-clear path (**12925**); “victory paid” still **after** persist returns (**12813** / catch **12863**).  
...
EVENT_SOURCE: `_handlePlayerDeath` **13261** when `inBattleRef` is true **and** the caller is not the flee button.  
AGGREGATION: Counter; dim `mode`.  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Combat difficulty vs hazard difficulty (see C-007).  
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low.  
IMPLEMENTATION_COMPLEXITY: Medium — must tag the caller (flee vs combat vs DoT) without changing death math. The linchpin comment is still wrong: HP-watch is a second entry (**13316+**).  
STATUS: PROPOSED  

METRIC_ID: GTAD-C-004  
NAME: Flees  
PURPOSE: Distinguish “I quit this fight” from “I died”. Flee **is** death (`onEndBattle` **18935** → `_handlePlayerDeath`).  
...
EVENT_SOURCE: `challengeTurnCountRef` / `battleTurn` at `handleBattleEnd` or death. Opening player turn is already counted (`shouldCountOpeningPlayerTurn` at **12139–12145**).  
...
EVENT_SOURCE: Tag **both persist entries** and the plague linchpin caller. Flee **18935**; lava/spike HP-watch **13316+** (still does **not** call `_handlePlayerDeath`); combat/DoT via `_handlePlayerDeath` callers; plague-zone lethal **14326–14331** (`source: plague-zone`, **does** call `_handlePlayerDeath`). Enum only: `combat_melee|combat_spell|dot|lava|spikes|plague|flee|other`. Same METRIC_ID as 08-31; `plague` is an enum value, not a new series.  
...
IMPLEMENTATION_COMPLEXITY: Medium (thread `DeathCause` into **two persist entries** plus the plague caller; do not change `persistDeathPenalty`).  
STATUS: PROPOSED  
...
EVENT_SOURCE: Battle-start roster (`pieceType`, `family` if not `default`/`""`, optional `enemyConfig.id`) + outcome. Prefer closed enums (`gameTypes.ts` **12–20**) over assigned display names.  
AGGREGATION: `combat.family.{family}.win|loss` += 1 per battle that contained that family (not per corpse).  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Enemy / kit / tier-spawn edits. **Caveat:** battle start overwrites family `sp`/`sr`/`init`/`res`/`chc` (`WX` **11873–11903**). Family HP/damage from the 30% roll can still be live; `applyEnemyFamilyStats` writes `res`/`sp` at spawn (`spawnPolicy.ts` **261–272**) and battle start then overwrites those. Do not treat family W/L as proof the overwritten rows are live.  
...
EVENT_SOURCE: `handleBattleEnd` `deathTriggered` early return (**12276**); `cleanupBattle` while `inBattleRef` without an outcome increment; persist catch that still showed a recap (**12611**).  
...
PURPOSE: There is **no** persisted spell-drop / first-seen store. `ownedSpells` (`WX` **2412–2440**) = starters ∪ backend rows with `usableByPlayer !== false` (or already on `spellLevelKeys` / `spellBarOrder`). `minLevel` is catalog metadata only. SDA-002 (owned/observed ids) is design-only.  
...
EVENT_SOURCE: WX `action.kind === "cast"` after the intent comment (**16449+**); boss AI casts. Unique-set per battle.  
...
PURPOSE: NEW 2026-09-01. `shouldIncludeBackendSpellInLibrary` hides `usableByPlayer === false` unless already on keys/bar (`adminSafety.ts` **712–719**).  
...
PURPOSE: `EnemyArchetype` is `caster|healer|charger|flanker|berserker|summoner|generic` (`enemyAI.ts` **86–93**).  
...
PURPOSE: The 30% `EnemyFamily` overlay (`spawnPolicy.ts` `FAMILY_VARIANT_CHANCE` **35**; WX **5862–5866**) is a content knob.  
...
PURPOSE: NEW 2026-09-02. Battle start flags `isSummoner` with `ENEMY_SUMMONER_CHANCE_BASE + level * PER_LEVEL_ZONE` (`WX` **11932–11943**; `data/gameConstants.ts` **298–299**).  
...
PURPOSE: NEW 2026-09-02; **predicate updated 2026-09-21**. Replaces the dead 60s package auto-complete as the **shop earn** series.  
EVENT_SOURCE: After `redeemGameKeyThroughPersist` returns (`shopPurchase.ts` **294–330**; UI `DokaGameKeyShop.tsx` **184**). Increment only when `shouldCommitGameKeyRedeem(persist.isWalletSeeded(), gained)` (**275–279**).  
...
EVENT_SOURCE: `decideDungeonChainPortal` after snapshot **6288–6293**; abort via `resetRunState` on death.  
...
EVENT_SOURCE: Panel accept + `isChallengeCompleted` at `handleBattleEnd` / room clear. Ids `easy_1`…`legendary_3`. Persist reads **live refs**, not ChallengePanel `visible` (HUD hide after first action must not look like decline).  
...
METRIC_ID: GTAD-N-008  
NAME: Enemy Register is lore, not encounters  
PURPOSE: NEW 2026-09-21. `EnemyRegister` / `enemyRegisterCopy.ts` is flavour text. It is not a bestiary, spawn table, or encounter log.  
EVENT_SOURCE: **None.** Do not increment from Register open/close. Do not join Register blurbs to C-008 / E-001.  
AGGREGATION: Deferred / never.  
RETENTION: n/a.  
OWNER_DECISION_SUPPORTED: Prevents fake “players studied X family” charts.  
PERFORMANCE_COST: None.  
PRIVACY_RISK: None.  
IMPLEMENTATION_COMPLEXITY: None (explicit non-metric).  
STATUS: PROPOSED  
...
EVENT_SOURCE: After `readApplyRewardsOk` throws / `#err` — **outside** the persist-lock `enqueue` (existing `catch` **12611**; Boss Rush **12863**).  
...
EVENT_SOURCE: `onShowBattleSummary` success (**12517** / room clear **12925** / HP-watch defeat **13355**).  
...
PURPOSE: UX cancellation. Discrete handlers in `PostBattleRecap.tsx`. **Corrected 2026-09-22:** five `onClose` sites, not four.  
EVENT_SOURCE: `onClose` **once** per recap. Paths: Escape **67–68**, backdrop click **93–96**, overlay Enter/Space **98–111**, close button **208**, Continue **609**. 09-21 mislabeled **111** as backdrop. Do not scrape canvas clicks.  
...
PURPOSE: Paid Doka actually landed. Increment predicate is `shouldCommitGameKeyRedeem`, not `shouldCommitShopCredit`.  
EVENT_SOURCE: After `redeemGameKeyThroughPersist` commit when `shouldCommitGameKeyRedeem`. **No email, no code.** Do not increment on WX remount `creditPendingPurchasesThroughPersist` (**1494**) — `processPendingPurchases` returns 0 (`main.mo` **1338–1348**).  
...
PURPOSE: Admin-configured URLs (player sprites, enemy `spriteUrl`, ad boxes). World draw is still pixel patterns — **do not count empty or stored-not-rendered URL as CUSTOM_FALLBACK** (`adminVisualStatus.ts`).  
EVENT_SOURCE: `onerror` only when a **non-empty** URL is actually fetched and fails. **Do not send the URL.** Send `kind=enemy_sprite|player_sprite|ad_box`. World currently does not fetch these URLs — metric stays dormant until a loader exists.  
...
PURPOSE: `drawCombatant` falls back to `king.front` (`pieceArt.ts` **40**, **876+**). This is missing `pieceType`/palette — a content bug — not a custom-URL fail.  
...
PURPOSE: NEW 2026-09-02. `flushPendingDeathPenalty` exists because a failed 20/40 can be overwritten by portal/heal/shop. Persist lock may invoke it as `beforeEach`.  
EVENT_SOURCE: After `flushPendingDeathPenalty` (`deathPenalty.ts` **572–589**): `write` success vs fetch-null vs `action !== "write"` clear. Enum only.  
...
IMPLEMENTATION_COMPLEXITY: Low. Do not add a second penalty writer. Do not increment inside `beforeEach`.  
STATUS: PROPOSED  

METRIC_ID: GTAD-Q-015  
NAME: One-shot credit settle  
PURPOSE: NEW 2026-09-21. Ground Doka / shrine / dungeon-complete claim `applyRewards` is not idempotent. Transport miss must **keep** the claim (`settleOneShotAfterCredit`). Confirm may later `commit` via live wallet (`confirmKeptOneShotCredit`) only when the lock is already seeded.  
EVENT_SOURCE: After `resolveOneShotCreditSettle` (`dokaPersist.ts` **194–215**) returns. Enum only: `commit|release|keep|confirm_commit`.  
AGGREGATION: `quality.oneshot.{commit|release|keep|confirm_commit}`.  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Whether pickup remint / wipe races still fire.  
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low. Do not store claim ids, coordinates, or amounts.  
IMPLEMENTATION_COMPLEXITY: Low. Fail-open; never release a claim because the sidecar threw.  
STATUS: PROPOSED  

METRIC_ID: GTAD-Q-016  
NAME: Absolute write skipped (unconfirmed credit)  
PURPOSE: NEW 2026-09-21. Seeded transport-keep left the lock at the pre-credit wallet; recap heal `saveBattleStats` wiped the canister grant. `shouldSkipAbsoluteDokaWrite` (`progressPersist.ts` **202–212**) skips that snapshot.  
EVENT_SOURCE: When an absolute write is skipped for `unconfirmedWalletCredit` (live null or live ≤ committed).  
AGGREGATION: `quality.persist.absolute_skip.unconfirmed_credit`.  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Whether the wipe-after-keep class still occurs.  
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low.  
IMPLEMENTATION_COMPLEXITY: Low. Do not change skip math to “make the counter fire.”  
STATUS: PROPOSED  

METRIC_ID: GTAD-Q-017  
NAME: Persisted AP/MP vs formula divergence  
PURPOSE: NEW 2026-09-21. Battle start already logs when persisted `characterStats.ap/mp` diverge from `getPlayerBaseStats` (`WX` **12117–12133**). Combat uses the formula floor.  
EVENT_SOURCE: That existing warn, **session-once** (flag `_progressionDivergenceWarned` already exists).  
AGGREGATION: `quality.progression.ap_mp_diverge` += 1.  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Whether stale persisted AP/MP rows are common after a growth-percent change.  
PERFORMANCE_COST: Negligible (already session-once).  
PRIVACY_RISK: Low. Do not send the persisted values.  
IMPLEMENTATION_COMPLEXITY: Low. Phase 1 optional.  
STATUS: PROPOSED  

METRIC_ID: GTAD-Q-018  
NAME: Throw-after-mutation unconfirmed wallet note  
PURPOSE: NEW 2026-09-22; **deferral expanded through 2026-09-26**. Open persist PRs (#375 keep unconfirmed Doka through spends, #377 GameKey/claim throw-after-credit, #387 rename/upgrade throw-after-debit, #391 unpaid death before GameKey lock credit) would add `noteUnconfirmedCredit` / spend-note calls after the canister already mutated. After 09-22: **#426** skipBeforeEach death recut, **#431/#439** unseeded HUD Doka, **#435** flushed GameKey/feat vs first hydrate, **#445** respawn HP after unpaid-death remount. After 09-23: **#482** refuse stale unseeded seed, **#488** unseeded small-feat skip, **#490** principal-keyed BuffShop/feats, **#493** unseeded one-shot keep skip, **#497** unpaid death Doka on every slot, **#499** unseeded victory keep skip, **#508** keep unpaid death keys across version wipe. After 09-25: **#599** seeded portal-keep skip, **#602** skip rename/upgrade during Death Realm, **#604** skip feat/GameKey during Death Realm. On **main today**, `noteUnconfirmedCredit` is already used from one-shot settle (`dokaPersist.ts` **217–239**). That live path is Q-015 keep + Q-016 skip — **do not double-count it here**. Those unseeded-skip helpers are **not** on `main`.  
EVENT_SOURCE: **None on `main` beyond Q-015/Q-016.** After those PRs merge: fire-and-forget **after** the note helper returns, off the lock. Enum only: `credit_throw|spend_throw`. Do not invent the helpers in the telemetry PR. Do not treat unmerged branch line numbers as EVENT_SOURCE. Do not count version-gate key retention (#508) as a player death or a shop mint.  
AGGREGATION: `quality.persist.throw_after.{credit|spend}` (only once the helper exists on `main`).  
RETENTION: Lifetime + 28 days.  
OWNER_DECISION_SUPPORTED: Whether throw-after-add/debit races still fire after the persist PRs land.  
PERFORMANCE_COST: Negligible.  
PRIVACY_RISK: Low. Do not store amounts or principals.  
IMPLEMENTATION_COMPLEXITY: Low **after** the persist PRs merge; **do not implement against unmerged branches**.  
STATUS: PROPOSED  
...
| Intelligence UI over `getLeaderboard`, OQL owner rows, `getAllCharacters`, or `adminListGameKeyRequests` | Identifiable (includes **email**). |
| Joining `PurchaseRecord` / GameKey email / ledger codes into gameplay charts | Different purpose (fulfillment). |
| Telemetry **on** `createProgressPersist` (including `beforeEach`) | Wallet races. AQA enqueue wording is a hazard. |
| Counting `processPendingPurchases` remount as shop credit | Writer is a stub returning 0. |
| Using `shouldCommitShopCredit` as the GameKey predicate | Wrong helper; misses `#ok` grants. |
...
| Charts for `worldFeatures` / formations / discovery / Enemy Register as if live | Catalog-only or lore. N-007 / N-008. Open **#503** is still not a WX importer. |
...
| Editing frozen `NewActor`s or hitchhiking telemetry onto #327 / #331 / later persist-combat PRs (now through **#636**) | EOP + stack-compat. Overwriting #352/#422/#450/#527/#583 unique files is also forbidden. |
| Treating `getAdminAuditLog` as player telemetry | Ops ring with `adminPrincipal`; TADD H14 only, action counts. |
| Combatant `intelligence` stat as an Intelligence tab | Unrelated field. |
| Releasing a one-shot claim because telemetry failed | Fail-open; remint hazard. |
| Implementing Q-018 against unmerged persist PRs through **#604** (including #532/#540/#545/#552/#577/#599/#602) | EVENT_SOURCE must be on `main`. |
| Five recap-dismiss increments | One `onClose` per recap instance. |
...
- No principal search. No “inspect this player.” No GameKey inbox embed. No Enemy Register embed.
...
2. New persistent `let`/`var` = a **new later** chain file after `20260901_000000` (`OldActor = {}`). Never edit a shipped `NewActor`. Never rename/delete `20260801` / `20260803_185500` / `20260827` / `20260831` / `20260901`. Bump `mops.toml` `check-limit` from 5 to the new chain length. Keep `.old` byte-identical to Caffeine’s Aug-31 deployed `.most` (no GameKey). Run `python3 scripts/check-eop-stables.py` and `bash scripts/caffeine-import-gate.sh backend`.
3. Client sidecar **outside** `WorldExploration.tsx` (**19,213** lines). Flush from `rewardResolver`, `deathPenalty`, `bossRushProgress`, `challengeCompletion`, `shopPurchase` (`redeemGameKeyThroughPersist`), `dokaPersist` (`resolveOneShotCreditSettle`), `spellUpgrade` **after they return**.
4. Tests: sidecar swallows throws; increment API is **not** a required persist-lock mock; fail-open if `recordTelemetryIncrements` is missing on the mock actor. One-shot `keep` must still `keep` if the sidecar throws.
5. Bindgen after Candid change (`pnpm bindgen`). Telemetry must not ride on `updateCharacter`. `CharacterStats` stays 12 fields (`main.mo` **147–160**).
6. Import gate: `pnpm typecheck` && `pnpm check`. Motoko / mocks / `.old` → `mops check` or `caffeine check`. Unused locals and hook-deps are **errors**. Also `bash scripts/open-pr-stack-compat.sh --self`.
7. Do not modify RAF, map generation, turn logic, or damage math to “make metrics easier.”
8. Do not call `getAllCharacters`, OQL `execute`, or `adminListGameKeyRequests` from the Intelligence tab.
9. Do not land telemetry stables in the same PR as open **#327** (Striker / WX / challenge) or **#331** (portal destack / WX / mapGen / persist) or later persist/combat PRs unless a human explicitly unions them. Unmerged GTAD designs are **#352** (09-21), **#422** (09-22), **#450** (09-23), **#527** (09-24), and **#583** (09-25) — unique 09-26 files do not overwrite them. Union README if edited; do not concatenate duplicate `export function` copies if a later implementer restacks TypeScript.
10. GameKey increment predicate is `shouldCommitGameKeyRedeem`, not `shouldCommitShopCredit`.
11. Recap dismiss: wrap `onClose` once. Do not increment from each of the five UI paths.
12. After older queue items merge, **re-read EVENT_SOURCE line numbers** before implementing. This catalog is valid at `0f5363f` only. The 306-PR queue will move WX/persist lines the moment **#327** / **#331** land.
...
| :--- | :--- | :--- |
| Quality Auditor (`976261d8`) | INCONCLUSIVE. Do **not** require persist-lock enqueue. Shop credit = GameKey redeem via `shouldCommitGameKeyRedeem`, not `processPendingPurchases`. Cite Q-015/Q-016 as **proposed**, not live. Cite Q-018 only after the persist PRs merge. Recap dismiss = one increment despite five UI paths. | Cite `quality.persist.*`, `combat.outcome.*`, `quality.death_penalty.*`, `quality.recap.*`, `quality.shop.redeem_*`, `quality.death_replay.*`, `quality.oneshot.*`, `quality.persist.absolute_skip.*`. Missing increment ≠ regression. |
| Balance / Content Analyst (`2786666f`) | Stay `WAITING_FOR_TELEMETRY` (open **#609** already says so). | Use Phase 2 dimensions; never invent OVERPERFORMING from source. Enemy Register is lore (N-008). |
| Dashboard Designer (`4b026695`) | Snapshot panels + honest “not measured.” Shop series = GameKey. H14 audit log is ops, not battles. Open **#616** correctly skipped a sixth Health matrix. | Bind to `adminGetTelemetrySnapshot` / progression snapshot only. |
| `longHorizonSim` | Keep `telemetry.available = false`. | Flip only when increment/snapshot APIs exist **and** are populated. |
| Expansion / discovery / formations / world-events | Design docs stay design. Open **#503** is still not a WX importer. | Add metric keys only when a persist/event source exists. E-008 summoner roll is live; `elite_patrol` and Enemy Register are not. |

Related: `AQA-2026-08-30-012` (still NEW; shop site patched in 09-21). `GTAD-2026-09-01-*` remain the implementation tickets; **read this file for line numbers, GameKey helper, one-shot, recap-dismiss, EOP, plague-zone linchpin tag, and the 306-PR stack fence.** `GTAD-2026-09-02-*` and unmerged `GTAD-2026-09-21-*`…`GTAD-2026-09-25-*` (PRs #352/#422/#450/#527/#583) are still NEW additive deltas. `GTAD-2026-09-26-*` are this run’s deltas only.
