# ACTION_IDs — Content Diversity & Repetition Auditor 2026-09-24

Durable ledger for implementers and the Report Action Orchestrator.  
Source of every record: Content Diversity & Repetition Auditor.  
Audit: [`CONTENT_DIVERSITY_AUDIT_2026-09-24.md`](./CONTENT_DIVERSITY_AUDIT_2026-09-24.md).  
...
Unpublished ledgers: GitHub **PR #359** (09-21, `016`–`018`), **PR #403** (09-22, copies `016`–`018` + `019`), **PR #465** (09-23, copies `016`–`019` + `020`). All still **OPEN**. Do **not** mint twins. Those ids are copied here so they exist on `main` if #359/#403/#465 never merge. Status stays **NEW** until an implementer picks them.

This run mints **`CDA-2026-09-24-021`** only.
...
## Status of CDA-2026-09-02-001 … 015, unpublished 016–020, and 021
...
| CDA-2026-09-24-021 | NEW | **This run.** Freeze Wave-7 TypeScript dumps (`#503` 52→116 `WF-*`) until 001/014/008/018 |
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: No `isElite` / variant type. Leader = highest level (WX 12010–12015). `computeAITier` (`combatMath.ts` 36–51) 30% uniform 1–10. `WF-ELT-BANNER_PATROL` / `WF-ELT-TOLL_KEEPER` / `WF-ELT-CART_GUARD` (`worldFeatures.ts`) are unwired. Re-verified 2026-09-24.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Live placement is 4-quadrant + Chebyshev ≥ 4 (WX 5745–5750) then battle-start re-scatter ≥ 2 (11869–11891). `docs/design/ENEMY_FORMATIONS_*` and `worldFeatures.ts` encounter slots are unwired. Re-verified 2026-09-24.  
...
AUTONOMY: HUMAN_APPROVE  
DEPENDENCIES: Formation drop docs; CDA-2026-09-02-001 (roles need families/kits)  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Shield ≡ Iron Skin (`spellData.ts` 30–47 vs 293–312: +30% RES / 3, range 3; AP 2 vs 3). Poison ≡ Venom (49–67 vs 394–415: 4×3 DoT). Expose ≈ Shadow Veil (373–393 vs 480–500: damage + `debuffStat: "res_sp"`). Blood Mend ≈ Rallying Cry (84–102 vs 416–436: self heal + +15% CHC / 2). Boss kits reuse Veil (7), Cursed Wound (6), Frost Nova (6), Swap (5). Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `ENEMY_SUMMONER_CHANCE_BASE + characterStats.level * ENEMY_SUMMONER_CHANCE_PER_LEVEL_ZONE` (`gameConstants.ts` 298–299; WX 11932–11939). Hits 1.0 at player level 44. Pets 50/50 wolf or archer only. Sentinel / bomber / wisp stay `usableByEnemy: false`. Comment says “scales with levelZone”; code uses raw level. Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Nine `DEFAULT_CHALLENGES` (`challengeCompletion.ts` 44–109). Three turn-count clones (15/10/5), three damage clones (50 / 30+no-heal / 0). Unique: `no_healing`, `under_8_ap_per_turn`, `direct_hit`. Honesty patches landed; types did not. Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `defaultAchievements()` (`admin.mo` 309–326): `doka_1000` + `doka_10000`; `betrayal_witness` + `double_betrayal`; `level_10` (`unstoppable`) on a game with no level cap; all rewards Doka. WX already fires `level_10` at 10 (3632). Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: `slime_flood` and `frozen_terrain` are identical `onMpCost: ×2` (`mapModifiers.ts` 154–172). Registry hooks for `blood_moon` / `mirror_field` are empty (260–277) but `spellEngine.ts` 895 applies ×1.25 and 901–907 applies 20% ST reflect. `gravity_well` / `fog_of_war` (280–296) are empty hooks with announce text. Numeric reskins: `titans_vigor`, `glass_realm`, `doka_fever`. Re-verified 2026-09-24.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Frontend `SPELL_ID_CATALOG` (`bossKits.ts` 29–62) = 32 ids, all treated as base (WX 2395–2400). Backend `defaultSpells()` (`admin.mo` 168–191) = six different ids with `linear`/`diagonal`/`hitTiles`. `defaultBossConfigs()` still lists `fireball`, `cursed_gust`, `entangle`, `mist_form` (`admin.mo` 358+). Live kits cannot assign those ids. Re-verified 2026-09-24.  
...
PRIORITY: P0  
CONFIDENCE: HIGH  
EVIDENCE: Enemy Register lists Crimson Spawn / Shadow Lurker / Storm Caller (`EnemyRegister.tsx` 71–88) that are not `EnemyFamily`. Chrome is FLAVOR LORE (`enemyRegisterCopy.ts` 8–15; banner 356–370). Wave 1–7 sheets already propose dozens of ids. Shipping another sticker repeats CDA-001’s failure at larger scale. `EnemyFamily` union still the original seven + `default` (`gameTypes.ts` 12–20). Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Dungeon only adds `[0,2,3,4,4,5]` enemies and `[0,1,2,2,3,3]` tier steps (`spawnPolicy.ts` 27–28; WX 5720–5722). ENC-* catalogs and `worldFeatures.ts` (52 `WF-*` on `main`; shrine, teleport, elite patrol, spell-bearing enemy) are unwired. Live fight structure is always kill-all. Re-verified 2026-09-24.  
...
PRIORITY: P2  
CONFIDENCE: HIGH  
EVIDENCE: Chrome is honest (`enemyRegisterCopy.ts` 11–15: “Flavor only — not the live spawn roster”; banner `EnemyRegister.tsx` 356–370). The seven live-family rows (`EnemyRegister.tsx` 28–70) still teach wall-phase, poison-on-hit, burning tiles, adjacent slow + regen, Weaken-from-range, magic immunity until physical. Live verbs are only ember melee burn, tide melee MP−1 (WX 16789–16821), void 25% reflect (`castHelpers.ts` 335–345). Crimson / Lurker / Storm (71–88) are not `EnemyFamily`. Re-verified 2026-09-24 on HEAD `0f5363f`.  
...
PRIORITY: P1  
CONFIDENCE: HIGH  
EVIDENCE: Four live reflects: `spell-mirror` (next ST, `spellEngine.ts` 917–926); `void_mirror` 25% of pre-crit (`castHelpers.ts` 335–345); `mirror_field` 20% ST (`spellEngine.ts` 901–907, WX 9538–9558); boss `reflectShieldActive` 30% (`castHelpers.ts` 363–375). Backend `reflect_barrier` is a fifth ghost (`admin.mo` 182). 09-02 census listed REACTION as Under; it is a numeric clone cluster. SPELL_PROPOSALS already reserved delayed execute / redirect / absorb — unused. Re-verified 2026-09-24.  
...
## New this run (do not twin 001–020)
