# Enemy & Boss Admin — 2026-09-24 re-audit

**Author:** Enemy & Boss Admin Content Designer  
**Automation:** `299b70f5-a498-11f1-a7d1-d6b4613131ce`  
**Verified against:** `origin/main` @ `0f5363f` (merge of PR #332; **same SHA as the 2026-09-21, 2026-09-22, and 2026-09-23 EBA re-audits**)  
...
**Prior ledgers not yet on `main`:** [`ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-21.md`](https://github.com/Mr-Melic/stralt/pull/360) (`EBA-2026-09-21-001`) on open draft PR [#360](https://github.com/Mr-Melic/stralt/pull/360); zero-ID ledgers on [#404](https://github.com/Mr-Melic/stralt/pull/404) (09-22) and [#449](https://github.com/Mr-Melic/stralt/pull/449) (09-23)  
**This run:** documentation only. No production, RAF, map generation, turn, or damage-math code. This PR does **not** edit the 08-31 contract file (PR #360 owns that 09-21 pointer). Unique `*2026-09-24*` filenames only.
...
`origin/main` has not moved since the 2026-09-21 EBA re-audit (`0f5363f`). Spot-checks on 2026-09-24 match the 09-21 / 09-22 / 09-23 line numbers. The 2026-08-31 design remains the owner-studio contract.

**None of EBA-2026-08-31-001–024 shipped as gameplay.** There is still no `EnemyDefinition` / `BossDefinition`, no draft→validate→preview→activate path, no authored AI, no variants, no kit/discovery chips, and no visual mode `none|asset|pool`. Grep on `src/` for those type / helper names returns **zero** matches.

**No new ACTION_IDs this run.** Do not re-file `EBA-2026-08-31-*`, `EBA-2026-09-01-*`, `EBA-2026-09-02-*`, `EBA-2026-09-21-001` (still covered by open PR #360), or the zero-ID 09-22 / 09-23 ledgers (PRs #404 / #449). Consume `SDA-*`, `VAL-*`, `AFDA-002` / `011`, `WDEAD-*`, `AEE-*`, Enemy Elite Evolution, Enemy Formations / EBMA.

Open-PR queue at audit time: **171** still-open PRs targeting `main`. Oldest `createdAt` first: [#327](https://github.com/Mr-Melic/stralt/pull/327), [#331](https://github.com/Mr-Melic/stralt/pull/331), then the 2026-09-21 wave starting at [#333](https://github.com/Mr-Melic/stralt/pull/333). EBA siblings already in that queue: [#360](https://github.com/Mr-Melic/stralt/pull/360), [#404](https://github.com/Mr-Melic/stralt/pull/404), [#449](https://github.com/Mr-Melic/stralt/pull/449). This run adds unique `docs/automation/*2026-09-24*` files so it stays merge-clean after those older siblings.

---

## 1. What still drives live encounters (re-verified 2026-09-24)
...
| Spawn roster | Chess piece + `pickEnemyLevelFromTiers` (WX 5770–5772) + placeholder `level*8+20` HP (5831–5832), then `applyFamilyVariantsToRoster` (5864–5866). Tables live in `spawnPolicy.ts` (`FAMILY_VARIANT_CHANCE = 0.3` at 35; `DUNGEON_SPAWN_DEPTH_CAP = 5` at 29). | EBA-001, 007, 008; **EBA-2026-09-21-001** (PR #360) |
| Closed level bands | `newEnemy()` / `newRegion()` default `levelMax` to **9999** (`AdminDashboard.tsx` 137–157). Editor Level Min/Max with `ELIGIBILITY_BAND_HINT` (248–249, rendered at 813): “New drafts default max to 9999 so high-level play still matches.” List chips `Lv min–max` (2224). `AdminGuard` **requires** the closed band (`adminGuard.mo` 273–277, 297–301). Caffeine mock still seeds `levelMax: BigInt(5)` (`mocks/backend.ts` 69, 80) — sample content, not a product cap. Live region match is still `level <= Number(r.levelMax)` (WX 3702). | EBA-002; 09-01-001; 09-02-001; consume WDEAD |
| Hidden 999 cap | `combatMath.ts` 58: `maxTier = Math.floor(999 / ts)`. Comment: “cap for reasonable range.” | EBA-003 |
| Boss source | `useBossQueries.ts` 1–21 and portal entry (`WorldExploration.tsx` 6486) read `pbv_boss_configs`. `getAllBossConfigs` unused by gameplay. `useSetBossConfig` / `useDeleteBossConfig` (`useAdminQueries.ts` 528–557) still write localStorage. | EBA-004; AFDA-002 |
...
| Boss scaling | Spawn `level + 5` (WX 6537) and `Math.min(50, res/sp)` (6559–6560). `getBossEffectiveStats` is Boss Guide + `longHorizonSim` only. | EBA-011 |
...
| AI inference | `inferArchetype` (`enemyAI.ts` 447–477): healer from heal flags, flanker from `pieceType === "knight"`, berserker from `family.includes("berserk")`. | EBA-009; consume AEE |
| Boss Rush | `placeBossRushSpawns` / `applyFinalizedLayout` then 100-HP placeholders (`family: "boss"` at WX 5343 / 5368). Room 9 still `weeping_pawn_2` (`useBossRush.ts` 127). CatalogNote admits live rooms come from `BOSS_RUSH_ROOMS` (`AdminDashboard.tsx` 7141–7146). Canister `adminRollbackBossRushConfig` exists (`main.mo` 1822) — flavour JSON rollback, not `BossDefinition`. | EBA-015, 018 |
...
| Hard delete | `adminDeleteEnemyConfig` is `Map.remove` (`main.mo` 798–805). No `adminRollbackEnemyConfig` / `adminRollbackBossConfig`. Bosses tab holds unused `_deleteBossConfig`. Canister `deleteBossConfig` is portal-assignment only. | EBA-021; 09-01-003; 018 |
...
Same as 09-22 / 09-23, still true on this SHA:

- `generateEnemies` `tryPlaceEnemy` still calls `computeEnemyStats(enemyLevel, pieceType, …)` (WX 5834–5838) where `pieceType` is the **player** chess piece (hook deps at 5871). The spawned unit stores `pieceType: randomPieceType` (5812). Battle start reseeds with `e.pieceType`. When EBA-001 / 09-21-001 bind `EnemyDefinition.pieceType` through `spawnPolicy`, pass the **unit** piece. Do not retune damage math.
...
- Tiers tab `SAMPLE_LEVELS = [1, 10, 25, 50, 100, 200, 500]` (`AdminDashboard.tsx` 3877) — consume WDEAD. Death Realm fallbacks still stamp `maxLevel: 5`. Consume WDEAD; do not “fix” kit width by reading that object (EBA-013).
- `computeAITier` still bands through 900 then a flat tier 10 (`combatMath.ts` 36–47). Sophistication may plateau; **rolled enemy level must not** (EBA-003). Mixin `admin-api.mo` 18–25 still writes via `AdminLib.setEnemyConfig` with **no** `AdminGuard` and **no** audit; `main.mo` does not include the mixin (**09-01-005 OPEN**). `longHorizonSim.ts` 54 still has a private `buildEnemyKit` (**09-01-004 OPEN**).
...
### EBA-2026-09-22 / EBA-2026-09-23

No IDs. Ledgers on [PR #404](https://github.com/Mr-Melic/stralt/pull/404) and [PR #449](https://github.com/Mr-Melic/stralt/pull/449). Do not re-file empty records for those dates.
...
**None.** Ledger: [`ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-24.md`](./ACTION_IDS_ENEMY_BOSS_ADMIN_2026-09-24.md).
