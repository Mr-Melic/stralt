...
| `enemyWander.ts` | Overworld wander tick skip, Chebyshev target pick (`isEnemyWanderFloor`), path-step interpolation / facing / `ENEMY_MOVE_INTERVAL_*` delay. WX still owns RAF gates (battle / shop / transition), `findPath`, and `syncCombatants`. Do not fold spawn keep-clears into wander |
