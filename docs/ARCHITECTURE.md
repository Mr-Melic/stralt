...
| `spawnPolicy.ts` | Overworld / dungeon enemy spawn policy: dungeon extra-count + tier-boost tables, valid-cell filters, family 30% variants, visual scale, unique `assignedName` picker (`createEnemyNamePicker`). `generateEnemies` still places units and re-rolls `aiTier`. Family catalog `ap`/`mp` are unused. Empty admin name query uses client `DEFAULT_ANCIENT_NAMES` (not the canister empty-list defaults after `"Senvaris"`); exhausted admin pool leaves `assignedName` undefined. Portal keep-clear is Manhattan ≤ 2; map-spawn keep-clear is Chebyshev ≤ 3 from (8,8); enemy spacing is Chebyshev ≥ 4 — do not merge these |
