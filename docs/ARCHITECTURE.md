...
| Module | Job |
| :--- | :--- |
| `20260826_000000.mo` | Empty-canister genesis (`OldActor = {}`). **Keep this name** — v356 / Stralt_V2 recorded `20260826_000000`. Do not rename to `20260801` (that was for the original stralt project's `20260803_185500` tail) and do not add `20260803_185500` (it would sort before this file). Must stay first so a fresh install compiles (M0263). |
...
| `20260831_000000.mo` | **Frozen — the v356 deployed tail.** Seeds `isSummon` / `summonAI` / `summonLifespan` / `summonUnitDef` on persisted admin `SpellConfig` rows; introduces rollback stables (`*Prev` / `has*Prev`) and `adminAuditLog`. **No GameKey** — PR #258 adding it here (and #311 keeping it here) is what broke the old project's Sep-1 deploy and the #311 import (M0263). |
| `20260901_000000.mo` | **Frozen — GameKey.** `OldActor = {}`; `NewActor` = `gameKeyRequests`, `gameKeyLedger`, `gameKeyReveals`, `lastGameKeyRequestAt`, `nextGameKeyRequestId` with empty / zero defaults. Runs on the v356 tail. |
